...
free_mem_details="`free -h | grep Mem | awk -F ' ' '{print "Total_RAM_Size:"$2",Free_Size:"$4",Buff_Cache_Size:"$6",Available_Size:"$7}'`"
echo "RAM_DETAILS=\"${free_mem_details}\"" >> ${build_info_file}
cron_count="`ps -auwxxx | grep "cron_wrapper" | grep -v "grep" | wc -l`"
echo "CRON_SCRIPT_COUNT=\"${cron_count}\"" >> ${build_info_file}
falcon_enabled="`ps -auwxx | grep "falcon" | grep -v "grep" | wc -l`"
if [ "${falcon_enabled}" != 0 ]; then
    echo "FALCON_ENABLED=\"TRUE\"" >> ${build_info_file}
else
    echo "FALCON_ENABLED=\"FALSE\"" >> ${build_info_file}
fi
...
getIDCbuildDetails()
{
	...
	if [ "${TEAM_SPECIFIC_INFO}" != "" ]
	then
		resolveTeamSpecificInfo
		if [ "${COMPARE_IDC_BUILD}" = "YES" ]
		then
		     if [ "${PRODUCT_NAME}" = "ZOHOCHATWMS" ]
			 then
			      BTYPE="us4_zohocorp"
			 else
			      BTYPE="us3_pre_main"
...
docker_pull()
{
		...
		echo "docker_pull process started at : " `date "+%Y-%m-%d %T"`
                for image in `echo ${IMAGE_ORDER} | tr "," " " `
                do
...
commitSASReadme()
{
       if [ "${MILESTONE_NEEDED}" = "YES" -o "${BUILD}" = "CRON" -a "${BOTPOST_BRANCH}" != "" ]
       then
           echo "In Readme Commit for Saslite"
           for rbranch in `echo ${BOTPOST_BRANCH} | tr "," " "`
           do
              if [ "${rbranch}" == "${ARG}" ]
              then
...
chatConnectPost()
{
        if [ "${BUILD}" = "CI" -o "${BUILD}" = "WEBHOST" -o "${BUILD}" = "CRON" -a "${BOTPOST_BRANCH}" != "" ]
        then
             ##echo "In Bot Post for Saslite builds"
             for bbranch in `echo ${BOTPOST_BRANCH} | tr "," " "`
             do
                  if [ "${bbranch}" == "${ARG}" ]
                  then
                        getReleaseNotes
...
buildMoveInvoke()
{
        for mbranch in `echo ${BUILD_MOVE_BRANCH} | tr "," " "`
        do
...
acc_val_md()
{
CO_ACC_VAL=`curl -k -X GET -H "PRIVATE-TOKEN:${PRIVATE_TOKEN}" https://cmtools-api.csez.zohocorpin.com/api/v1/get_service_access_tokens?repository_url=${CLONE_VALUE}`
ACCESS_VALUE=`echo $CO_ACC_VAL | sed "s@\"@@g"`
export ACCESS_VALUE
...
{	
	...
	then
		...
		elif [ "${PRODUCT_NAME}" == "WEBMONITOR" -o "${PRODUCT_NAME}" == "WEBFONTS" ]
                then
                       ...
                       then
                               ...
                               if [ "${PRODUCT_NAME}" == "HG_UTILS" -a  "${hg_clone_arg}" != "integ_build_branch" ]
                               then
                                       hg clone --insecure https://Zoho-oauthtoken:${ACCESS_VALUE}@${REP_PATH}  -r ${hg_clone_arg}  ${SOURCE_BASE_DIR}
                               else
                                       hg clone --insecure https://Zoho-oauthtoken:${ACCESS_VALUE}@${REP_PATH}  -r ${hg_clone_arg}
                               fi
	...
	if [ "$3" == "MAIN_SRC"  ]
        then
		if [ "${PRODUCT_NAME}" != "HG_UTILS" -a "${PRODUCT_NAME}" != "WEBMONITOR" -a "${PRODUCT_NAME}" != "WEBFONTS" ]
		then
	                mv ${WORK_DIR}/${MODULE_NAME} ${SOURCE_BASE_DIR}
		fi
        fi
...
empty_srcdir_check()
{
               ...
               clone_arg_name="$1"
                emp_check_mod="$2"
                if [ $clone_arg_name == "integ_build_branch" ]
                then
...
zrepo_wget()
{
	...
	clone_arg="$1"
        ...
        if [ "${clone_arg}" == "integ_build_branch" ]
        then
	...
	then
        if [ "${PRODUCT_NAME}" != "PAYROLL_SERVER" -a "${PRODUCT_NAME}" != "ZOHOBOOKS" ]
        then
		...
		git_lfs_clone=`cat ${WORK_DIR}/hg_utils/build/lfs_clone_product.txt | grep -w "${PRODUCT_NAME}"`
		if [ -z "${zrepo_git}" -o -n "${git_lfs_clone}" ]
                then
	...
	if [ ! -d "${checkout_mod_name}" -a ! -f "${WORK_DIR}/${checkout_mod_name}_gitmod.txt" ]
	then
              CHANGE_API_PRD=`cat ${WORK_DIR}/hg_utils/build/CHANGEFILES_API_USED_PRODUCTS.txt | grep -w "${PRODUCT_NAME}"`
	      if [ -n "${CHANGE_API_PRD}" -a "${clone_arg}" != "integ_build_branch" ]
              then
...
getRepoSrc()
{
	...
	if [ "$1" = "clone" -a "${REPOSITORY_TYPE}" = "GIT" ]
        then
		if [ "$2" = "integ_build_branch" ]
		then
			cd ${CONF_DIR}
        ...
        elif [ "$1" = "clone" -a "${REPOSITORY_TYPE}" = "HG" ]
        then
		if [ $2 = "integ_build_branch" ]
                then
                        cd ${CONF_DIR}
	...
	if [ ! -d "${WORK_DIR}/Repo_Src" ]
        ...
        then
		if [ ! -d "${WORK_DIR}/Repo_Src/${MODULE_NAME}" ]
		then
        ...
        then
		if [ ! -d "${WORK_DIR}/Repo_Src/${MODULE_NAME}.git" ]
		then
...
getReleaseNotes()
{
        ...
        if [ -n "${BRANCH_LATEST_MILESTONE_CHANGESET}" -a "${BRANCH_LATEST_MILESTONE_CHANGESET}" != "None" ]
        then
                PREV_CSET_VAL=${BRANCH_LATEST_MILESTONE_CHANGESET}
        else
        ...
        then
                ...
                if [ "${PRODUCT_NAME}" == "SAS_PROVISIONING" ]
                then
                COMMIT_MESSAGE=`cat ${BUILD_DIR}/milestone.tag | cut -d "=" -f2`
                elif [ "${PRODUCT_ID}" == "912" -o "${PRODUCT_ID}" == "516" -o "${PRODUCT_ID}" == "2351" -o "${PRODUCT_ID}" == "2359" -o "${PRODUCT_ID}" == "1919" -o "${PRODUCT_ID}" == "1891" -o "${PRODUCT_ID}" == "1998" -o "${PRODUCT_ID}" == "1712" -o "${PRODUCT_ID}" == "2789" -o "${PRODUCT_ID}" == "2795" ]
                then
        ...
        then
                echo "Inside getReleaseNotes Method for Product Id ====> ${PRODUCT_ID}"
                if [ "${PRODUCT_ID}" == "728" -o "${PRODUCT_ID}" == "3088" -o "${PRODUCT_ID}" == "1237" -o "${PRODUCT_ID}" == "6303" ]
                then
...
getAuthors()
{
	if [ "${PRODUCT_NAME}" == "ZOHOPROJECTS" -a "${ARG}" == "default" ]
	then
		IDC_CHANGESET_VAL=`python apirequest ${PRODUCT_NAME} IDC_Changeset`
		if [ "${IDC_CHANGESET_VAL}" != "" ]
		then
			if [ "${REPOSITORY_TYPE}" = "HG" ]
			then
				cd ${REPO_DIR}/${MOD_NAME}
				if [ "${IDC_CHANGESET_VAL}" = "${CHANGESET_VALUE}" ]
				then
...
fetchBuildDetails()
{
	...
	then
        ...
        #CEPH_COPY_NEEDED="no"
	if [ "${GROUP_ID}" -eq "16" -o "${GROUP_ID}" -eq "18" -o "${GROUP_ID}" -eq "19" ]
	then
		export IMAGE_NAME="integ-cm.csez.zohocorpin.com/ubuntu22/android"
		sudo docker pull ${IMAGE_NAME}

	elif [ "${GROUP_ID}" -eq "1" -o "${GROUP_ID}" -eq "10" ]
	then
		export IMAGE_NAME="integ-cm.csez.zohocorpin.com/me/centos7"
	elif [ "${GROUP_ID}" -eq "15" -o "${GROUP_ID}" -eq "20" ]
	then
...
get_conf_file()
{
    ...
    if [ -n "${TEAMSPECIFIC_INTEG_CHECKOUT_FILE}" ]
        then
...
EOF
			...
			if [ ${BRANCH_LATEST_MILESTONE_URL} != "" ]
			then
				cd ${CONF_DIR}
...
hg_subsource_wget()
{
	...
	if [ ! -f "${SUBMODULE_MODNAME}-${CLONE_ARG}.zip" ]
        then
...
git_subsource_wget()
{
	...
	cd "${WORK_DIR}"
	zrepo_wget  "${CLONE_ARG}" "${SUBMODULE_MODNAME}" "${sub_zrepo_id}"
	if [ ! -f "${WORK_DIR}/${SUBMODULE_MODNAME}-${CLONE_ARG}.zip" -o -f "${WORK_DIR}/${checkout_mod_name}_gitmod.txt" ]
	then
...
hg_source_wget_common()
{
    ...
    then
	    echo "Repo avilable under zrepositry so clone method calling added."
	        ...
	        if [  !  -f  ${MODULE_NAME}-${CLONE_ARG}.zip -a "${PRODUCT_NAME}" != "HG_UTILS" ]
                then
                       getRepoSrc "clone" "integ_build_branch"
          	fi
		...
		if [ "${PRODUCT_NAME}" == "HG_UTILS" -o "${PRODUCT_NAME}" == "WEBMONITOR" -o "${PRODUCT_NAME}" == "WEBFONTS" ]
                then
                        getRepoSrc "clone"
                else
		...
		if [  !  -f  ${MODULE_NAME}-${CLONE_ARG}.zip -a "${PRODUCT_NAME}" != "HG_UTILS" -a "${PRODUCT_NAME}" != "WEBMONITOR" -a "${PRODUCT_NAME}" != "WEBFONTS" ]
                then
                       getRepoSrc "clone"
                fi
    ...
    elif [ "${PRODUCT_NAME}" == "SASLITE" -o "${PRODUCT_NAME}" == "CACHE_CP" ]
    then
	if [ ${CLONE_ARG} = "integ_build_branch" ]
        then
          getRepoSrc "clone" "integ_build_branch"
    ...
    SRC_ZIP_STATUS=`echo $?`
    if [ ${SRC_ZIP_STATUS} -eq 0 ]
    then
    echo "Source zip downloaded successfully ."
    unzip -q $CLONE_ARG.zip -d ${REPO_MODULE_NAME}_SRC
    if [ ! -d ${REPO_MODULE_NAME}_SRC/${REPO_MODULE_NAME} ]
    then
     ...
     then
	     if [ ${CLONE_ARG} = "integ_build_branch" ]
                then
	              ...
	              if [  !  -f  ${MODULE_NAME}-${CLONE_ARG}.zip  ]
        	      then
                          getRepoSrc "clone" "integ_build_branch"
...
updateExactSrc()
{
	...
	then
		hg_grp_nam=`echo $CLONE_VALUE | cut -d "/" -f4`
	        if [ "$hg_grp_nam" = "zoho" -o "$hg_grp_nam" = "iphone" ]
        	then
                	CLONE_VALUE_CLN=`echo ${CLONE_VALUE} | sed -e "s@/$hg_grp_nam/@/zohoclone/@g"`
	        elif [ "$hg_grp_nam" = "android" -o "$hg_grp_nam" = "fw" -o "hg_grp_nam" = "integ" ]
        	then
                	CLONE_VALUE_CLN=`echo ${CLONE_VALUE} | sed -e "s@/$hg_grp_nam/@/meclone/@g"`
		elif [ ${PRODUCT_NAME} != "APPMANAGER" -a "$hg_grp_nam" = "me" ]
		then
			CLONE_VALUE_CLN=`echo ${CLONE_VALUE} | sed -e "s@/$hg_grp_nam/@/meclone/@g"`
	        else
...
get_tplabel()
{
	...
	if [ ! -z ${lyte_imagename} ]
        then
		if [ ! -z ${lyte_loc_needed} -a ! -z ${lyte_npminstall_needed} ]
		then
...
clean()
{
	...
	if [ "${SOURCE_BASE_DIR}" != "${HOME}" -a "${SOURCE_BASE_DIR}" != "${HOME}/" -a "${SOURCE_BASE_DIR}" != " " -a "${SOURCE_BASE_DIR}" != "${HOME}/dload" ]
	then
...
{
        if [ "${BUILD}" != "WEBHOST" -a "${BUILD}" != "CI" -a -z "${DAILY_BUILD_STATUS}" ]
        then
                if [ -n "${PREV_DBUILD_LBL}" ]
                then
                        if [ ${PREV_DBUILD_LBL} != ${TIP_CHANGESET} ]
                        then
                                        DIFF_CONTENT="True"
                        fi

                        echo "DAILY BUILD STATUS : ${DAILY_BUILD_STATUS}"
                        if [ -z "${DIFF_CONTENT}" -a -z "${DAILY_BUILD_STATUS}" ]
                        then
                                echo "NOTIFICATION STATUS : ${NOTIFICATION_STATUS}"
                                if [ -z "${NOTIFICATION_STATUS}" ]
                                then
...
resolveWebHost()
{
	...
	if [ "${CUSTOMER_NAME}" != "DEFAULT" -a "${CUSTOMER_NAME}" != "" ]
	then
...
resolveVariables()
{
	...
	then
		...
		if [ "${customer_sts_flag}" = "${LABEL_VAR}" ]
		then
...
resolveBuildDependency()
{
	...
	export reports_value sec_reports_value
	if [ "${TEAMSPECIFIC_DEPENDENCY_TRIGGER}" != "YES" ]
	then
		if [ -z ${CUSTOM_DEPENDENCY_BUILD_TRIGGER} ]
                then
			if [ "${SPI_BUILDS}" != "NA" ]
			then
				...
				for dep_bld in `echo ${DEPENDENCY_BUILDS} | tr "," " "`
					do
	...
	if [ "${CUSTOM_DEPENDENCY_BUILD_TRIGGER}" = "YES" -a "${BUILD_TASKS}" != "JAVA_JSP" ]
        then
...
getStartedDepBuildID()
{
       ...
       if [ "${SEQUENTIAL_BUILD}" == "YES" ]
        then
                for seqlist in `echo ${SEQ_BUILD_ORDER} | tr "," " "`
                do
                ...
                #      if [ ${MULTIBLD_EXISTS} -eq 0 ]
                #      then
                #             for depbldId in `echo $STARTED_BUILDID | tr "," " " | sed -e "s@\"@@g"`
                #             do
                #                  checkDependentBuildStatus ${depbldId} ${dpprd}
                #             done
                #      else
        ...
        if [ "${DEP_BUILD}" == "STARTED" ]
        then
            if [ -z ${SEQUENTIAL_BUILD} ]
            then
...
getParallelTriggeredBuildStatus()
{
        if [ -f "${WORK_DIR}/paralleltriggered_builds.txt" ]
        then
           ...
           do
                ...
                # fi
                # MULTIBLD_EXISTS=`echo $?`
                # echo "MULTI_BLDID ${MULTI_BLDID} ---- ${MULTIBLD_EXISTS}"
                # if [[ "${MULTI_BLDID}" =~ [0-9]+ ]]; then
...
breakPrimaryBuild()
{
        if [ -f ${LOGS_DIR}/dependency_buildstatus.txt ]
        then
               failedprds=""
...
fetchDependencyUrlFromFile(){
	...
	DEP_PRD_ID="${2}"
        ...
        if [ -f "${dep_file_path}" -a -n "${DEP_PRD_NAME}" -a -n "${DEP_PRD_ID}" -a "${DEP_PRD_ID}" != "" ] ;then
...
getDependentBuildStatus_OLD()
{
	...
	lyte_webapps=`echo $modified_src_files | grep -Ew "(webapps/zohocrm/CRMClient|webapps/zohocrm/css/style|webapps/zohocrm/css/lessconfig|webapps/zohocrm/CRMOrganization)"`
		...
		echo "[$(date "+%Y-%m-%d %T")] :: INFO :: ZOHOCRM_LYTE Curl Command Ended..."
                ...
                if [ -z "${SEC_MULTI_BLDID}" ]
	        then
		    SEC_STARTED_BUILDID=`curl -k -X GET -H "Content-Type: application/json" -H "PRIVATE-TOKEN: ${PRIVATE_TOKEN}" "https://cmtools-api.csez.zohocorpin.com/api/v1/buildlogs?comment=${primary_build_id}&product_id=${sec_prod_id}"`
                    SEC_MULTI_BLDID=`echo $SEC_STARTED_BUILDID | tr "," " " | sed -e "s@\"@@g"`
	        fi
                SEC_MULTIBLD_EXISTS=`echo $?`
                echo "SEC_MULTI_BLDID ${SEC_MULTI_BLDID} ---- ${SEC_MULTIBLD_EXISTS}"

                if [[ "${SEC_MULTI_BLDID}" =~ [0-9]+ ]]; then
...
checkDependentBuildStatusOLD()
{
        ...
        if [ $stat != "Success" -o $stat != "Failure" ]
        then
               DEPBLD_STATUS=`curl -k -X GET -H "PRIVATE-TOKEN:${PRIVATE_TOKEN}" "${cmtools_domain_name}/api/v1/buildlogs?secondary_buildId=${depbldId}"`
               stat=`echo $DEPBLD_STATUS | sed -e "s@\"@@g"`
        fi
	...
	bld_cnt=0
        ...
        while [ $stat != "Success" -o $stat != "Failure" ]
        do
             holdBuildProcess $stat
             DEPBLD_STATUS=`curl -k -X GET -H "PRIVATE-TOKEN:${PRIVATE_TOKEN}" "${cmtools_domain_name}/api/v1/buildlogs?secondary_buildId=${depbldId}"`
             stat=`echo $DEPBLD_STATUS | sed -e "s@\"@@g"`
...
replaceUrls()
{
    ...
    for i_url in `echo ${changed_urls}`
    do
        if [ "${i_url}" != "" ]
        then 
...
appendDependencyUrls()
{
	...
	do
		...
		dep_val=`echo ${dep_bld} | cut -d "=" -f2`
		export ${dep_name}="${dep_val}"
		if [ "${dep_val}" != "" -a "${dep_val}" != "NA" -a "${dep_val}" != "DEFAULT" ]
		then
			...
			if [ -n "${dep_prod_url}" -a -z "${dep_prod_param}" ]
			then
			...
			if [ -n "${dep_prod_url}" -a -n "${dep_prod_param}" ]
			then
				for i in `echo ${dep_prod_param}`
				do
					...
					if [ -n "${dep_prod_val}" -a -z "${excluded_dep_param_search}" ]
					then
						dep_prod_basename=`basename ${dep_prod_val}`
						if [ "${dep_prod_basename}" == "ZohoMickeyLite.zip" ]
                                                then
                                                        dep_prod_basename="mickeylite/${dep_prod_basename}"
						elif [ "${dep_prod_basename}" == "ZohoMickeyLiteClient.zip" ]
						then
							dep_prod_basename="mickeylite_client/${dep_prod_basename}"
                                                fi
						if [ "${dep_name}" == "ZOHOWMS_API" ]
						then
        ...
        if [ "${PRODUCT_NAME}" = "ZOHOCRM" -a "${BUILD_TASKS}" != "JAVA_JSP" ]
        then
           if [ ! -z ${BRANCH_CHK} ]
           then
               for sasurl in `echo ${ALLOWED_SAS_BUILDS} | tr "," " "`
               do
...
EOF
			  ...
			  copyBuilderrorlogs
						  ...
						  common_webhookcall_api
                          exit 1
               fi

           for logsurl in `echo ${ALLOWED_LOGS_BUILDS} | tr "," " "`
           do
...
EOF
		 ...
		 export REASON_FOR_BUILD_FAILURE
           ...
           for storeurl in `echo ${ALLOWED_STORE_BUILDS} | tr "," " "`
           do
...
EOF
	   ...
	   for earurl in `echo ${ALLOWED_EAR_BUILDS} | tr "," " "`
           do
               EARURL_MATCH=`cat ${BUILD_DIR}/temp_dependency_details.txt | grep "${earurl}"`
               if [ ! -z ${EARURL_MATCH} ]
               then
                      EAR_URL=`echo $EARURL_MATCH | cut -d "=" -f2`
                      BUILD_OPTS="${BUILD_OPTS} -Dearagent_url=${EAR_URL}"
                      break;
                fi
           done
           if [ -z ${EARURL_MATCH} ]
           then
                  mail -r "integration-team@zohocorp.com" -s "EAR build url mismatch error for ${ARG}" "${ERROR_MAIL_ID}"<<-EOF

                           EAR url mentioned is not mathced with available builds.
...
EOF
                 BUILD_STATUS="FAILURE"
                 export BUILD_STATUS
                 EXIT_MESSAGE="EAR url mentioned is not matched with available builds... Hence exiting .."
                 echo "${EXIT_MESSAGE}"
                 echo "REASON_FOR_BUILD_FAILURE=\"Dependency_Build_Failure:${EXIT_MESSAGE}\"" >> ${build_info_file}
                 REASON_FOR_BUILD_FAILURE="Dependency_Build_Failure:${EXIT_MESSAGE}"
                 export REASON_FOR_BUILD_FAILURE
	...
	if [ "${STATIC_DIRNAME}" != "" -a "${STATIC_DIRNAME}" != "NA" ]
	then
		BUILD_OPTS="${BUILD_OPTS} -Dstatic_dir=${STATIC_DIRNAME}"
	fi
	if [ "${PREV_DBUILD_URL}" != "" -a "${PREV_DBUILD_URL}" != "NA" ]
	then
		BUILD_OPTS="${BUILD_OPTS} -Dprev_build=${PREV_DBUILD_URL}"
	fi
	if [ "${PATCH_BUILD}" == "TRUE" ]
	then
		BUILD_OPTS="${BUILD_OPTS} -Dtarget=target -Dpatchbuild_binary_name=${PATCH_BINARY}"
	fi
	...
	TEMP_PORT=`echo ${WH_BUILD_ID} | awk '{print substr($0, length($0)-2)}'`
	TEMP_PORT=`expr 8000 + ${TEMP_PORT}`
	BUILD_OPTS="${BUILD_OPTS} -Dbuild_id=${WH_BUILD_ID} -Dtemp_port=${TEMP_PORT} -Dproduct_name=${PRODUCT_NAME} -Dbranch_name=${ARG} -Dceph_copy_needed=${CEPH_COPY_NEEDED} -Dservername=${SERVER_NAME} -Dpre_build_url=${PREV_DBUILD_URL} -Dlast_milestone_url=${LATEST_MILESTONE_URL} -Dpdt_name_from_conf=${productname} "

	#winexebranch=`cat ${WORK_DIR}/hg_utils/build/winexetask.txt | grep -w ${ARG}`
	if [ "${PRODUCT_NAME}" != "LOG360" -a "${PRODUCT_NAME}" != "AD360" -a "${PRODUCT_NAME}" != "ZOHOSYNC" -a  -z "${winexetask_excluded_product}" -a "${winexetask_excluded_product}" != "yes" ]
	then
...
{
        concat=""
        for dep_bld_url in `echo $DEP_BUILDS | tr ',' ' '`
        do

                dep_build_url_all=`echo $dep_bld_url | cut -d "=" -f2 | grep -w "zip"`
                if [ ! -z $dep_build_url_all ]
                then
...
then
	...
	echo $replace_file
		...
		for i in ` echo $getval_earsaslist | tr "," " " `
                do
                        grep -w $i ${WORK_DIR}/ear_block.txt
                        if [ $? -eq 0 ]
                        then
                            for j in `echo $getval_earblock_list | tr "," " " `
                            do
					...
					export REASON_FOR_BUILD_FAILURE
					 ...
					 updateBuildDetails
                            ...
                            echo "warn list"
                            for k in `echo $getval_earwarnlist | tr "," " " `
                            do
                                echo "$k"
...
gettip_components_api()
{
	...
	tp_comp_url=`echo $tp_components_path | sed -e "s@http://build/@/zoho/build/downloads/dload/@g" -e "s@https://build.zohocorp.com/@/zoho/build/downloads/dload/@g"`
...
updateValidTPUrl()
{
	...
	if [ "${TPBRANCH_REVIEWED_CHECKOUT}" = "Yes" ]
        then
...
#	else
	       cmtp_branchname_trim=`echo ${cmtp_branchname} | sed -e 's/^[[:space:]]*//'`
               if [ "${TP_DOWNLOAD_URL}" != "" ]
               then
		    GO_BUILD_UTIL_BINARY=${WORK_DIR}/hg_utils/build/build_utils
		    TP_BUILD_RESPONSE_TXT_PATH=${WORK_DIR}/tp_url.txt
		    chmod 755 ${GO_BUILD_UTIL_BINARY}
		    ${GO_BUILD_UTIL_BINARY} tp_build_process ${WORK_DIR}/${PRODUCT_NAME}.txt ${cmtp_branchname_trim}
                    if [ -s ${TP_BUILD_RESPONSE_TXT_PATH} ]; then
			TP_NEW_BLD_URL=$(sed -n 1p ${TP_BUILD_RESPONSE_TXT_PATH})
			NEW_TP_CSET=$(sed -n 2p ${TP_BUILD_RESPONSE_TXT_PATH})
			if [ "${NEW_TP_CSET}" != "" ]; then
			    TP_CSET_LABEL=${NEW_TP_CSET}
			    TP_CHANGESET_VAL=${NEW_TP_CSET}
			    TP_CHANGESET_VAL_LATEST=${NEW_TP_CSET}
			    ...
			    TP_CHANGESET_VAL_LATEST="NA"
                        ...
                        if grep -q 'ErrorLogs\/' ${TP_BUILD_RESPONSE_TXT_PATH}; then
			    echo "TP build generation failed.."
			    sed -n 1p ${TP_BUILD_RESPONSE_TXT_PATH}
                            echo -e "Thirdparty ${cmtp_branchname_trim} build generation failed, Kindly check with integration-team." > ${WORK_DIR}/${PRODUCT_NAME}_DBUILD/build/build.log
                            errormail
                            updateBuildErrorStatus "The build has been stopped due to thirdparty branch(${cmtp_branchname_trim}) build is not available"
			    cmtpurlpath_new="${TP_NEW_BLD_URL}"
			else
			    echo "TP build check completed ..."
			    echo "New TP URL : ${TP_NEW_BLD_URL}"
			    cmtpurlpath_new=${TP_NEW_BLD_URL}
			fi
			BUILD_OPTS="${BUILD_OPTS} -Dcmtp_hgroot=$cmtpurlpath_new"
		    else
			echo "TP branch build not available !.."
			updateBuildErrorStatus "The build has been stopped due to thirdparty branch(${cmtp_branchname_trim}) build is not available"
			cmtpurlpath_new="NA"
		    fi
               else
		...
		#tmp_cmtp_replace=`echo $cmtp_url | sed "s@${http_domain_name}@/zoho/build/downloads/dload/@g"`
		...
		if [ "${tprepo_source_tip}" != "${TP_CHANGESET_VAL_LATEST}" -a "${cmtp_branchname}" != "BASE"  -a "${cmtp_branchname}" != "HG_UTILS_BUILD_BRANCH" ]
		then
		...
		cmtpurlpath_new=`echo $cmtpurlpath | sed -e "s@${http_domain_name}@${buildZohoCorpDomainName}@g"`
                fi
...
mickeyPrechecktxt()
{
	...
	catbuildopt=`cat ${BUILD_DIR}/temp_dependency_details_opts.txt`
        for bopt in `echo "$catbuildopt" | sed -e "s@-D@ @g" | sed -e "s@|@ @g"`
        do
    ...
    if [ -f  ${BUILD_DIR}/buildmatchlist.txt -a -s ${BUILD_DIR}/buildmatchlist.txt ]
    then
...
invalidDependencyUrlCheck()
{
    ...
    if [ "${PRODUCT_NAME}" == "ONEZOHO" -a "${invalid_check}" != "" ]
    then
...
{
	if [ "${BUILD}" == "CI" -a -z "${BUILD_DLOAD_DIR}" ]
	then
...
getModifiedFilesOnRetry()
{
	...
	RETRY_PARAMS="repo_link=${CLONE_VALUE}&revision1=${TIP_CHANGESET}&revision2=${RETRY_TIP_CSET}&files_only=true"
        ...
        if [ "${MODIFIED_FILE_LIST_ON_RETRY}" != "" ]
	then
...
resolvebuild_type()
{
	...
	for reviewedname in ${REVIEWED_LABEL_NAMES}
	do
		if [ "${ARG}" == "${reviewedname}" ]
		then
	...
	local count=1
	for reviewedbranchname in ${REVIEWED_BRANCH_LABEL_NAMES}
	do
		echo "${ARG} ${reviewedbranchname}"
		if [ "${ARG}" == "${reviewedbranchname}" ]
		then
			reqcount=${count}
	...
	local count=1
	for customerbranchname in ${CUSTOMERS_BRANCH_LABEL_NAMES}
	do
		echo "${ARG} ${customerbranchname}"
		if [ "${ARG}" == "${customerbranchname}" ]
		then
			reqcount=${count}
	...
	local count=1
	for configuredbranchname in ${CONFIGURED_BRANCH_NAMES}
	do
		if [ "${ARG}" == "${configuredbranchname}" ]
		then
			reqcount=${count}
	...
	local count=1
	for configuredreviewedbranchname in ${CONFIGURED_REVIEWED_BRANCH_LABEL_NAMES}
	do
		if [ "${ARG}" == "${configuredreviewedbranchname}" ]
		then
			reqcount=${count}
...
triggerCIBuildNotify()
{
	...
	mail -r "integration-team@zohocorp.com" -s "CI Build Status of ${PRODUCT_NAME} in ${ARG} (${TIP_CHANGESET})" "${SUCCESS_MAIL_ID}"<<-EOF
                Dear Team,

                The CI build for latest commit in ${ARG} (${TIP_CHANGESET}) branch is triggered and its status can be checked in the below mentioned URL:
...
source_checkout()
{
	...
	tipchangeset_Notification
        ...
        if [ -n "${CHECKIDC_CHANGESET}" -a "${CHECKIDC_CHANGESET}" = "YES" -a -n "${BUILD_TO_COMPARE}" -a "${BUILD_TO_COMPARE}" != "NA" ]; then
            source ${WORK_DIR}/hg_utils/build/idc_changeset_check.sh
            initiateIDCCheckProcess > ${LOGS_DIR}/idc_changeset_check.log 2>&1 &
        fi
...
then
	...
	if [ "${BUILD}" = "CI" -a ${PRODUCT_NAME} = "ZOHOCRM" ]
        then
                PATCH_BUILD="TRUE"
                #getModifiedSource
	...
	MAIL_PRD=`cat ${WORK_DIR}/hg_utils/build/reports_products.txt | grep -E "ZM|ZOHOMAIL" | grep -w "${PRODUCT_NAME}"`
        if [ -n "${MAIL_PRD}" ]; then
                sh ${WORK_DIR}/hg_utils/build/source_checkout.sh ${WORK_DIR}/${PRODUCT_NAME}.txt ${PRIVATE_TOKEN}
                wait
		if [ -f "${WORK_DIR}/sub_product_names.txt" ]; then
                        . ${WORK_DIR}/sub_product_names.txt
		fi
	...
	copyNmsBuildFile
	if [ -z "PATCH_BUILD" -o "${PATCH_BUILD}" != "TRUE" ]
	then
		get_tplabel
	fi
	
	echo "appendDependencyUrls started at : `date +%b_%d_%Y@%T`"
        appendDependencyUrls >> ${LOGS_DIR}/appendDependencyUrls.log 2>&1
        echo "appendDependencyUrls ended at : `date +%b_%d_%Y@%T`"

	if [ "${PRODUCT_NAME}" != "ITSF" ]
	then
		echo "mickeyPrechecktxt started at : `date +%b_%d_%Y@%T`"
		mickeyPrechecktxt >> ${LOGS_DIR}/mickeyPrechecktxt.log 2>&1
		echo "mickeyPrechecktxt ended at : `date +%b_%d_%Y@%T`"
	fi
	invalidDependencyUrlCheck
	...
	if [ "${GROUP_ID}" = "5" -o -n "${EVAL_REPORT_PRODUCT_SEARCH}" ]
	then
		evalRestriction
		if [ -f "${EVAL_REPORT_FILE}" ]
		then
	...
	then
        ...
        if [ "${PRODUCT_NAME}" = "ZOHOMAIL" -a "${BUILD_TASKS}" = "JAVA_JSP" ]; then
            ${WORK_DIR}/hg_utils/build/patch_build.sh "JAVA_JSP"
        fi
fi
...
common_mergecheck()
{
       ...
       if [ "${COMMON_CHANGESET_MERGE_CHECK}" = "YES" ]
       then
...
pkg_lyte_notify()
{
	...
	do
		...
		sent_op=`python ${WORK_DIR}/${PRODUCT_NAME}_DBUILD/lyte.py ${pkg_file}`
	        ...
	        if [ "$sent_op" != "package not available" ]
        	then
...
gitSubModuleCheckout()
{
    ...
    then
        if [ -f "${SOURCE_BASE_DIR}/.gitmodules" ]
        then
            cd ${SOURCE_BASE_DIR}
            git submodule init
...
tomcat7JspTimeChange()
{
	if [ -n "${TOMCAT7_RUNTIME_JSP_TIME_CHANGE}" -a "${TOMCAT7_RUNTIME_JSP_TIME_CHANGE}" = "YES" ]
	then
...
subModulesCheckout_ui()
{
    ...
    then    
        ...
        do
	     ...
	     if [ ! -z "$SUBPROD_LBL" -a ! -z "$SUB_PROD_TIP" ]
             then
...
subModulesCheckout_wget()
{
	...
	SUBMODULES_LIST=""
        ...
        if [ -n "${SUBMODULES_LIST}" ]
        then
                echo "Sub Module List values are ${SUBMODULES_LIST}"
                for i in `echo ${SUBMODULES_LIST}`
                do
                        ...
                        if [ -n "${SUBMODULE_NAME_SEARCH}" -a "${SUBMODULE_LABEL}" != "NA" ]
                        then
 
            echo "submodule checkout call"
                                if [ "${SUBMODULE_REP_TYPE}" = "HG" ]
                                then
                                        echo ${SUBMODULE_CLONE_URL}
                                        echo "*** sub clone val available"
                                        if [ ${SUBMODULE_CLONE_URL} != "" ]
                                        then
        ...
        then
                ...
                do
                        ...
                        time_cal "Cloning ${SUBMODULE_NAME} Sources" "${SM_CLONE_ST_TIME}" "${SM_CLONE_END_TIME}"
                                        ...
                                        SM_CLONE_ST_TIME=`date +%b_%d_%Y@%T`
					if [ ${ZREPO_GITCLONE_CONF} == "YES" ]
                                        then
                                               SUBMODULE_CLONE_URL=`echo ${SUBMODULE_ARGS} | grep ":" | cut -d ":" -f2`
                                               if [ ! -z "${ACCESS_VALUE}" -a ! -z ${SUBMODULE_CLONE_URL} -a ${ZREPO_GITCLONE_CONF} == "YES" ]
                                               then
...
copyCustomerSources()
{
	...
	then
		...
		echo "Selected CUSTOMER_NAME is ${CUSTOMER_NAME}"
			...
			customer_dirname=`echo ${CUSTOMER_NAME} | tr [A-Z] [a-z]`
			if [ -d "${cust_resource_path}/${customer_dirname}" ]
			then
...
modified_src_copy()
{
   ...
   for i in `echo $modified_src_files | tr "," " "`
    do
...
submodule_patch_diff()
{
	...
	if [ -n "$SUBPREV_TXT_FILE" -a -n "$SUBCURRENT_TXT_FILE" ]
	then
	...
	for suburl in `echo $SUBCURRENT_TXT_FILE | tr "," "\n" | sed "s@\"@@g"`
        ...
        do
                ...
                if [ "${SUB_CURRENT_CHANGESET}" == "${SUB_PREV_CHANGESET}" ]
                then
                        echo "same changesets ignore copy files .."

		elif [ -n "${SUB_CURRENT_CHANGESET}" -a -n "${SUB_PREV_CHANGESET}" ]
		then
...
updModifiedSource_api()
{
    ...
    changed_files=`echo ${modified_src_files} | tr " " "\n" | grep -Ev "(.java|.jsp|.properties|.conf|.txt|.html|.sh|.xml|.scala|.py)" | wc -l`
echo ${changed_files}
    if [ "${modified_src_files}" != "" -a "${changed_files}" -eq 0 ]
    then
	...
	if [ -d "${SOURCE_BASE_DIR}/source" -o -d "${SOURCE_BASE_DIR}/src" ]
        then
	...
	then
        ...
        then
	       ...
	       then
                ...
                elif [ "${jsp_file}" -gt 0  -a "${REPOSITORY_TYPE}" = "GIT" ]
                then
		...
		then
			...
			do
				...
				do
					...
					jarname=`basename $i`
					if [ "${jarname}" == "jstl-1.2.jar" -a ! -f "${WEBINF_dir}/lib/${jarname}" ]
					then
						jarname="jstl.jar"
					fi
					if [ "${value}" != "" ]
...
gitclone_src_sub()
{
	...
	echo $CLONE_VALUE | grep "repository.zoho.com"
		...
		CHANGE_API_PRD=`cat ${WORK_DIR}/hg_utils/build/CHANGEFILES_API_USED_PRODUCTS.txt | grep -w "${PRODUCT_NAME}"`
		if [ -n "${CHANGE_API_PRD}" -a ! -f ${WORK_DIR}/${MODULE_NAME}-${CLONE_ARG}.zip ]
                then
		...
		if [ ! -f  ${MODULE_NAME}-${CLONE_ARG}.zip -o -f "${WORK_DIR}/${checkout_mod_name}_gitmod.txt" ]
                then
			echo "GitModules Configured. Hence, clone process initiated."
                        getRepoSrc "clone"
			#src_corrupt_SendMail
        ...
        if [ -n "`echo ${CLONE_VALUE} | grep "https://zbgit.csez.zohocorpin.com"`" ]
	then
		DOMAIN_PREFIX="zbgit"
	elif [ "`echo ${CLONE_VALUE} | grep "https://zgit.csez.zohocorpin.com"`" ]
	then
		DOMAIN_PREFIX="zgit"
	else
		DOMAIN_PREFIX="git"
	fi
	if [ -z ${TIP_CHANGESET} -a -z ${TIP_CHANGESET_RETRY} ]
	then
...
removeEmptyFiles()
{
	if [ "${REMOVE_EMPTY_FILES_NEEDED}" = "YES" ]
	then
		...
		if [ -z "${EMPTYFILE_REMOVAL_EXCLUDE}" ]
		then
	...
	then
		...
		for i in `echo $GET_EMPTY_FOLDERS`
		do
			rmdir -p $i
			if [ $? -ne 0 ]
			then
...
copyNmsBuildFile()
{
	if [ "${IsNmsBuildFileCopyNeeded}" == "TRUE" ]
	then
...
getTP_TipVersion()
{
	...
	if [ -d "${BUILD_BASE_DIR}/components" ]
	then
...
do
        ...
        then
                ...
                if [ $filetype = "zip" -o $filetype = "ppm" ]
                then
...
updateBuildDetails()
{
	...
	OP_DIR=""
	common_webhookcall
	if [ ! -z "${TIP_CHANGESET_CLN}" ]
        then
          CHANGESET_VALUE=${TIP_CHANGESET_CLN}

	elif [ ! -z "${TIP_CHANGESET}" ]
	then
	...
	then
		if [ -s "${WORK_DIR}/lyte_det.txt" ]
                then
                    
                    for lyte_pkg in `cat ${WORK_DIR}/lyte_det.txt`
                    do
		...
		if [ "${REPOSITORY_TYPE}" == "GIT" -o "${REPOSITORY_TYPE}" == "HG" ]
		then
	...
	echo "OP_DIR=\"${OP_DIR}\"" >> ${build_info_file}
       	...
       	for opts_st in `echo $BUILD_OPTS`
       	do
	...
	if [ -f ${BUILD_DIR}/winpack_details_all.txt ]
        then
	...
	update_dependency_server_details
        if [ "${GROUP_ID}" = "16" -o "${GROUP_ID}" = "18" -o "${GROUP_ID}" = "27" ]
	then  
		maxdepth_length="5"
                find_extension="*.*" 
        elif [ "${GROUP_ID}" = "15" -o "${GROUP_ID}" = "20" -o "${GROUP_ID}" = "21" ]
        then
        	maxdepth_length="2"
		find_extension="*.zip"
        elif [ "${GROUP_ID}" = "1" ]
	...
	#then
		...
		then
			...
			if [ -f "${WORK_DIR}/sub_prod_conf.txt" ]
                        then
	...
	#fi
	if [ "${REPOSITORY_TYPE}" = "CVS" -o -n "${TP_EXCLUDED_PRODUCTS_SEARCH}" ]
	then
	...
	if [ "${GROUP_NAME}" == "INTEG" -o "${GROUP_NAME}" == "ZOHO" -o "${GROUP_NAME}" == "ZOHO_ANDROID" -o "${GROUP_NAME}" == "ZOHO_ELECTRON" -o "${GROUP_NAME}" == "ZOHO_IPHONE" -o "${GROUP_NAME}" == "ZOHO_WINDOWS" -o "${GROUP_NAME}" == "ZORRO" -o "${GROUP_NAME}" == "GOFRUGAL" -o "${GROUP_NAME}" == "FUNPROJECTS" -o "${GROUP_NAME}" == "ITS" -o "${GROUP_NAME}" == "OPENSOURCE" ]
	then
		buildlog_api="http://integrepo-secondary:8081"
	elif [ "${GROUP_NAME}" == "ME" -o "${GROUP_NAME}" == "FRAMEWORK" -o "${GROUP_NAME}" == "ME_ANDROID" -o "${GROUP_NAME}" == "ME_IPHONE" -o "${GROUP_NAME}" == "THIRDPARTY" -o "${GROUP_NAME}" == "VTITAN" -o "${GROUP_NAME}" == "WEBNMS" -o "${GROUP_NAME}" == "WEBNMS_ANDROID" -o "${GROUP_NAME}" == "WEBSITE" -o "${GROUP_NAME}" == "PROJECTS" -o "${GROUP_NAME}" == "SD" -o "${GROUP_NAME}" == "SWISSSQL" -o "${GROUP_NAME}" == "TESTAUTOMATION" -o "${GROUP_NAME}" == "TP_PKG" ]
	then
...
update_dependency_server_details()
{
    ...
    if [ -f "${BUILD_DIR}/dependency_server_name.txt" ]
    then
	    deps_servers=""
            sort_server=`sort -u ${BUILD_DIR}/dependency_server_name.txt`
            for dep_server in $sort_server
            do
...
{
        process_type=$1
        if [ -f "${BUILD_OUTPUT}/ROOT.war" ]
        then
              rm -f ${BUILD_OUTPUT}/ROOT.war
        fi
        if [ -f "${BUILD_OUTPUT}/Charm_ROOT.war" ]
        then
              rm -f ${BUILD_OUTPUT}/Charm_ROOT.war
        fi
        if [ -f "${BUILD_OUTPUT}/ehr_ROOT.war" ]
        then
              rm -f ${BUILD_OUTPUT}/ehr_ROOT.war
        fi

        if [ "${PRODUCT_NAME}" != "SASLITE" -a "${PRODUCT_NAME}" != "GAC" -a "${PRODUCT_NAME}" != "DC_CLOUD" ]
        then
        if [ -f "${BUILD_OUTPUT}/grid.war" ]
        then
	...
	then
		TEMP_DLOAD_DIR=`echo ${DLOAD_DIR} | sed -e "s@^/zoho/@/data1/@g"`
			...
			mkdir -p ${BUILD_DIR}/TEMP_BKP
                        ...
                        cp -r ${BUILD_DIR}/{buildlogs,build.log,post_build.log,combine.js,lyte_docker_creation.log} ${BUILD_DIR}/TEMP_BKP/logs/
...
if [ ! -f "${HOME}/centos5.txt" ]
then
        touch ${HOME}/centos5.txt
elif [ ! -f "${HOME}/centos6.txt" ]
then
        touch ${HOME}/centos6.txt
fi


if [ -f "${WORK_DIR}/${PRODUCT_NAME}_DBUILD/build/OS_details_5.txt" ]
then
        echo "PRODUCT_NAME=${PRODUCT_NAME}--->Branch_NAME=${ARG}" >> ${os5_file_txt}
elif [ -f "${WORK_DIR}/${PRODUCT_NAME}_DBUILD/build/OS_details_6.txt" ]
then
...
xml_to_html_creation()
{
	if [ "${BUILD_STATUS}" != "FAILURE" ]
		...
		if [ ! -f ${BUILD_DIR}/buildlogs/buildtime.html ]
	        then
        	        sleep 1m
	        fi
...
get_android_dep_urls()
{
	...
	if [ -f ${android_bld_log_file_path} ]
	then
		...
		then
                        count_aurl=0
                        for aurl in `cat ${BUILD_DIR}/android_urls.html`
                        do
...
build_output_Sync()
{
	...
	then
			...
			for ceph_server_mac in `echo $copy_ceph_servers| tr "," " "`
			do
...
runtime_jarname()
{
        ...
        val="${BUILD_DIR}/buildtime_created_jars.txt"
        if [ -f "${BUILD_DIR}/buildtime_created_jars.txt" ]
        then
        echo "runtime_jarname started at: " $(date "+%Y-%m-%d %T")
        cd ${BUILD_DIR}
        for i in `cat $val`
        do
                ni=`echo $i | cut -d ":" -f1`
                unzip_val=`unzip -l $ni | egrep -iw 'com/zoho/|com/adventnet/'`
                op_val=`echo $?`
                if [ "$op_val" == 0 ]
                then
                        echo $ni >> "${BUILD_DIR}/create.txt"
                fi
                runtime_txt="${BUILD_DIR}/create.txt"
        done
                awk -F/ '{print $NF}' $runtime_txt | sort -u >> "${LOGS_DIR}/runtime_jarname.txt"
        cd -
        echo "runtime_jarname ended at: " $(date "+%Y-%m-%d %T")
        fi
...
postbuild()
{
	...
	##sed -i 's@ACCESS_VALUE=\(.*\)@ACCESS_VALUE=\"\"@g' ${build_info_file}
        if [ "${GROUP_ID}" -ne "16" -a "${GROUP_ID}" -ne "18" -a "${GROUP_ID}" -ne "19" -a "${GROUP_ID}" -ne "15" -a "${GROUP_ID}" -ne "20" -a "${PRODUCT_NAME}" != "ZOHOSEARCH_MISC" -a "${PRODUCT_NAME}" != "HG_UTILS" -a "${GROUP_ID}" -ne "31" ]
        then
             grep --no-messages -Eo "integ-cm.csez.zohocorpin.com[^ \s]*|zregistry.csez.zohocorpin.com[^ \s]*" ${WORK_DIR}/${PRODUCT_NAME}_DBUILD/*.sh ${WORK_DIR}/${PRODUCT_NAME}_DBUILD/*.properties ${WORK_DIR}/${PRODUCT_NAME}_DBUILD/*dockerfile | cut -d ":" -f2- | sed -e 's@\"@@g' | sort | uniq >> ${WORK_DIR}/logs/dind_images.txt
        fi
	if [ "${IS_ANDROID_BUILD}" == "YES" ]
	then
	...
	updateBuildDetails
	if [ -d "${WORK_DIR}/${PRODUCT_NAME}_DBUILD/build/unzipsrc/" ]
        then
		windowpkgDetails
	fi
	###updateOS
	...
	then
		...
		OUTPUT_DLOAD_DIR=${ERROR_DLOAD_DIR}
                if [ "${HOSTNAME}" != "integ-hm3" ]
                then
                    hm3build_Sync
                fi
                #integ_storage_Sync "post" &
                if [ ! -d "/zoho/build/downloads/dload/" ]
                then
                        build_output_Sync "post"
                fi
		if [ "${WEBHOOK_URL_NEEDED_FOR_FAILURE}" == "YES" ]
		then
			CustomWebURLCall
		fi
		...
		then
			successmail
                ...
                #   integ_storage_Sync "post" &
                #fi
		if [ -f ${BUILD_DIR}/ant.properties -a -f ${BUILD_DIR}/alltaskname.txt ]
		then
		...
		zohoUpgrade
		CustomWebURLCall
		if [ "${AUTO_DAILYBUILD_UPGRADE}" == "YES" ]
		then
			if [ "${BUILD}" == "CRON" -o "${PRODUCT_NAME}" == "ZOHOSHOW" -o "${PRODUCT_NAME}" == "SHOWSERVER" ]
			then
	...
	echo "xmltohtml_py script ended at : " `date "+%Y-%m-%d %T"`
        if [ -d "${WORK_DIR}/${PRODUCT_NAME}_DBUILD/build/dependencies/integ_file_transfer/logs/" ]
        then
	...
	#    integ_storage_Sync "post" &
        #fi
        if [ "${BUILD_BASE_DIR}" != "/zoho/build/downloads" -a "${BUILD_BASE_DIR}" != "/zoho/build/WH_DIR/" -a "${BUILD_BASE_DIR}" != "${PRODUCT_NAME}_DBUILD" -a "${BUILD_BASE_DIR}" != " " ]
        then
          
            if [ "${BUILD_STATUS}" == "SUCCESS" -a ${copy_op_status} -eq 0 ]
            then
	...
	if [ "${label_flag}" != "FAILED" -a "${BUILD_STATUS}" != "FAILURE" -a "${PRODUCT_NAME}" != "COMPONENTS" ]
	then
	...
	then
		moveBuild
             ...
             if [ "${ARG}" != "SASLITE_M21_277_CITEST_BRANCH" ]
             then
		if [ "${BUILD}" = "CI" -a "${BUILD_MOVE_BRANCH}" != "" -a "${PRODUCT_NAME}" == "SASLITE" ]
		then
...
common_webhookcall_api()
{
	if [ -f "${WORK_DIR}/webhook.txt" ]
	then
		echo "common_webhookcall_api started at : " `date "+%Y-%m-%d %T"`
		json_val="@${WORK_DIR}/webhook.json"
		curl -k -X POST -H "PRIVATE-TOKEN:${PRIVATE_TOKEN}" -d "${json_val}" https://cmtools-api.csez.zohocorpin.com/api/v1/trigger_webhook_request?build_id=${WH_BUILD_ID}
		echo "common_webhookcall_api ended at : " `date "+%Y-%m-%d %T"`
	fi
}


checkIDCChangesMergedOLD()
{
...
getCheckSumForUpdateManager()
{
	...
	do
        	if [ -f ${update_manager_home}/$zpath ]
        	then
...
reportGeneration()
{
	...
	then
		...
		if [ -n "${MIL_DETAILS}" -a "${MIL_DETAILS}" != "None" ]
		then
		...
		if [ -n "${MISSING_KEYS_REPORT_RELEASEBUILD}" ]
                then
		...
		if [ -n "${BUILD_DEPENDENCY_CHANGE_NOTIFY}" ]
		then
		...
		then
			...
			realsebuildchk_reports=`echo $RELEASED_BRANCHES | grep -w "$ARG"`
                        if [ "${PRODUCT_NAME}" = "ZOHOCRM" -o "${PRODUCT_NAME}" = "APPMANAGER" -o "${PRODUCT_NAME}" = "CHARM_EHR" -o "${ARG}" = "default" -o "${ARG}" = "master" -o -n "${realsebuildchk_reports}" ]
			##if [ "${ARG}" = "default" -o "${ARG}" = "master" -o -n "${realsebuildchk_reports}" ]
                        then
...
errormail()
{
	...
	then
        ...
        getErrorTasks
	getExactError
	if [ -f "${ERROR_DLOAD_DIR}/logs/buildlogs/ExactError.txt" ]
        then
		...
		if [ -s "${ERROR_DLOAD_DIR}/logs/buildlogs/ExactError_space.txt" ]
		then
	...
	CMTOOL_OPTIONS=`echo $TEAM_SPECIFIC_INFO | tr "," "\n" | sed 's/=/:/' | pr -Td`
        ...
        if [ -f "${ERROR_DLOAD_DIR}/logs/ExactError.html" ]
        then
            err_file_name="ExactError.html"
            if [ "${GROUP_ID}" == "15" -o "${GROUP_ID}" == "20" -o "${GROUP_ID}" == "21" ]
            then
...
--data '{
  ...
  "tar_names": [
   ...
   "regions_to_be_synced": [
       {
          "region": '${SD_REGION_TO_BE_SYNCED}',
          "sync_time": '${SD_SYNC_TIME}'
       }
    ],
  "notify_to": [
    "'${SD_NOTIFY_TO}'"
...
zohoUpgrade()
{
        ...
        if [ "${GO_FRUGAL_SD_API_CALL}" == "YES" -o "${ZOHOBUILD_SD_API_CALL}" == "YES" ]
        then
        ...
        if [ "${SD_NEW_NIC_API_CALL}" == "YES" ]
        then
               echo "NIC sd api call started."
               sd_new_nic_prod_zoho_apicall >> ${LOGS_DIR}/sd_trigger.log 2>&1
               echo "NIC sd api call ended."
	fi
	if [ "${AUTOBUILD_UPGRADE}" = "YES" -o -n "${ZOHO_UPGRADE}" -a "${ZOHO_UPGRADE}" = "YES" ]
	then
		if [ -n "${SD_PROD_NAME}" -a -n "${SD_UPGRADEZIP_NAME}" ]
		then
			for SD_UPGRADEZIP_NAME_ZIP in `echo ${SD_UPGRADEZIP_NAME} | tr "," " "`
			do
		...
		if [ -n "${SD_BUILD_TYPE}" -a -n "${SD_MULTI_UPGRADEZIP_NAME_BTYPE}" ]
                then
			for i in `echo $SD_MULTI_UPGRADEZIP_NAME_BTYPE | tr "," " "`
                        do
		...
		if [ -n "${SD_MULTI_UPGRADEZIP_NAME_WITH_BTYPE}" ]
                then
                        for i in `echo $SD_MULTI_UPGRADEZIP_NAME_WITH_BTYPE | tr "," " "`
                        do
                                SD_PROD_NAME=`echo $i | cut -d ":" -f1`
                                SD_UPGRADEZIP_NAME=`echo $i | cut -d ":" -f2`
...
zohoUpgrade_Auth_common_Btype()
{
        ...
        then
                ...
                if [ ! -z "${SD_COMMENT}" ]
                then
			if [ ! -z "${CHANGE_BUILDBY}" -a ! -z "${SD_PARALLEL_DEPLOY_FORDIFF}" -a ${SD_PARALLEL_DEPLOY_FORDIFF} == "YES" ]
		        then
			...
			then
                ...
                elif [ ! -z "${SD_PARALLEL_DEPLOY}" -a ! -z ${SD_STATIC_NEEDED} ]
                then
	         ...
	         elif [ ! -z "${SD_PARALLEL_DEPLOY}" -a ! -z "${SD_ENABLE_AUTOMATION}" -a ! -z "${SD_PARALLEL_DEPLOY_FORDIFF}" -a ${SD_PARALLEL_DEPLOY_FORDIFF} == "YES" ]
		then
			 ...
			 UPGRADE_URL="https://sd.csez.zohocorpin.com/api/startupgrade?authtoken=${sdauthtokenapi}&product=${SD_PROD_NAME}&bType=${SD_BUILD_TYPE}&buildUrl=${DOWNLOAD_BUILD_URL}&parallel_deploy_for=1&enable_automation=1&parallel_deploy_for_diff_buildtypes=1"
               ...
               elif [ ! -z "${SD_PARALLEL_DEPLOY}" -a ! -z "${SD_ENABLE_AUTOMATION}" ]
                then
			UPGRADE_URL="https://sd.csez.zohocorpin.com/api/startupgrade?authtoken=${sdauthtokenapi}&product=${SD_PROD_NAME}&bType=${SD_BUILD_TYPE}&buildUrl=${DOWNLOAD_BUILD_URL}&parallel_deploy_for=1&enable_automation=1"
                ...
                elif [ ! -z "${SD_PARALLEL_DEPLOY_FORDIFF}" -a ${SD_PARALLEL_DEPLOY_FORDIFF} == "YES" ]
		then
...
CustomWebURLCall()
{
	...
	then
		...
		if [ ! -z ${WEBHOOK_BRANCH_NAMES} ]
		then
			for wbranch in `echo ${WEBHOOK_BRANCH_NAMES} | tr "," " "`
			do
				if [ "${ARG}" = ${wbranch} ]
				then
...
zohoUpgrade_Auth()
{
	if [ -n "${SD_UPGRADEZIP_NAME}" ]
	then
		zohoUpgrade_Auth_common
	fi
	if [ -n "${SD_MULTI_UPGRADEZIP_NAME}" ]
	then
		for i in `echo $SD_MULTI_UPGRADEZIP_NAME | tr "," " "`
		do
...
{
	DOWNLOAD_BUILD_URL=${BLD_URL}/${SD_UPGRADEZIP_NAME}
        if [ "${PRODUCT_NAME}" == "ZOHOLIVESUPPORT" ]
        then
	    ...
	    if [ "${PARALLEL_UPDATE_NEEDED}" == "YES" -a ! -z "${SD_PARALLEL_DEPLOY_FORDIFF}" ]
	    then
	    ...
	    elif [ "${PARALLEL_UPDATE_NEEDED}" == "YES" ]
            then
...
sanityRegressionreport()
{
	if [ "${SANITY_REGRESSION_NEEDED}" = "YES" ]
	then
...
callSdApihook()
{
	...
	if  `echo "$MODULE_NAME" | grep -q "cobbler" `
	then
	   sdauthtokenval=${zorro_cobbler_val}
	   echo "cobbler"
	elif  `echo "$MODULE_NAME" | grep -q "puppet\|dc"`
	then
...
callSecreport()
{
        ...
        if [ "$SECURITY_REPORTS_NEEDED" = "TRUE" ]
        then
            ...
            SECURITY_FLAG="TRUE"
        elif [ "${BUILD}" = "CI" -a "${PRODUCT_ID}" != "516" ]
        then
            SEC_REPORT_PRODUCTS="ZOHOLOGSERVER MICKEYLITE DEVICE_MGMT_FRAMEWORK MDM MDMP SASLITE DESKTOPCENTRAL CMTOOLS_IOS RECORDS"
            SEC_REPORT_PRODUCT_SEARCH=`echo ${SEC_REPORT_PRODUCTS} | grep -w "${PRODUCT_NAME}"`
            SECURITY_FLAG="TRUE"
        fi
        if [ "${GROUP_ID}" = "16" -o "${GROUP_ID}" = "18" -o "${GROUP_ID}" = "20" ]
        then
             SECURITY_FLAG="TRUE"
        fi
        if [ "${SECURITY_REPORTS_NEEDED}" = "TRUE" -a "${GROUP_ID}" == "5" ]
        then
             SECURITY_FLAG="TRUE"
        fi
		if [ "${SECURITY_REPORTS_NEEDED_BY_DEFAULT}" == "YES" ]
		then
			SECURITY_FLAG="TRUE"
		fi
	if [ "${SECURITY_FLAG}" = "TRUE" ]
	  then
		ORIG=$IFS
		IFS=,
                if [ "${GROUP_NAME}" == "ZOHO" -o "${GROUP_NAME}" == "ZOHO_ANDROID" -o "${GROUP_NAME}" == "ME_ANDROID" -o "${GROUP_NAME}" == "WEBNMS_ANDROID" -o "${GROUP_NAME}" == "ME" -o "${GROUP_NAME}" == "ZOHO_ELECTRON" -o "${GROUP_NAME}" == "FRAMEWORK" -o "${GROUP_NAME}" = "INTEG" -o "${GROUP_NAME}" = "OPENSOURCE" -o "${GROUP_NAME}" = "THIRDPARTY" -o "${GROUP_NAME}" = "ME_IPHONE" ]
                then
			...
			BUILD_URL=${BLD_URL}/${productname}.zip
                        ...
                        nohup curl -m 10 -X POST -H "PRIVATE-TOKEN: ${PRIVATE_TOKEN}" --insecure "${cmtoolsnewapi_domain_name}/api/v1/hacksaw?build_url=${BUILD_URL}" >> ${LOGS_DIR}/hacksaw_response.log 2>&1
...
post_buildURL()
{
	...
	then
		if [ -n "${POST_BRANCH}" ]
		then
			for branchname in `echo ${POST_BRANCH} | tr "," " "`
			do
				if [ "${branchname}" = "${ARG}" ]
				then
					posturl
...
fontCheck()
{
	...
	cd ${WORK_DIR}
        ...
        for pattern in `echo ${pattern_match}`
        do
...
get_githgclone_src_changeset()
{
	...
	repochangeset_val=""
	for fle in `echo $clone_txt_file | tr "," " "`
	do
        	if [ -f  "${BUILD_DIR}/$fle" ]
	        then
		    CHANGESET_VALUES=""
...
androidBBCheck()
{
        ...
        if [ ! -z "${allProject_result}" -o ! -z "${buildScript_result}" -o ! -z "${androidScript_result}" ]
        then
...
{
    string_match="${1}"
    for mline in $(echo "${jitpack_line_matches}")
    do
        if [ ${string_match} = "android" ]
        then
...
dBInsertionClassInSourceCode(){
        CURRENT_WORKING_DIR="`pwd`"
        CURRENT_JAVA_VERSION="${JAVA_HOME}"

        if [ "${PRODUCT_NAME}" == "ZOHOCRM" -a "${DBINSERTIONCLASS}" == "true" -a -n "${DBINSERTION_ORG_NAME}" -a "${DBINSERTION_ORG_NAME}" != "NA" -a -n "${DBINSERTION_SERVICE_NAME}" -a "${DBINSERTION_SERVICE_NAME}" != "NA" ]; then
                INSTRUMENTATION_BUILD_PATH="/zoho/build/downloads/dload/zoho/cia/milestones/Instrumentation"
                if [ -d "${INSTRUMENTATION_BUILD_PATH}" -a -f "${INSTRUMENTATION_BUILD_PATH}/CIA.zip" ]; then
                        mkdir -p ${WORK_DIR}/CIA
                        unzip ${INSTRUMENTATION_BUILD_PATH}/CIA.zip -d ${WORK_DIR}/CIA/
                        cd ${WORK_DIR}/CIA/AdventNet/Sas/tomcat/webapps
                        unzip ROOT.war
                        cd ${WORK_DIR}/
                        export JAVA_HOME=/zoho/build/downloads/java/zulujdk11.41.23
                        export PATH=$JAVA_HOME/bin:$PATH
                        java -cp "${WORK_DIR}/CIA/AdventNet/Sas/tomcat/webapps/WEB-INF/lib/*" com.zoho.cia.instrumentation.JavaJspInstrumentation ${WORK_DIR}/${PRODUCT_NAME}_DBUILD ${DBINSERTION_ORG_NAME} ${DBINSERTION_SERVICE_NAME}
                        if [ $? -eq 0 ]; then
                                log_message "DBINSERTION Process have been completed successfully without any issues." "INFO"
                        else
                                log_message "DBINSERTION Process having an issue in adding the class during java class execution." "SEVERE"
                                log_message "Java Command-Line process have been " "SEVERE"
                                log_message "java -cp \"${WORK_DIR}/CIA/AdventNet/Sas/tomcat/webapps/WEB-INF/lib/*\" com.zoho.cia.instrumentation.JavaJspInstrumentation ${WORK_DIR}/${PRODUCT_NAME}_DBUILD ${DBINSERTION_ORG_NAME} ${DBINSERTION_SERVICE_NAME}" "INFO"
                        fi
                else
                        echo "${INSTRUMENTATION_BUILD_PATH} (or) ${INSTRUMENTATION_BUILD_PATH}/CIA.zip file doesn't exists. Hence, DBINSERTION process quitted..."
                fi
        fi
...
EOF
	...
	if [ -f "${CONF_DIR}/patch_build.properties" -a "${PREV_DBUILD_URL}" != "" ]
	then
                if [ "${PRODUCT_NAME}" == "ZOHODESK" -a "${BUILD_TASKS}" == "JAVA_JSP_DEV" ]
	        then
        	    #getModifiedSource
		       source ${CONF_DIR}/buildmergecheck.conf
			if [ "${IS_ENABLE_PARTIAL_BUILD}" == "YES" ]
			then
	       ...
	       if [ "${BUILD_TASKS}" = "JAVA_JSP" -a "${PATCH_BUILD_SRC_COMPILATION}" = "FULL" ]
	       then
	...
	if [ "${PRODUCT_NAME}" == "ZOHOREPOSITORY_MIGRATION" ]
	then
	...
	if [ "${PRODUCT_NAME}" == "DEVICE_AUTH_GO" -o "${PRODUCT_NAME}" == "PILOT_NGINX" -o  "${PRODUCT_NAME}" == "EBPF_AGENT" -o "${PRODUCT_NAME}" == "POSTGRES" -o "${PRODUCT_NAME}" == "SITE24X7_HELP" -o "${PRODUCT_NAME}" == "ZOHOCRM" -o "${PRODUCT_NAME}" == "DISTRIBUTEDDB" -o "${PRODUCT_NAME}" == "ZCAL_WEB_CLIENT" -o "${PRODUCT_NAME}" == "EXTENSIONS_BUILDER" -o "${PRODUCT_NAME}" == "PHP_SOURCE_COMPILATION" -o "${PRODUCT_NAME}" = "ZOHOBOOKS_GUIDES" -o "${GROUP_ID}" -eq "16" -o "${GROUP_ID}" -eq "18" -o "${GROUP_ID}" -eq "19" -o "${PRODUCT_NAME}" = "ZOHOBOOKS_WEB_SSR" -o "${PRODUCT_NAME}" == "VANI_NUCLEUS" -o "${PRODUCT_NAME}" == "ZOHOPAGES" ]
        then
            BUILD_ENVIRONMENT="${BUILD_ENVIRONMENT} --security-opt seccomp:unconfined"
        fi
		if [ "${PRODUCT_NAME}" == "JETTY"  -o "${PRODUCT_NAME}" == "TOMCAT_NATIVE" -o "${PRODUCT_NAME}" == "ZOHO_CONFERENCING" -o "${PRODUCT_NAME}" == "ZPHOTOS" -o "${PRODUCT_NAME}" == "CRMINTELLIGENCEPY" ]
		then
...
COMMENT
	if [ ! -z "`echo ${TEAM_SPECIFIC_SHARED_FOLDER} | grep -w 'id_rsa'`" ]
        then
          ...
          TEAM_SPECIFIC_SHARED_FOLDER="${TEAM_SPECIFIC_SHARED_FOLDER} -v ${WORK_DIR}/ssh/id_rsa.pub:/root/.ssh/id_rsa.pub:ro -v ${WORK_DIR}/ssh/id_rsa:/root/.ssh/id_rsa:ro "
        fi

	if [ "${GROUP_ID}" -eq "15" -o "${GROUP_ID}" -eq "20" ]
	then
	    if [ -z "${TEAM_SPECIFIC_SHARED_FOLDER}" ]
	    then
		    TEAM_SPECIFIC_SHARED_FOLDER="${TEAM_SPECIFIC_SHARED_FOLDER} -v ${WORK_DIR}/ssh/id_rsa.pub:/root/.ssh/id_rsa.pub:ro -v ${WORK_DIR}/ssh/id_rsa:/root/.ssh/id_rsa:ro "
	    fi
	fi
        ...
        if [ "${PRODUCT_NAME}" = "ZOHOMAILSTATIC_ZM" ]
	then
        ...
        if [ "${PRODUCT_NAME}" = "ZOHOREPORTS" ]
        then
            echo "sudo docker run ${BUILD_NETWORK} -t -u ${DOCKER_USER} --rm --name="${CONTAINER_NAME}" ${BUILD_ENVIRONMENT} ${AUTH_CONFIGURATION_FILE} -v ${WORK_DIR}:${WORK_DIR} ${TEAM_SPECIFIC_SHARED_FOLDER} ${CEPH_SHARED_FOLDER} ${JDKANT_SHARED_VARIABLE} -w="${BLD_DIR}" ${IMAGE_NAME} ${BUILD_SCRIPT}"
...
COMMENT
            ...
            if [ "${PRODUCT_NAME}" = "RECEIPT_DIGITIZER" ]; then
                STORAGE_OPT=" --storage-opt size=50G "
            fi
	    if [ "${PRODUCT_NAME}" = "CONFIGTOOL" ]; then
                ENTRYPOINT=" --entrypoint= "
            fi
            echo "sudo docker run ${ENTRYPOINT} ${STORAGE_OPT} ${BUILD_NETWORK} -t -u ${DOCKER_USER} --rm --name="${CONTAINER_NAME}" ${BUILD_ENVIRONMENT} -e TZ="IST" ${AUTH_CONFIGURATION_FILE} -v ${WORK_DIR}:${WORK_DIR} ${TEAM_SPECIFIC_SHARED_FOLDER} ${CEPH_SHARED_FOLDER} ${JDKANT_SHARED_VARIABLE} -w="${BLD_DIR}" ${IMAGE_NAME} ${BUILD_SCRIPT}"
            sudo docker run ${ENTRYPOINT} ${STORAGE_OPT} ${BUILD_NETWORK} -t -u ${DOCKER_USER} --rm --name="${CONTAINER_NAME}" ${BUILD_ENVIRONMENT} -e TZ="IST" ${AUTH_CONFIGURATION_FILE} -v ${WORK_DIR}:${WORK_DIR} ${TEAM_SPECIFIC_SHARED_FOLDER} ${CEPH_SHARED_FOLDER} ${JDKANT_SHARED_VARIABLE} -w="${BLD_DIR}" ${IMAGE_NAME} ${BUILD_SCRIPT} > build.log 2>&1
:<<COMMENT
	    halt_bool=`cat build.log | grep -E "192.168.112.119:5000"`
            if [ -n "$halt_bool" ]
            then
...
COMMENT
	...
	if [ -L "${DLOAD_DIRECTORY}/webhost" ]
        then
            echo "${DLOAD_DIRECTORY}/webhost is a symlink to a directory"
            if [ ! -d "${DLOAD_DIRECTORY}/webhost" ]
            then
...
{
if [ "${PRODUCT_NAME}" = "ZOHOLOGSERVER" -o "${zohologserver_blog_update_needed}" = "yes" ]
then
        ...
        mkdir temp_build
        for zipfile in `echo $ziplist | sed "s@matches@@g" | sed "s@Binary file @@g" | sed "s@./@@g"`
        do
                echo $zipfile
...
ubi_files_removal()
{
        ...
        if [ -f "UBI_files_to_be_removed.txt" ]
        then
        full_cont=`cat UBI_files_to_be_removed.txt`
        for i in `echo $full_cont`
        do
...
get_md5_list()
{
        ...
        for all_bin in `echo $all_binary`
        do
                bin1=`md5sum $all_bin`
...
sac_warcreation()
{
        echo "Creation of SAC War Started at : " `date "+%Y-%m-%d %T"`
	bash ${WORK_DIR}/hg_utils/build/sac_war_creation_script.sh ${SOURCE_BASE_DIR} ${WORK_DIR}/sac_war_xml ${BUILD_OUTPUT} "zsocroles=\"socadmin\"" "zsocroles=\"hiadmin\""
        echo "Creation of SAC War ended at : " `date "+%Y-%m-%d %T"`
...
copyBuildoutput()
{
	...
	zohologserver_blog_update
        if [ "${PRODUCT_NAME}" = "SOC" ]
	then
		sac_warcreation > ${LOGS_DIR}/sac_warcreation.log 2>&1
	fi
	echo "Copy Build output started at : " `date "+%Y-%m-%d %T"`
        if [ "${BUILD}" == "CI" ]
        then
            if [ "${PRODUCT_NAME}" == "ZOHOREPOSITORY" ]
            then
	...
	sed -i 's@http://sd\/@https://sd.csez.zohocorpin.com\/@g'  ${CONF_FILE}
        ...
        if [ "${PRODUCT_NAME}" == "OPMUNIFIED" ]
        then
             cd ${BUILD_OUTPUT}
             if [ "${Binary_32bit_Zip_Needed}" = "No" -o "${Binary_32bit_Zip_Needed}" = "" ]
             then
                 find -maxdepth 1 -type f -name "*32bit.zip" -exec rm -f {} \;
                 find -maxdepth 1 -type f -name "*32.zip" -exec rm -f {} \;
             fi

             if [ "${ARG}" = "OPMUNIFIED_REVIEWED" -o "${ARG}" = "OPMUNIFIED_SITE24_7_128238_BRANCH_BRANCH" ]
	     then
	...
	if [ "${BUILD_TASKS}" == "JAVA_JSP" ]
        then
                ...
                if [ -f "${JAVA_JSP_FILE}/java_jsp_source.zip" ]
                then
        ...
        then
	       ...
	       find . -type f -iname "sample.zip" | sed -e "s@^./@@g" >> $WORK_DIR/exclude_files.txt
            ...
            if [ -s "$WORK_DIR/exclude_files.txt" ]
            then
            ...
            if [ -n "${EXCLUDE_ZIPS}" ]
            then
                echo "EXCLUDE ZIPS ARE ::: ${EXCLUDE_ZIPS}"
                cd ${BUILD_OUTPUT}
                cp -rfv `ls -A ${BUILD_OUTPUT}/ | grep -vE "$EXCLUDE_ZIPS"` ${DLOAD_DIR}/${BUILD_DLOAD_DIR}
                copy_op_status=$?
                cd -
            else
                cp -rfv ${BUILD_OUTPUT}/** ${DLOAD_DIR}/${BUILD_DLOAD_DIR}
		copy_op_status=$?
            fi
            ...
            cp -rf ${BUILD_DIR}/{buildlogs,build.log,post_build.log,pre_build.log,combine.js,lyte_docker_creation.log} ${DLOAD_DIR}/${BUILD_DLOAD_DIR}/logs/
	...
	if [ "${SRC_ZIP_PASSWORD_PROTECT}" == "YES" ]
	then
        	if [ -d "${DLOAD_DIR}/${BUILD_DLOAD_DIR}/zips" ]
	        then
	...
	cd -
        ...
        if [ "${BUILD}" = "CI" -o "${GROUP_ID}" -eq "4" ]
        then
		Autolink
	fi
	if [ -z "`echo ${AUTO_LINK_BRANCH_LIST} | grep -w "${ARG}"`" -a "${BUILD_FROM}" == "${BUILD}" -a "${BUILD_DAY}" == "`date +%A`" ]
        then
            Autolink
	fi
...
findAndDeleteRestrictedFile()
{
    ...
    then
         cd ${build_pkg_dir}
         for cksum_file in $(find ${PWD} -type f -iname "*" | grep -v "android_dependency_report.zip\|android_java_classes.zip")
         do
...
copyBuilderrorlogs()
{
	...
	mkdir -p ${ERROR_DLOAD_DIR}/logs
        ...
        then
            ...
            cp -rf ${BUILD_DIR}/{build.log,buildlogs,post_build.log,combine.js,lyte_docker_creation.log} ${ERROR_DLOAD_DIR}/logs
	    ...
	    if [ -f "${ERROR_DLOAD_DIR}/js_source.zip" ]
            then
...
getExactError()
{
    ...
    then
        for line in `cat ${ERROR_DLOAD_DIR}/logs/buildlogs/taskname.txt`
        do
			...
			do
				...
				if [ -f "${ERROR_DLOAD_DIR}/logs/buildlogs/${exfilename}" ]
				then
					if [ -s "${ERROR_DLOAD_DIR}/logs/buildlogs/${exfilename}" ]
					then
...
checkAndCleanDir()
{
	...
	Rem_dir=`dirname "${dir}"`
	if [ "${Rem_dir}" = "/zoho" -o "${Rem_dir}" = "${HOME}/dload" ]
	then
...
buildDockerFile()
{
        sudo cp -rf /zoho/build/downloads/ssh ${WORK_DIR}/
        sudo chown build:build -R ${WORK_DIR}/ssh/
        echo "buildDockerFile started at : " `date "+%Y-%m-%d %T"`
        if [ "${GROUP_ID}" -ne "16" -a "${GROUP_ID}" -ne "18" -a "${GROUP_ID}" -ne "19" -a "${GROUP_ID}" -ne "15" -a "${GROUP_ID}" -ne "20" -a "${PRODUCT_NAME}" != "ZOHOSEARCH_MISC" -a "${PRODUCT_NAME}" != "HG_UTILS" -a "${GROUP_ID}" -ne "31" -a "${GROUP_ID}" -ne "30" ]
        then
             sed -i "s@cmtools.csez.zohocorpin.com:5000@integ-cm.csez.zohocorpin.com@g" $(grep -irl "cmtools.csez.zohocorpin.com:5000" `find ${WORK_DIR} -maxdepth 4 -type f \( -name "*.sh" -o -name "*.properties" -o -name "*Dockerfile" \)`) 2> /dev/null
             sed -i "s@integ-cm.csez.zohocorpin.com:5000@integ-cm.csez.zohocorpin.com@g" $(grep -irl "integ-cm.csez.zohocorpin.com:5000" `find ${WORK_DIR} -maxdepth 4 -type f \( -name "*.sh" -o -name "*.properties" -o -name "*Dockerfile" \)`) 2> /dev/null
        fi
	...
	then
		...
		if [ "${PRODUCT_NAME}" != "TEAMINBOX" ]; then
	...
	then
        ...
        if [ `echo "${IMAGE_NAME}" | grep "5000"` ]
        then

         IMAGE_NAME_WITHOUT_TAG="`echo ${IMAGE_NAME} | cut -d ":" -f1-2`"
	 IMAGE_NAME_OF_TAG="`echo ${IMAGE_NAME} | cut -d ":" -f3`"
        else
         IMAGE_NAME_WITHOUT_TAG="`echo ${IMAGE_NAME} | cut -d ":" -f1`"
	 IMAGE_NAME_OF_TAG="`echo ${IMAGE_NAME} | cut -d ":" -f2`"
        fi
...
evalRestriction()
{
	...
	for i in `find ${WORK_DIR}/${CHECKOUT_DIR} -iname "*.jsp" -o -iname "*.jspf" -o -iname "*.js"`
	do
...
compareThirdpartyLabels()
{
	if [ "${TP_LBL_DIFF_NOTIFY}" = "TRUE" -a -n "${TP_CO_LABEL}" -a -n "${ACTUAL_TP_LBL}" ]
	then
		if [ "${ACTUAL_TP_LBL}" != "${TP_CO_LABEL}" ]
		then
