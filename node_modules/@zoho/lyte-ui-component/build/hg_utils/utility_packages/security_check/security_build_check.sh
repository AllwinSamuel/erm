...
SecurityCheck()
{
	...
	then
		...
		accesslog_filterMap=`cat ${XML_OUTPUT} | grep ACCESS_FLT_VALUE | cut -d "=" -f2`
		fname=`basename ${web}`
		dname=`dirname ${web}`
		if [ -d ${dname}/CVS ]
		then
...
printSecurityFilterError()
{
	...
	Please add the AccessLogFilter mapping entry at the top in ${req_filename} (after <web-app ..> tag)
