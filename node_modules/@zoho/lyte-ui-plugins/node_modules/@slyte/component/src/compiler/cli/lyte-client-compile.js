...
// function ClientCompiler(Lyte, _LC, Compile,doCompile ,expHandlers,Espree) {
    ...
    Compile.defaultSrcComponents = ["lyte-event-listener","import-shadow-style","lyte-yield"];
    ...
    Compile.defaultDirectives = ["shadow","shadow-supported","turbo","unbound-supported","view-in","view-out","unbound"];
        ...
        if (s) {
            ...
            for(let i = 0; i< attr.length; i++){
                if(attr[i].name.startsWith("@")){
                    attrDirectives.push(attr[i].name);
                }
            }
            let dirMap = {
                "shadow-supported" : "lyte-shadow",
                "unbound-supported" : "lyte-turbo",
            }
            if(this.impObj.defaultDirectives.length || this.impObj.directives.length || attrDirectives.length){
            // if(this.impObj.defaultDirectives.length || this.impObj.directives.length){
                this.impObj.defaultDirectives.push("lyte-directive");
            }
            if(this.impObj.defaultDirectives.indexOf("unbound") != -1 && this.impObj.defaultDirectives.indexOf("lyte-turbo")==-1 && this.impObj.defaultDirectives.indexOf("turbo") == -1){
                this.impObj.defaultDirectives.push("lyte-turbo");
            }
            let fastInd = this.impObj.defaultDirectives.indexOf("turbo")
            if(fastInd != -1){
                this.impObj.defaultDirectives.splice(fastInd,1,"lyte-turbo");
            }
            let shadowInd = this.impObj.defaultDirectives.indexOf("shadow")
            if(shadowInd != -1){
                this.impObj.defaultDirectives.splice(shadowInd,1,"lyte-shadow");
            }    
            let self = this;
            attrDirectives.forEach(function(dir){
                let d = dir.split("@")[1];
                if(dirMap[d]){
                    d = dirMap[d];
                }
                if(self.impObj.defaultDirectives.indexOf(d) == -1){
                    self.impObj.defaultDirectives.push(d);
                }          
            })
            if (errors.length) {
                returnValue = {
    ...
    var ClientCompilerInternal = Compile;
// }
try {
    module.exports = ClientCompilerInternal;
}
catch(e){
