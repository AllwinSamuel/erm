...
SecurityCheck()
{
	...
	then
		...
		commerceinternal_filterMap=`cat ${XML_OUTPUT} | grep COMMERCE_INTERNAL_FM_VALUE | cut -d "=" -f2`
		fname=`basename ${web}`
		dname=`dirname ${web}`
		if [ -d ${dname}/CVS ]
		then
...
commerceInternalFilterCheck()
{
	if [ "${commerceinternal_filterMap}" == "TRUE" -o "${commerceinternal_filterMap}" == "NOT_CONFIGURED" ]
	then
		echo "*** CommerceInternal filter check is success ***"
	else
		echo "CommerceInternalFilter=FAILED" >> ${WORK_DIR}/xmloutval.txt
	fi
}
printSecurityCodeError()
{
...
printFilterError()
{
	if [ "${commerceinternal_filterMap}" == "TRUE" ]
	then
		FILTER_ORDER="Instrumentation, AccessLogFilter, CommerceInternalFilter, SecurityFilter, ServerHealthCheckFilter"
	else
