// debugger
import { establishObjectBinding, isEntity, validateData, establishObserverBindings, cmpData, removeNestScp, nestScp, bindObj, deepCopyObject, checkNestedProp, prop} from "@slyte/core/src/lyte-utils";
import { Lyte, LyteAddon, Logger } from "@slyte/core";
import Compile from './compiler/cli/lyte-base-compile.js';
// window.cliConfig = {"component" : { "needDummyComponentsDiv" : true} };
// Compile.needDummyComponentsDiv = window.cliConfig.component.needDummyComponentsDiv;
import './ZohoSecurity/lyte-component-security.js';
import { ValidationError } from "@slyte/data/src/dberror.js";
...
// import "./components/lyte-yield.js";
...
class ComponentRegistry extends Service{
    ...
    constructor(opt){
        ...
        if(!registryClass._registered){
            Lyte.error("'" + registryClass.name + "' registry is not registered with Lyte.")
            return;
        }
        ...
        this.viewPortSettings = _LC.viewPortSettings;
        this.chI = _LC.chI;
        this.chromeBugFix = _LC.chromeBugFix;
        this.isInViewPort = _LC.isInViewPort;
    ...
    setDefaultDirectives(app){
        let defaultDirectives = ["directive","shadow","shadow-style","shadow-supported","turbo","unbound"];
        let self = this;
    ...
    static register(options){
        ...
        /*convert to custom class*/
        this.Directive = class newD extends Service {
            constructor(){
                super();
            }
            static _registryClass = registryClassDef;
            static register(compName){
                compName = compName || _LC.String.dasherize(this.name);
                this._registryClass = registryClassDef;
                registryClassDef.registerDirective(compName, this);
            }
        };
        this.registerDirective = function(name,directiveClass){
            var registry = this;
            if(registry._directivesList[name]){
                Lyte.error("Component "+name+" already registered with "+registry.name+" registry.");
                return;
            }
            ...
            if(registry._instanciated){
                registry._instanceList.forEach(function(ins){
                    // registry.actualRegisterDirective(name,directiveClass,ins);
                    // ins.registeredDirectives.push(name);
                    let directiveObj = ins.getDirectiveObj();
                    if(directiveObj){
                        directiveObj.constructor.actualRegistration(name,directiveClass,ins);
                    }else{
                        ins.constructor._directivesList[name] = directiveClass;
                    }
                })
            }
        }
    
        // this.actualRegisterDirective = function(name,directiveClass,registryInstance){
        //     // registryClassDef.registerDirective.apply(_LC, arguments);
            
        //     if(directiveObj){
        //         directiveObj.actualRegistration(name,directiveClass,registryInstance);
        ...
        //     }else{
        //         // registryInstance._preLoadedDirectives[name] = directiveClass;
        //         registryInstance._directivesList[name] = directiveClass;
    ...
    static registerAllComponent = function(compClassList, registryInstance) {
        ...
        if (registryInstance.constructor._definedComponents[compName] && registryInstance.constructor._reRegisteredComponents.indexOf(compName) == -1 && !registryInstance.constructor._unRegisteredComponents[compName]) {
            ...
            compClass.__observers = compClass.observers();
    ...
    static Component = class newC extends ComponentRegistry._componentApis{
        ...
        static register(compName){
            ...
            if(ComponentRegistry.globalComponents.indexOf(compName) != -1){
                ...
                Lyte.error("'" +compName + "' : Component class should extend a ComponentRegistry's Component property directly or extend through base component.");
            }
        }
    };
    static _addDirective = function(directiveBaseClass){
        this._lazyRegisterDirectives["directive"] = directiveBaseClass;
        this.directive = directiveBaseClass;
        _LC.updateDirectivesInReg(this,directiveBaseClass,"directive");
    }
    static _addShadow = function(ShadowBaseClass){
        this._lazyRegisterDirectives["shadow"] = ShadowBaseClass;
        this.shadow = ShadowBaseClass;
        _LC.updateDirectivesInReg(this,ShadowBaseClass,"shadow");
    }
    static _addFastRender = function(FastRenderClass){
        this._lazyRegisterDirectives["turbo"] = FastRenderClass;
        this.fast = FastRenderClass;
        _LC.updateDirectivesInReg(this,FastRenderClass,"turbo");
    }
};
/*convert to custom class*/
class Component extends ComponentRegistry._componentApis {
    ...
    static register(compName, options){
        ...
        if(!_registryClass){
            Lyte.error("registry class for the component '"+compName+"' not registered with Lyte")
            return
        }
        this._refHash = options.refHash
        compName = compName || _LC.String.dasherize(this.name);
...
class RawComponent extends Service {
    ...
    static register(compName, options){
        var _registryClass = _LC.getRegistryClass(options);
        if(!_registryClass){
            Lyte.error("registry class for the rawcomponent '"+compName+"' not registered with Lyte")
            return;
        }
        this._refHash = options.refHash
        compName = compName || _LC.String.dasherize(this.name);
...
class Helper {
    static register(name, helper, options){
        var _registryClass = _LC.getRegistryClass(options);
        if(!_registryClass){
            Lyte.error("registry class for the helper '"+name+"' not registered with Lyte")
            return;
        }
...
var _LC = {
    ...
    _render : function(object){
        ...
        if(compClass._registered){
            if(_LC.validateRenderData(data)){
                if(registryInstance){
                    if(!_LC.isValidReg(registryInstance)){
                        Lyte.error("Invalid registry instance'");
                        return;
                    }
            ...
            Lyte.error("Component is not registered with Lyte");
        }
    },
    Security : {
        defaultTags : ["link-to"],
        ...
        sanitizeHTML : function(obj){
            ...
            if(instance && Object.keys(instance).length){
                ...
                Lyte.error("Sanitizer instance should be passed to sanitize the content");
                return;
            }
    ...
    "validateRegistry" : function(reg,moduleName){
        if(!_LC.isValidReg(reg)){
            ...
            Lyte.error("Invalid registry instance '" + name + "' passed in addRegistries hook of "+moduleName);
            return false;
        }
        return true;
    },
    ...
    "isValidateLyteReg" : function(reg,registryMap){
        if(!_LC.isValidReg(reg)){
            Lyte.error("Invalid registry instance" + reg.constructor.name);
            return false;
        }else if(registryMap.get(reg)){
            Lyte.error("'" + reg.constructor.name+ "' : Registry linked recursively in the app.");
            return false;
        }
    ...
    "registerCeComponent" : function(componentName, componentClass, registryInstance){
        ...
        if(registryInstance.registeredCeComponents[componentName] && ComponentRegistry._registeredCommonCeClass[componentName]._reg[registryName]) {
            ...
            if (document.readyState === "complete" || document.readyState === "interactive") {     
                customElements.define(componentName, customLyteCustomComponent);
            }
            else{
                _LC.toBeRegistered.$push({name:componentName, def: customLyteCustomComponent});//af check
            }
    ...
    "registerComponent" : function(componentName,componentClass,registryInstance) {
        ...
        if(!ComponentRegistry._definedComponents[componentName]) {
            if (document.readyState === "complete" || document.readyState === "interactive") {     
                ...
                _LC.toBeRegistered.$push({name:componentName, def: customCrmComponent});//af check
            }
        }
    ...
    "componentSet" : function(key, value, options, forceExecute, fromParent) {
        if(!forceExecute && this.get(key) === value) {
            ...
            //     }else{
            //         Lyte.warn("The data passed to update '" + key + "' is same as actual data. So component bindings won't be updated.");    
            //     }
            // }
            return;
        }
...
//                                          this.establishBindings(item._propBindingObject, value);
        ...
        case "sort" : 
            ...
            commands.forEach(function(command) {
                switch(command.type) {
                    case "Remove" : {
                        array.splice(command.toIndex, 1);
                        optionsArray.push({"firstIndex" : command.toIndex, "secondIndex" : 1, "type" : "remove"});
                    }
                    break;
                    case "Move" : {
                        array.splice(command.fromIndex, 1);
                        array.splice(command.toIndex, 0, command.element);
                        optionsArray.push({"firstIndex" : command.fromIndex, "secondIndex" : 1, "type" : "remove"});
...
//                    }
    ...
    "executeMethod" : function() {
        ...
        if(!this._methods[methodName]) {
            Logger.error("LC009", methodName, this.$node.tagName);
            return;
        }
    ...
    "render" : function(componentName, data, outlet, _lyteOptions) {
        ...
        if(componentName && _LC.validateRenderData(data)) {
            ...
            } else if(componentName._compName) {
...
Lyte._component.didConnect = function(lyteIns){
    ...
    if(lyteIns.getDefaultRegistry){
        ...
        if(!_LC.isValidReg(regIns)){
            ...
            Lyte.error("Invalid registry instance '" + name + "' passed in setDefaultRegisty hook of "+ name);
            return false;
        }
...
// if(!customElements.get("lyte-yield")){
    ...
    //         let shadowObj = registry._shadow;
...
class LyteCustomElement extends HTMLElement{
    constructor(){
        ...
        if(!_LC.verifyDetails(obj)){
            Lyte.error(this.localName + " : Component Not found in any registry.");
            return
        }
        var lIns = obj.lIns ,compClass = obj.compClass ,regIns = obj.regIns;
        var compInstance;
        ...
        if(options && options.clone){
            ...
            this._connectedCallback = options.clone.connectedCallback;
            this._disconnectedCallback = options.clone.disconnectedCallback;
        }
    ...
    connectedCallback(){
        ...
        if(this.isClonedNode() && (this._allCallbacks == false || this._connectedCallback == false)){
            return;
        }
    ...
    disconnectedCallback(){
        ...
        if(this.isClonedNode() && (this._allCallbacks == false || this._disconnectedCallback == false)){
            return;
        }
...
class customElementPrototype extends elementPrototype{
    ...
    actualConstructor() {
        ...
        if(directiveObj){
            directiveObj.setTagDirectives(compClass,this);
            directiveObj.setAttrFromRender(this,this._tagDirectives);
        }
    ...
    getDirectiveObj(){
        if(this.$registry._directive){
            return this.$registry._directive;
        }
        return false;
    }
    getShadowObj(){
        ...
        // if(this._registryClass._shadow){
        //     return this._registryClass.shadow;
        // }
        return false;
    }
    getFastObj(){
...
//        		let attr = templateAttributes.attr[i];
        		...
        		if((!this.hasAttribute(attr.name) && !compData.hasOwnProperty(attr.name) ) || attr.globalEvent) {
                    ...
                    if(!attr.globalEvent) {
                        ...
                        if(attr.staticValue) {
                        ...
                        else if(directiveObj && directiveObj.isDirectiveNode(attr)){
                            ...
                            Lyte.error("Directive source not loaded")
                        }else{
        ...
        if(directiveObj) {
            let unboundDirective = directiveObj.getTransitionArg(this,"unbound");
            if(unboundDirective){
                _LC.unbound = true;
            }
            if(!dumFlg){
        ...
        if(directiveObj){
            fastRender = directiveObj.getTransitionArg(this,"turbo");
            this.component.data.lyteFastRender = fastRender;
            if(fastRender){
                fastRenderSupported = directiveObj.getTransitionArg(this,"unbound-supported");
                if(!fastRenderSupported){
                    Lyte.error(this.localName+" component not supporting fast render. please add @unbound-supported in the component's template tag.");
...
//                updateHelpers.$push(dynamicN);
        ...
        for(let i=0;i<processLast.length;i++) {
             let dynamicPosition = processLast[i].dynamicPositions;
                 ...
                 if(Lyte._rwpf && processNode.parentNode.nodeName === "#document-fragment") {
...
//              this.updateYield(updateHelpers[i], false, contextSwitchInfo);
    ...
    static get observedAttributes() {
            ...
            this.componentClass._registryClass.customPropHandlers.forEach(function(item,index) {
    ...
    removeHelpersSpecificIndex(node, index,totalProms,fakeRemove,previousPromise) {
        ...
        let directiveObj = this.getDirectiveObj();
        if(directiveObj && node.hc && node._forContent[index]){
    ...
    updateYield(node, update, contextSwitchInfo,helperNode) {
        ...
        if(!Lyte._ms && toAppend._ht && !toAppend._htA) {
            toAppend._htA = true;
        ...
        if(!directiveObj || !directiveObj.getTransitionArg(node,"unbound")) {
...
//          let componentData = {};
            for(let i=0;i<node.attributes.length;i++) {
                if(node.attributes[i].name !== "is" && node.attributes[i].name !== "component-name" && node.attributes[i].name !== "lyte-keep-alive" && !node.attributes[i].name.startsWith("@")) {
...
//                  }
        ...
        if(directiveObj) {
            let unboundDirective = directiveObj.getTransitionArg(node,"unbound");
            if(unboundDirective){
                if(unboundDirective == "lyteFastRender"){
                    Lyte.error("Passing lyteFastRender to unbound option in 'for' helper is depricated. So please make use of '@turbo' directive in the node - ." + _LC.errorNodeDetails(node));
                }else{ // if(unboundDirective !== "false")
                    localUnbound = true;
                    _LC.unbound = true;
                }
            }
            let fastDirective = directiveObj.getTransitionArg(node,"turbo");
            if(fastDirective){
        ...
        if(!Lyte._ms && (lastIndexForIteration - firstIndexForIteration) > 0) {
...
//                  keysArray = Object.keys(object);
        ...
        if(!Lyte._ms && keysArray.length && node._ht && !node._htA) {
            node.content.append(node._ht.content.cloneNode(true));
            node._htA = true;
        }
...
//              }
   ...
   updateSwitchHelper(type,node, contextSwitchInfo, update, establishBindings,yieldComp,templateCaseNode,helperNode,id,idx){
        ...
        if(!Lyte._ms && node._ht && !node._htA) {
            node.content.append(node._ht.content.cloneNode(true));
            node._htA = true;
        }
        if(node.tagName !== "TEMPLATE") {
        ...
        if(!scope){
            ...
            defaultContent = (Lyte._ms) ? scope.templateContent : nodeTemp.content.querySelector('[default]');
            if(!Lyte._ms && scope._ht && !defaultContent._htA) {
...
//        }
        ...
        if(defaultContent) {
            ...
            template = (Lyte._ms) ? {} : nodeTemp.content.querySelector((scope.dcn ? '[lc-id=\'' : '[case=\'') +_LC.cssEscape(scope.cn ? scope.cn : dummyCaseName)+'\']');
        }
        // if(!template._callee){
        ...
        let nextCaseName;
        while(dummyScope) {
            if(!Lyte._ms && dummyScope._ht && !template._htA) {
                template._htA = true;
            ...
            if(dummyScope.additional) {
                ...
                if(dummyScope.additional.next) {
                    ...
                    template = (Lyte._ms) ? {} : node.content.querySelector(caseStr+_LC.cssEscape(dummyScope.additional.next)+'\']');
                    dummyScope = node._cases[dummyScope.additional.next];
                } else {
                    template = (Lyte._ms) ? {} : node.content.querySelector('[default]');
...
//            this._template.content = frag;
        ...
        if(!componentClass._dynamicNodes){
            if(Compile.getDynamicNodes) {
                var returnVal = Compile.getDynamicNodes(a, undefined, Lyte._ie ? s : undefined);
                if(returnVal.errors){
        ...
        if(componentClass._dynamicNodes) {
            ...
            let fastRenderClass = registry.fast;
            ...
            if(hasUnbound) {
                if(fastRenderClass){
                    ...
                    Lyte.error("Fast render source not bundled");
                }
                // if(Lyte._ie) {
            ...
            doCompile(s, componentClass._dynamicNodes, a, b, newCompile ? (Lyte._ie  ? newCompile : newCompile.content) : undefined, componentClass, fastRenderClass);
            if(Lyte._ed) { 
    ...
    static splitTextNodes(node) {
        ...
        if(node.nodeType === node.TEXT_NODE) {
            ...
            if(nodeValue){
                ...
                if(Lyte._rwpf && node.parentNode.nodeName === "#document-fragment") {
...
//        }
        if(node.nodeType === 2) {
            ...
            if(directiveObj){
                isDirectiveNode = directiveObj.isDirectiveNode(node)
            }
        }
...
//            			}
        ...
        if(isDirectiveNode){
            directiveObj.setNodeArgs(node,nodeValue);
        }
        else if(node.nodeType === 2) {
...
//                      node.ownerElement.removeAttribute(node.nodeName);
    ...
    updateNode(node, updatePath) {
        ...
        let isDirectiveNode;
        let directiveObj = this.getDirectiveObj();
        if(node.nodeType == 2 && directiveObj){
            isDirectiveNode = directiveObj.isDirectiveNode(node)
        }
        ...
        if(isDirectiveNode){
            directiveObj.setNodeArgs(node,origNodeValue);
        }
        else if(node.nodeType === 2) {
        	let parentNodes = [];
...
//                 	}
    ...
    connectedCallback() {
        ...
        if(isViewPort && this.component.constructor._template.getAttribute("__vp") != "c-new") {
    ...
    actualConnectedCallback(){
        ...
        if(typeof content === "string") {
            ...
            if(shadowObj){
                directiveObj.setAttrFromRender(this,this._tagDirectives);
                var shadowSupported = directiveObj.getTransitionArg(this,"shadow-supported")
                if(shadowSupported){
                    if(this.$registry._defaultDirectives && this.$registry._defaultDirectives.indexOf("shadow") != -1){
                        var shadowVal = directiveObj.getTransitionArg(this,"shadow")
    ...
    disconnectedCallback() {
        ...
        if(_LC.viewPortSettings.handlePendingItems && this._vpc) {
            _LC.executeVpe = true;
        }
        if(pve.length){
            for(let i=0; i<pve.length; i++){
                if(pve[i] && pve[i].tagName == "TEMPLATE" && pve[i].getAttribute("is") == "switch" && pve[i]._callee == this){
                    pve.splice(i,1);
                    i--;
                }
            }
...
//                              LN to do
        ...
        case "cM" : {
            var actualTemplate = Lyte._ie ? dynN : dynN.content;
        ...
        case "s" : {
            ...
            if(Lyte._ms) {
            	var cases = {};
                var defCase;
                var dynNchildNodes = Lyte._ed ? dynN.content.childNodes : dynN.childNodes;
...
function getDynamicNode(content, positions){
    ...
    for(var i=0; i<positions.length; i++){
        dynamicN = (dynamicN.tagName != "TEMPLATE" || Lyte._ie) ? dynamicN.childNodes[positions[i]] : dynamicN.content.childNodes[positions[i]];
    }
    return dynamicN;
}
...
defHelpers["method"] = function(parentComponent, attributeNode, functionName) {
    ...
    var newFunc = function() {
        ...
        Lyte.error("LC009", functionName, self.$node.tagName);
    }
    if(childComponent) {
...
// ComponentRegistry.prototype.addAction = function(element, eventName, func, context) {
...
// Object.defineProperty(window, "_LC", {
// 	get : function() {
// 		Lyte.error("Usage of _LC is deprecated. Use Lyte.Component instead");
// 		return _LC;
// 	}
// })
_LC.chromeBugFix();
...
//         _LC.updateCustomCrmComponent(newDefinition,customCrmComponent);
...
// }
    ...
    //     var stack=[], start=0, helper = { name:"concat", args: []}, flag=false;
    ...
    //             if(actObj){
    //                 helper.args.$push({type:"helper",value:actObj});
    //                 if(/\[(?=([\s]*[\w-_.]+)([\(\[]))/g.test(toPush)){
    //                     flag = true;
    //                 }
    //             }else{
    ...
    //             }
    //         }
    //         if(actObj && actObj.name == "lbind"){
    //             errors.$push({message:"(LC:lbind5) Syntax Error : lbind are not allowed with prepending or appending with texts "+"for :"+str +" in the component "+componentName});
    //             return undefined;
    //         }
    //         else if(actObj && (actObj.name == "action")){
    //             errors.$push({message:"(LC:9)Syntax Error : action are not allowed with prepending or appending with texts "+"for :"+str +" in the component "+componentName});
    //             return undefined;
    //         }
    //         else if(actObj && (actObj.name == "method")){
    //             errors.$push({message:"(LC:10)Syntax Error : method are not allowed with prepending or appending with texts "+"for :"+str +" in the component "+componentName});
    //             return undefined;
    //         }
    //         start = i + 1;
    //      }
    ...
    //     if(helperValue){
    //         let actObj = this.getHelperInfo(dynamicValue,helperValue, newCompile);
    ...
    //                 return argValue;
    //             }
    //             else if(/\[(?=([\s]*[\w-_.]+)([\(\[]))/g.test(argValue) && !(/\)$/g.test(argValue))){
    //                 //if array inside helper or array inside array it should work
    ...
    //     }
    // }
    // Lyte.compile.getMustache = function(value,syn){
    ...
    //     return undefined;
    // }
    // Lyte.compile.syntaxCheckWorkerNew = function(val){
    //     var parsingMustache;
    //     var retObj = {
    //         "mustache":0,
    //         bool : true,
    ...
    //     return retObj;
    // }
    // Lyte.compile.getArray = function(mustacheVal){
    ...
    //     }
    // }
    // Lyte.compile.splitMultiDimensionArray = function(val,arrayExtra){
    ...
    //     return resultStack;
    // }
    // Lyte.compile.getArrayArg = function(argStr){
    //     if(/\(.*\)$/.test(argStr)){
    ...
    //         for(var i=0; i<arr.length; i++){
    //             if(/\[(?=([\s]*[\w-_.]+)([\(\[]))/g.test(arr[i]) || /\(.*\)$/.test(arr[i])){
    ...
    //     } 
    // }
    // _LC.replaceWithOrigTemplate = function(node){
    ...
    var defHelpers = ComponentRegistry._defaultHelpers;
    defHelpers["lyteViewPort"] = function(node,disableViewPort){   
            ...
            if(!_LC._pendingAnimationFrame){
                requestAnimationFrame(function () {
                    requestAnimationFrame(function () {
                        ...
                        _LC._pendingAnimationFrame = false;
                ...
                _LC._pendingAnimationFrame = true;
            }
    ...
    _LC._isInViewPort = function(node,comp){
    ...
    // _LC.getActViewPortTemplate = function(node){
    //     return node._origTemplate ? node._origTemplate : node;
    // }
    // _LC.getFakeTemplate = function(node){
    ...
    _LC.executePendingViewPort = function() {
        ...
        for(var i=arr.length-1;(i >= 0);i--) {
            let ele = arr[i];
            if(ele.tagName == "TEMPLATE" && ele.getAttribute("is") == "switch"){
    ...
    // if(compConfig.idePlugins){
    //     compConfig.idePlugins.forEach(function(plugin){
    //         plugin(_LC);
    //     })
    // }
...
// function lyteCompConstructor() {

// }

// function getCustomElementClass() {
//     var _LC = createCustomClass(function(arg1, overrides){
//         class _LC extends getClass([Service], arg1){
//             // static register(compName, componentClass) {
//             //     var definition = {};
//             //     definition.data = componentClass.prototype.data;
//             //     definition.actions = componentClass.prototype.actions();
//             //     definition.methods = componentClass.prototype.methods();
//             //     this._lyteInstance.Component.register(compName, definition, {}, componentClass);
//             // }
//         } 
//         return overrides(_LC)
//     })
//     return _LC;
// }
...
export {ComponentRegistry,Component,Directive,RawComponent,Helper,arrayUtils,objectutils,set,appendChild,insertAfter,insertBeforeFn as insertBefore,replaceWith,shouldIgnoreDisconnect,defProp,render,_LC};
