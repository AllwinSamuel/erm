...
function buildVariables(gqlvar, variables, key, type){
	...
	for(var v in variables){
		...
		if(variables[v].search(/!/g)!=-1){
			variables[v]=variables[v].replaceAll(/!/g,"");
		}
		...
		if (typeof(fieldArr[v]) == "object"){
			...
			if(sendvar){
				...
				for(var s in sendvar){
					...
					else if(parent!='' && s.search(/[.]/s)!=-1){
						//var pcheck=s.replaceAll(".","_");
						if(parent == s.match(/.*(?=\..*$)/s)[0]){
							var split= s.split('.');
...
function parseQuery(db,def,key,type,queryParams,gqlObj,mdlq,gqlvar,urlObj,LocalAttrs,data,customData,argsObj){
	...
	if(typeof(mdlq) == "string" && mdlq.search(/[ ]/g)!=-1){
		...
		cquery=cquery.replaceAll(new RegExp('\n','g')," ");
		cquery=cquery.replaceAll(/ *[ ]*\(/g,":{variables:[");
		cquery=cquery.replaceAll(/ *\) *{/g,"],attrs:[");
		cquery=cquery.replaceAll(/\w+/g,'"$&"');
		cquery=cquery.replaceAll(/"!/g,'!"')
		cquery=cquery.replaceAll(/" +"/g,'", "');
		cquery=cquery.replaceAll(/\S+{"variables"/g,"{$&");
		cquery=cquery.replaceAll(/} *"/g,'}, "')
		cquery=cquery.replaceAll(/" *{/g,'" :{"attrs" :[')
		cquery=cquery.replaceAll(/\} *\}/g,"]}}");
		cquery=cquery.replaceAll(/" *}/g,'"]} }');
		...
		cquery=cquery.replaceAll(/]}}]}} *}/g,"]}}]}}]}}")
		cquery=cquery.replaceAll(/} *{/g,"},{");
		cquery=cquery.replaceAll(/\S+ *:{"attrs"/g,"{$&");
