...
import { _bindLyteInstanceToError, _consoleError, _getError } from './router-errors';
	...
	constructor() {		
		...
		run[beforeRouteNavigationStr] = function() {
			if(validateTransition(this)) {
				...
				callback && callback.call(LR, obj.prevTrans, obj.trans , obj.history);
		...
		run[afterRouteNavigationStr] = function(_trans) {
			...
			triggerEventOnInsAndClass(afterRouteNavigationStr, {trans : _trans})
		};
			...
			return new Promise(function(resolve, reject) {
				...
				st = setTimeout(function() {
					if(hook == afterRenderStr) {
						...
						callHookWithPromise.call(this,getHook(routeInstance, hook),routeInstance,[routeInstance.currentData,routeInstance.__lp.param,routeInstance.component])
						.then(function(data) {
		...
		run[renderLoadingTemplateStr] = function(hook,index) {
			...
			if(loadingTemplate && loadingTemplate.outlet && (outlet = getOutlet(loadingTemplate.outlet,route.parent))) {
				...
				if(loadingTemplate.component) {
					renderComp(outlet, loadingTemplate.component, {data : loadingTemplate.data , ins : route, hook,index , registry : loadingTemplate.registry})
						...
						if(routeInstance.outlet = outlet = getOutlet(renderTemplate.outlet,routeInstance.parent)) {
							...
							if(renderTemplate.component) {
								if(routeInstance.component && !renderTemplate.reRender && (routeInstance.component.localName == renderTemplate.component._compName) && routeInstance.outlet == outlet && outlet.contains(routeInstance.component)) {
									...
									routeInstance.component = renderComp(outlet, renderTemplate.component,{data, ins : routeInstance , hook,index , registry : renderTemplate.registry})
								}
							...
							_consoleError.call(lyte,428,renderTemplate.outlet);
						}
					} else {
		...
		function componentDataCheck(data,routeInstance,hook,index) {
			if(data) {
				if(typeof data != "object" && Array.isArray(data)) {
					processError.call(this,{stopTrans : true, err :Error(_getError(203)).stack,instance :routeInstance,hook : hook,index : index});
				}   
			}
		}
		...
		function renderComp(outlet, component, {data, ins, hook,index, registry }) {
			...
			// compIns._route = ins.__lp.objPath;
			return compIns;
		}
		...
		this.init = function(r) {
			...
			this.history = lHistory = new History({
			...
			if(matched) {
				...
				if(!sticky && (matchedCache = cache[Array.isArray(matched.route) ? matched.route.join('.') : matched.route])) {
					...
					if(l = matched.dynamicParams && matched.dynamicParams.length) {
						...
						_consoleError.call(lyte,499,matched.route,linkTo ? this : undefined);
					}
		...
		function routeTransition(args, replace ) {
			...
			if(processed.error) {
				if(trans.running) {
					trans.pause();
					run[onErrorStr].call(trans,trans.prom.hook,trans.prom.index,processed.error,420,{ consoled : true});
				}
				return;
			}
			if(replace) {
			...
			if(matched && matched.route) {
				...
				for(var i = 0, l = matched.route.length; i < l; i++) {
					...
					if(!(routeObj && routeObj.__lp && routeObj.__lp.path)) {
						_consoleError(422,matched.route,i);
						return false;
					}
					...
					if(routeObj.__lp.dkey) {
						...
						_consoleError.call(lyte,499,route,linkTo ? this : undefined);
						return false;   
						} else {
			...
			if(callback) { 
				...
				return new Promise(function(resolve,reject) {
					try {
						...
						if(stopTrans && result) { /* check why stopTrans is needed? */
					...
					resp.then(function(data) {
						...
						processError.call(self,{hook, index,stopTrans,err,instance, PR : true});
					});  
				});
			} else {
				return resolvedPromise;
		...
		function callHookWithoutPromise(callback,hook,index,args,splInstance) {
			...
			if(callback) {
				...
				try {
					...
					processError.call(this,{hook, index, stopTrans : errorStoppableHook(hook),err,instance});
					return;
				}  
			}
		}
		...
		function _consoleErrorFromCallback(err,hook,routeName,state,options) {
			if(typeof err == "string" || (typeof err == "object" && err.stack && !err.$)) {
				...
				err.$ = true;
				var internalErr = _getError(state || 420,hook,routeName);
				err.stack = err.stack.replace(err.message,err.message = err.message+"\n\t"+internalErr);
				_consoleError.call(lyte,err);
			} else {
				_consoleError.call(lyte,state,hook,routeName,err,options && options.PR);
			}
		}
		...
		function traverse(path, get) {
			if(path) {
				...
				if(routeObj) {
					return pathProcessor(get,...routeObj);
				} else {
					_consoleError.call(lyte,400,config.baseURL && path.indexOf(config.baseURL) != 0 ? '' : path);
					return { error : true }
				}
			}
		}
		...
		function normalizeMatchedObj(obj) {
			...
			if(obj.route) {
				...
				errorCheck = function(route,i) {
					...
					if(!routesObj) {
						throw Error(_getError(400,matched.target));
					}
				...
				if(this.tagName == LINKTOStr) {
					_consoleError.call(lyte,498, pRoute ,this.outerHTML);
				} else {
					_consoleError.call(lyte,499);  
				}
			}
		}
		...
		function initRoute(trans, processed, from) {
			...
			function promFunc({routeStringArr, i, routeObj}, res) {
				routeObj.__lp.handler().then(function(data) {
					...
					lyte.scopedInstance(RouteClass,[{ i , options : routeOptions, init : pushRoute.bind(this,i,routeObj)}],emptyFn,[LR]);
					res()
				}.bind(routeObj));
			};
			...
			for(var i = from || 0, route; route = matched.route[i]; i++) {
				...
				if(refMatch && similarRoute && refMatch.matched && refMatch.matched.route[i] == route) {
					modifyInstance({route : refMatch.routes[i],i,processed,trans : newTrans});
		...
		function modifyInstance({route, i, processed, trans }) {
			var rOpts = trans.rOpts[i] = deepCopyObject(LR.__lp.nav.get(route).rOpts[i])
			LR.__lp.nav.set(route, trans);
			...
			if(processed.transComp && !processed.transComp.rendered[i] && processed.transComp.redirected) {
				if(processed.transComp.redirected.index < i) {
					...
					route.$.beforeFetch = route.$.fetch = route.$.afterFetch;
		...
		var allHooks = [getRequirementsStr,beforeFetchStr,fetchStr,afterFetchStr,divertStr,renderStr,afterRenderStr];
