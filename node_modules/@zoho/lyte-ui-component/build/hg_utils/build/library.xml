...
<if>	<or>
	...
	<contains casesensitive="false" substring="https://build.zohocorp.com/tp/components/" string = "${co_wget}" />
	<contains casesensitive="false" substring="https://build.zohocorp.com/tp_pkg/" string = "${co_wget}" />
	</or>
        <then>
...
<if>
 ...
 <then>
	<if>
		<available file="${co_dir}/temp_comp/components/.hg" type="dir"/>
	     <then>
	 	    <var name="cvsroot" value="https://cmsuite.csez.zohocorpin.com/integ/components"/>
		    <echo message="Directory ${hgclonedir} exists" />
		    <exec dir="${hgclonedir}/temp_comp" executable="hg" failonerror="true">
                          <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com pull -u -r${co_lbl}; hg upd -C ${co_lbl}"/>
        	    </exec>
             </then>
	     <else>
	 	   <var name="cvsroot" value="https://cmsuite.csez.zohocorpin.com/integ/components"/>
		...
		<mkdir dir="${codir}/temp_comp"/>

	   <exec dir="${codir}/temp_comp" executable="hg" failonerror="true">
        	       <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com clone -r${co_lbl} https://cmsuite.csez.zohocorpin.com/integ/components"/>
		   </exec>
		   <loadfile property="hgrcfile" srcfile="${codir}/temp_comp/components/.hg/hgrc" >
                        <filterchain>
                                <striplinecomments>
                                        <comment value="[paths]" />
                                </striplinecomments>
	...
	<exec dir="${codir}/temp_comp/components/" executable="hg" failonerror="true">
	...
	<copy todir="${codir}" overwrite="true" includeEmptyDirs="false">
		<fileset dir="${codir}/temp_comp/components/.hg/hgrc" includes="**" excludes=""/>
	</copy>
...
<if>
        <not> <available file="${build_dir}/utility_packages" type="dir"/> </not>
        <then>
...
<target name="zrepohgclone" depends="init">

<property name="zrepohgcout" value="zrepohgclone"/>
<record name="${basedir}/buildlogs/${zrepohgcout}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${zrepohgcout}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${zrepohgcout}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="zrepohg_cloneurl" from="${val}_zrepohgclone_url"  silent="false"/>
<propertycopy name="zrepohgcolabel" from="${val}_zrepohgco_label" silent="false"/>

<echo message="zrepohg_clone_url : ${zrepohg_cloneurl}"/>
<echo message="zrpohg_checkout_label : ${zrepohgcolabel}"/>

<basename property="clonemodname" file="${zrepohg_cloneurl}"/>

<exec dir="${basedir}" executable="hg" failonerror="true">
    <arg line="clone -b ${zrepohgcolabel}  ${zrepohg_cloneurl}"/>
</exec>

<exec dir="${basedir}" executable="sh" failonerror="true" output="${basedir}/zrepohgclone.txt" append="true">
     <arg line="githg_changset.sh ${basedir}/${clonemodname} HG ${val} ${zrepohgcolabel}" />
</exec>

<record name="${basedir}/buildlogs/${zrepohgcout}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${zrepohgcout}_${val}_${msec}.log" action="stop"/>
<echo message="${val}_zrepohgco_label=${zrepocohglabel}#${val}_zrepohg_cloneurl=${zrepohg_cloneurl}" file="${basedir}/zrepohgclone.txt" append="true"/>
<echo message="${line.separator}" file="${basedir}/zrepohgclone.txt" append="true"/>
...
<target name="gitclone" depends="init">

<property name="gitcout" value="gitclone"/>
<record name="${basedir}/buildlogs/${gitcout}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${gitcout}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${gitcout}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="git_cloneurl" from="${val}_gitclone_url"  silent="false"/>
<propertycopy name="gitcolabel" from="${val}_gitco_label" silent="false"/>

<echo message="git_clone_url : ${git_cloneurl}"/>
<echo message="git_checkout_label : ${gitcolabel}"/>

<basename property="clonemodname" file="${git_cloneurl}"/>
<propertyregex property="prop_clone" input="${clonemodname}" regexp=".git" replace="" global="true" />

<if>
  <and>
  <equals arg1="${product_name}" arg2="DESKTOPCENTRAL"/>
  <contains  substring="https://zrepository.zoho.com/zohocorp/webmaster/manageengine/" string = "${git_cloneurl}" />
  </and>
<then>
      <echo message="Inside Product is DC and clone Url contains repository.zoho.com"/>
      <exec dir="${basedir}" executable="sh" failonerror="true" output="${basedir}/gitclone_wget.txt" append="true">
            <arg line="download_websiterepo.sh ${prop_clone}" />
      </exec>
</then>
<else>

<exec dir="${basedir}" executable="git" failonerror="true">
        <arg line="clone ${git_cloneurl}"/>
</exec>

<exec dir="${basedir}/${prop_clone}" executable="git" failonerror="true">
        <arg line="checkout -f ${gitcolabel}"/>
</exec>
<echo message="${basedir}/${prop_clone}/" />

<exec dir="${basedir}" executable="sh" failonerror="true" output="${basedir}/gitclone.txt" append="true">
        <arg line="githg_changset.sh ${basedir}/${prop_clone} GIT ${val} ${gitcolabel}" />
                </exec>
</else>
</if>

<record name="${basedir}/buildlogs/${gitcout}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${gitcout}_${val}_${msec}.log" action="stop"/>
...
<if>
        <equals arg1="${co_lbl}" arg2="BASE" />
       <then>
               <echo message="BASE LABEL SO CLONE NOT NEEDED"/>
       </then>
<else>
<if>
      <or>
       ...
       <if>
	       <not>
		       <available file="${hgclonedir}" type="dir"/>
	       </not>
	       <then>
			<mkdir dir="${hgclonedir}" />
	       </then>
       </if>
       <antcall target="unzip">
	       <param name="${val}_unzip_srcdir" value="${build_dir}/zips/"/>
...
<if>
	...
	<available property="value" file="${hgclonedir}/temp_comp/.hg" type="dir"/>
     ...
     <then>
 		...
 		<contains casesensitive="false" substring="_REVIEWED" string = "${co_lbl}" />
                <then>
			     ...
			     <exec dir="${hgclonedir}/temp_comp" executable="hg" failonerror="true">
                                 <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com pull -b tags_branch"/>
                        </exec>
        	</then>
        </if>
	<exec dir="${hgclonedir}/temp_comp" executable="hg" failonerror="true">
              <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com pull -r${co_lbl}"/>
      </exec>

      <exec dir="${hgclonedir}/temp_comp" executable="hg" failonerror="true">
              <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com upd -C ${co_lbl}"/>
      </exec>

     </then>
      <elseif>
	 <not>
		     <contains casesensitive="false" substring="BASE" string = "${hgcolabel}" />
	</not>
       ...
       <then>
           ...
           <exec dir="${basedir}" executable="hg" failonerror="true">
		   <arg line="--config http_proxy.no=cmsuite.csez.zohocorpin.com clone -r${co_lbl} ${hgroot} ${hgclonedir}/temp_comp"/>
           </exec>
	   <loadfile property="hgrcfile" srcfile="${hgclonedir}/temp_comp/.hg/hgrc" >
			<filterchain>
				<striplinecomments>
					<comment value="[paths]" />
				</striplinecomments>
...
<if>
	<not> <available file="${codir}" type="dir"/> </not>
	<then>
	<mkdir dir="${codir}"/>
        </then>
...
<copy todir="${hgclonedir}" overwrite="true">
	<fileset dir="${hgclonedir}/temp_comp">
         <exclude name=""/>
        </fileset>
</copy>
...
<target name="geturl" depends="init, wget_credentials">
...
<loadfile property="message" srcFile="${basedir}/allowed_dependency_urls.txt"></loadfile>
<echo message="${url_src}"/>
<propertyregex property="http_url" input="${url_src}" regexp="(http://[a-zA-Z0-9.-]*/)" select="\1" casesensitive="true"/>
<propertyregex property="https_url" input="${url_src}" regexp="(https://[a-zA-Z0-9.-]*/)" select="\1" casesensitive="true"/>

<if>
        <not>
        <or>
        <contains string="${message}" substring="${http_url}"/>
        <contains string="${message}" substring="${https_url}"/>
        </or>
        </not>
        <then>
                <echo  message="Not a valid download Url"/>
...
<if>
 ...
 <then>
	<if>
		...
		<then>
			<propertyregex property="ceph_url" input="${url_src}" regexp="((.*http|http[s]):\/\/(build|build.csez.zohocorpin.com|build.zohocorp.com))\/.*" select="\1" casesensitive="true"/>
			<if>
				<isset property="ceph_url"/>
				<then>
					<property name="regexp" value="${ceph_url}" />
					<propertyregex property="dependency_dir" input="${url_src}" regexp="${regexp}" replace="" casesensitive="true"/>
					...
					<property name="dependency_path" value="/zoho/build/downloads/dload/${dependency_dir}"/>
						...
						<and>
                                                ...
                                                <not>
                                                  <equals arg1="${started_by}" arg2="kaleeswaran.n" />
                                                </not>
                                                </and>
						<then>
							<if>
							...
							<if>
							 ...
							 <else>
								...
								<if>
								    <not>
								    <and>
								     <isset property="${url_username}" />
								     <isset property="${url_password}" />
								    </and>
								    </not>
								    <then>
									<property name="url_username" value="${wurl_username}" />
									<property name="url_password" value="${wurl_password}" />
								    </then>
								</if>
                                                         <if>
                                                                <not>
...
<get src="${url_src}" dest="${url_dest}" verbose="true" usetimestamp="true" ignoreerrors="true" username="${url_username}" password="${url_password}" />
        <!--get src="${url_src}"
        dest="${url_dest}"
        verbose="true"
        usetimestamp="true"
        ignoreerrors="true"
        username="${wurl_username}"
        password="${wurl_password}"/-->
		</else>
	</if>
			...
			<if>
				...
				<then>
					...
					<if>
						...
						<then>
							<get src="${modified_url_src}" dest="${url_dest}" verbose="true" usetimestamp="false" ignoreerrors="true" username="${wurl_username}" password="${wurl_password}" maxtime="600" />
					...
					<else>
					<get src="${modified_url_src}" dest="${url_dest}" verbose="true" usetimestamp="false" username="${wurl_username}" password="${wurl_password}"/>
					<propertycopy name="url_src" from="modified_url_src" silent="false" override="true"/>
					</else>
					</if>
				</then>
...
<propertycopy name="compile_verbose" from="${val}_verbose_needed" silent="true" />
...
<if>
       <isset property="compile_verbose"/>
       <then>
               <property name="verbose_status" value="${compile_verbose}"/>
       </then>
       <else>
               <property name="verbose_status" value="no"/>
       </else>
</if>
...
<target name="getcacerts" depends="init">

<property name="getcacertsdep" value="getcacerts"/>
<record name="${basedir}/buildlogs/${getcacertsdep}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${getcacertsdep}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${getcacertsdep}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<if>
        <not>
                <isset property="cacerts_url"/>
        </not>
        <then>
                <mkdir dir="${build_dir}/CACERTS/"/>
                <antcall target="exec">
                        <param name="${val}_exec_dir" value="${basedir}"/>
                        <param name="${val}_exec_command" value="wget"/>
                        <param name="${val}_exec_args" value="https://us4-appsense.zohosecurity.com/ts/api?action=downloadlatestzohocacerts --no-check-certificate -O ${build_dir}/CACERTS/cacerts"/>
                </antcall>
...
<record name="${basedir}/buildlogs/${getcacertsdep}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${getcacertsdep}_${msec}.log" action="stop"/>
</target>
...
<propertycopy name="jarmanifestupdate" from="${val}_jar_info_update" silent="true"/>
<if>
<not>
...
<echo message="JarUpdate:-${jarupdate}" />
<echo message="JarManifesetUpdate:-${jarmanifestupdate}" />

<basename property="zipbasename" file="${destfile}"/>
<if>
    <or>
        <equals arg1="${zipbasename}" arg2="Root.war"/>
        <equals arg1="${zipbasename}" arg2="root.war"/>
        <equals arg1="${zipbasename}" arg2="ROOT.war"/>
     </or>
     <then>
         <echo message="ROOT.war creation, hence copying version.txt and ${pdt_name_from_conf}.txt to ${jarsrcdir}/WEB_INF/blog/  "/>
         <copy todir="${jarsrcdir}/WEB-INF/blog" overwrite="true">
    <fileset dir="${basedir}">
        <include name="version.txt"/>
        <include name="${pdt_name_from_conf}.txt"/>
    </fileset>
</copy>
     </then>
</if>
...
<if><isset property="jarmanifest"/>
        <then>
                                ...
                                <echo message="${destfile}:updated${line.separator}" file="${basedir}/buildtime_updated_jars.txt" append="true"/>
        ...
        <else>
                                <if><equals arg1="${jarupdate}" arg2="true"/>
                                        ...
                                        <else>
                                                    ...
                                                    <echo message="${destfile}:created${line.separator}" file="${basedir}/buildtime_created_jars.txt" append="true"/>
                                        </else>
                                </if>
        </else>
</if>
...
<if>
        <equals arg1="${jarmanifestupdate}" arg2="true" />
        <then>
            <if>
               <and>
                <available file="${basedir}/manifest_info.txt" />
                <available file="${basedir}/manifest_update/${jar_outputname}.txt" />
               </and>
               <then>
                   <concat destfile="${basedir}/manifest_info_${jar_outputname}.txt" append="true">
                       <filelist dir="${basedir}/manifest_update" files="${jar_outputname}.txt" />
                       <filelist dir="${basedir}/" files="manifest_info.txt" /> 
                   </concat>
               </then>
            </if>
        </then>
    ...
    <if>
        <equals arg1="${jarmanifestupdate}" arg2="true"/>
        <then>
            <property name="temp_file" value="manifest_info_${jar_outputname}.txt" />
        </then>
        <else>
            <property name="temp_file" value="manifestinfo_temp.txt" />   
        </else>
</if>
<if>
	<not>
	...
	<then>
			...
			<then>
				<if>
                                        <or>
                                        <not>
                                                <available file="${basedir}/${temp_file}" type="file"/>
                                        </not>
                                        <equals arg1="${jarmanifestupdate}" arg2="true"/>	
                                        </or>
                                        <then>
                                                <if>
                                                <equals arg1="${jarmanifestupdate}" arg2="false"/>
                                                <then>
                                                <copy file="${basedir}/manifest_info.txt" tofile="${basedir}/${temp_file}" overwrite="true"/>
                                                </then>
                                                </if>
                                                 <if>
                                                <isset property="build_lbl_static"/>
                                                <then>
                                                        <echo message="Implementation-Version: ${build_lbl_static}${line.separator}" file="${basedir}/manifestinfo_temp.txt" append="true"/>
                                                        <echo message="Build_Label: ${build_lbl}${line.separator}" file="${basedir}/${temp_file}" append="true"/>
                                                  </then>
                                                  <else>
                                                    <echo message="Implementation-Version: ${build_lbl}${line.separator}" file="${basedir}/${temp_file}" append="true"/>
                                                </else>
                                                </if>
                                        </then>
                                        </if>
                                        <jar destfile="${destfile}" basedir="${jarsrcdir}" includes="${jarincludes}" excludes="${jarexcludes}" update="true" manifest="${basedir}/${temp_file}"/>
                ...
                <if>
					...
					<copy file="${basedir}/manifest_info.txt" tofile="${basedir}/manifestinfo_temp.txt" overwrite="true"/>
					 <if>
                                        ...
                                        <then>
                                                <echo message="Implementation-Version: ${build_lbl_static}${line.separator}" file="${basedir}/manifestinfo_temp.txt" append="true"/>
                                                <echo message="Build_Label: ${build_lbl}${line.separator}" file="${basedir}/manifestinfo_temp.txt" append="true"/>
                                          </then>
                                          <else>
                                ...
                                <jar jarfile="${dest_merge_file}" manifest="${basedir}/manifestinfo_temp.txt" duplicate="preserve">
                                        <zipgroupfileset dir="${mergejarsrcdir}" includes="${mergejarincludes}" excludes="${mergejarexcludes}"/>
                                </jar>
                        </then>
                        <else>
                                 <jar jarfile="${dest_merge_file}" duplicate="preserve">
                                        <zipgroupfileset dir="${mergejarsrcdir}" includes="${mergejarincludes}" excludes="${mergejarexcludes}"/>
                                </jar>
                        </else>
                </if>
...
<if>
	...
	<then>
		<replaceregexp match="${tok}" replace="${value}" byline="${byline}">
			<fileset dir="${dir}"
...
<propertycopy name="retry_option" from="${val}_exec_retry_needed" silent="true"/>
<propertyregex property="argument" input="${arguments}" regexp="http://integ-docker:4873" replace="http://cm-npmregistry" global="true" />
<if>
    <equals arg1="${cmd}" arg2="sh"/>
    <then>
            <echo file="${basedir}/buildlogs/script.txt" append="true">script:${arguments}</echo>
    </then>
    <else>
            <echo file="${basedir}/buildlogs/package.txt" append="true">${cmd}:</echo>
            <exec dir="${execdir}" executable="${cmd}" failonerror="false" append="true" output="${basedir}/buildlogs/package.txt"><arg line="--version"/></exec>
            <if>
                <equals arg1="${cmd}" arg2="npm"/>
            <then>

                  <echo file="${basedir}/buildlogs/package.txt" append="true">node:</echo>
                  <exec dir="${execdir}" executable="node" failonerror="false" append="true" output="${basedir}/buildlogs/package.txt"><arg line="--version"/></exec>
            </then>
            </if>
    </else>
</if>
...
<if>
         <and>
               <equals arg1="${exe_bin_skip_needed}" arg2="yes" />
               <contains  substring=" pack " string = "${arguments}" />
         </and>
<then>
         <record name="${basedir}/buildlogs/${exc}_${val}_${msec}_exe_exectask_skipped.log" action="start" loglevel="warn"/>
         <echo  message="winexe exec task skipped "/>
...
<if>
    ...
    <elseif>
            <and>
                    <isset property="retry_option"/>
                    <equals arg1="${retry_option}" arg2="yes"/>
            </and>
            <then>
                    ...
                    <for param="callexec" list="1,2">
                            <sequential>
                                    <if>
                                            ...
                                            <then>
                                                    <echo message="retry[@{callexec}] ... sleep 10 seconds.."/>
                                                    <var name="exitValue" unset="true"/>
                                                    <sleep seconds="10"/>
                                                    <exec dir="${execdir}" executable="${cmd}" failonerror="${failonerror}" resultproperty="exitValue">
                                                            <arg line="${arguments}"/>
                                                    </exec>
    ...
    <else>
	     <!--if>
            <equals arg1="${cmd}" arg2="bower"/>
            <then>
                    <antcallback target="bower">
                            <param name="${val}_json_dir" value="${execdir}/" />
                    </antcallback>
            </then>
            <else-->
              <exec dir="${execdir}" executable="${cmd}" failonerror="${failonerror}" resultproperty="exitValue">
...
<propertycopy name="zip_exclude" from="${val}_zip_exclude" silent="true"/>

<basename property="zipbasename" file="${zipfile}"/>
...
<if>
    <or>
     ...
     <then>
         <echo message="ROOT.war creation, hence copying version.txt and ${pdt_name_from_conf}.txt to ${exec_dir}/WEB_INF/blog/  "/>
         <copy todir="${exec_dir}/WEB-INF/blog" overwrite="true">
    <fileset dir="${basedir}">
        <include name="version.txt"/>
        <include name="${pdt_name_from_conf}.txt"/>
    </fileset>
...
<propertycopy name="javajvmargs"    from="${val}_java_jvmargs" silent="true"/>
<propertycopy name="javafailonerror"    from="${val}_java_failonerror" silent="true"/>
	 ...
	 <elseif>
                 ...
                 <then>
                          <java classpath="${javaclp}"
                                   ...
                                   <sysproperty key="org.apache.jasper.compiler.Parser.STRICT_QUOTE_ESCAPING" value="false"/>
				   <sysproperty key="org.apache.jasper.compiler.Parser.STRICT_WHITESPACE" value="false"/>
				   <jvmarg value="${javajvmargs}"/>
                                   <arg line="${args}"/>
                           </java>
                 </then>
         </elseif>
	<elseif>
                 <isset property="javafailonerror"/>
                 <then>
                          <java classpath="${javaclp}"
                                   fork="yes"
                                   classname="${javacls}"
...
<if>
        <isset property="clpath"/>
        <then>
        <if>
    <contains string="${clpath}" substring=":" />
    <then>
        <for param="classpathval" list="${clpath}" delimiter=":">
            <sequential>
                <if>
                <available file="@{classpathval}" type="file"/>
                <then>
                    <var name="clpath" value="${clpath}:@{classpathval}"/>
                </then>
                <else>
        <if><not>
        <available file="@{classpathval}" type="dir" />
        ...
        <else>
             <for param="dirjarfiles">
                     ...
                     <sequential>
                             <var name="clpath" value="${clpath}:@{dirjarfiles}" />
                     </sequential>
                     </for>
        </else></if>
                </else>
  ...
  <else>
        <if>
                <available file="${clpath}" type="file"/>
                        <then>
                                <var name="clpath" value="${clpath}:${clpath}"/>
                        </then>
                        <else>
                <if><not>
                    <available file="${clpath}" type="dir" />
                </not>
                <then>
                <var name="clpath" value="${clpath}"/>
                </then>
                ...
                <sequential>
                    <var name="clpath" value="${clpath}:@{jarfiles}" />
                </sequential>
                </for>
                </else>
                </if>
...
<if>
        <isset property="clpath"/>
        <then>
                <property name="clpath_javadoc" value="${clpath}"/>
        </then>
...
<target name="signjar" depends="init,signtask_prop">
...
<propertycopy name="keyalias" from="${val}_jar_alias" silent="true"/>
<propertycopy name="keystorepass" from="${val}_jar_storepass" silent="true"/>
...
<if>
   ...
   <then>
      <property name="keyfileset" value="no"/>
   </then>
   <else>
      <basename property="pfxname" file="${keyfile}"/>
   </else>
</if>

<property name="aespath" value="${user.home}/integ/aes.jar"/>
<property name="pfxpath" value="${user.home}/integ/pfx/sectigo-cs.pfx"/>
<property name="pfxalias" value="{9DD425EC-409F-45CE-B386-C98A4CAC1692}"/>
<property name="keystorepasswd" value="QQXN5NhuNcG5i6bD2bWmrQ=="/>

<exec dir="${basedir}" executable="/usr/java/jdk1.8.0_112/bin/java" resultproperty="exitValue" outputproperty="outputval" logError="true">
    <arg line="-jar ${aespath} ${keystorepasswd} decrypt"/>
</exec>

<if>
    <or>
    <equals arg1="${pfxname}" arg2="zohomc.pfx"/>
    <equals arg1="${keyfileset}" arg2="no"/>
    </or>
        <then>
                <var name="keystorepass" unset="true"/>
                <property name="keystorepass" value="${outputval}"/>
                <var name="keyfile" unset="true"/>
                <property name="keyfile" value="${pfxpath}"/>
                <echo message="pfx path updated!.."/>
                <var name="keyalias" unset="true"/>
                <property name="keyalias" value="${pfxalias}"/>
                <echo message="aliasname updated: ${keyalias}"/>
                <var name="storetype" unset="true"/>
                <property name="storetype" value="pkcs12"/>
                <!--var name="timestamp" unset="true"/>
                <property name="timestamp" value="yes"/-->
        </then>
</if>
...
<!-- New signing changes added started here -->
    <echo message="FilePath: ${jarfile}"/>
        <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
                <sysproperty key="prod.home" value="${fltransfer_prod_home}" />
                <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/" />
                <jvmarg value="-Dsun.zip.disableMemoryMapping=true" />
                <arg line="${jarfile} ${jarfile} ${basedir} ${source_basedir}/../logs/ ${product_name} ${branch_name} ${build_id} linux --individual-file-signing --specific-host-connect"/>
        </java>
</else>
</if>
<!-- New signing change condition ends here -->
...
<target name="getKeyValue" depends="init">
	...
	<if>
		<isset property="jsoncreation"/>
		<then>
		<exec dir="${basedir}/hg_utils/build/" executable="python" failonerror="true">
                	<arg line="createi18njs.py ${prop}_NEW ${propfile_key}"/>
	                <redirector outputproperty="depdet" errorproperty="depdeterr"></redirector>
	         </exec>
	        <echo message="${depdet}" file="${i18n_destdir}/${getjsfile}.js" append="true"/>
		</then>
	<else>
	<if>
                <not>
...
<target name="createi18njs" depends="init">
	...
	<propertycopy name="jsoncreation" from="${val}_i18n_jsoncreation" silent="true"/>
	<if>
		<isset property="combinedi18njs"/>
		<then>
...
<!-- ==================================================================
                  ...
                  I18N ANDROID iOS STRINGS XML CREATION
============================================================================= -->

<target name="createi18nstringsprop" depends="init">
        <property name="createiosstrings" value="createi18nstringsprop"/>
        <record name="${basedir}/buildlogs/${createiosstrings}_${val}_${msec}.log" action="start" loglevel="info"/>
        <record name="${basedir}/buildlogs/${createiosstrings}_${val}_${msec}_err.log" action="start" loglevel="error"/>
        <record name="${basedir}/buildlogs/${createiosstrings}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
        <propertycopy name="i18n_filedestdir" from="${val}_filei18n_destdir" silent="false"/>
        <propertycopy name="prop_filesrcdir" from="${val}_fileprop_srcdir" silent="false"/>
        <propertycopy name="propfilename" from="${val}_propfilename" silent="false"/>
        <propertycopy name="getiosfilename" from="${val}_prop_iosfilename" silent="true"/>
        <propertycopy name="getandroidfilename" from="${val}_prop_androidfilename" silent="true"/>

                 <property name="getiosfile" value="${getiosfilename}"/>
                 <property name="getandroidfile" value="${getandroidfilename}"/>
	         <loadfile property="propcontent" srcFile="${prop_filesrcdir}/${propfilename}">
                     <filterchain>
                              <striplinecomments>
                                    <comment value="#"/>
                               </striplinecomments>
		 ...
		 <if>
                         <isset property="getandroidfilename"/>
                     <then>
                                 <echo message="&lt;resources xmlns:tools=&quot;http://schemas.android.com/tools&quot;&gt;" file="${i18n_filedestdir}/${getandroidfile}" append="true"/>
                     </then>
                 </if>
      ...
      <sequential>
                 <if>
                         <isset property="getiosfilename"/>
                     <then>
                                 <echo message="@{line};${line.separator}" file="${i18n_filedestdir}/${getiosfile}" append="true"/>
                     </then>
                 </if>
                 <if>
                         <isset property="getandroidfilename"/>
                      <then>
                                 <propertyregex property="keys" input="@{line}" regexp="([^=\s]*)\s*=" select="\1" casesensitive="true" override="true" global="false"/>
                                 <propertyregex property="values" input="@{line}" regexp="([^=]*)=\s*&quot;([^&quot;]*)" select="\2" casesensitive="true" override="true" global="false"/>
                                 <echo message="${line.separator}&lt;string name=${keys}&gt;${values}&lt;/string&gt;" file="${i18n_filedestdir}/${getandroidfile}" append="true"/>
                      </then>
                 </if>
      </sequential>
      </for>
                 <if>
                         <isset property="getandroidfilename"/>
                      <then>
                                 <echo message="${line.separator}&lt;/resources&gt;" file="${i18n_filedestdir}/${getandroidfile}" append="true"/>
                      </then>
                 </if>
</target>
...
<if>
         <equals arg1="${exe_bin_skip_needed}" arg2="yes" />
<then>
        <record name="${basedir}/buildlogs/${etask}_${val}_${msec}_exetask_skipped.log" action="start" loglevel="warn" append="yes"/>
        <echo  message="exetask skipped "/>
...
<if>
       <equals arg1="${winexetask_needed}" arg2="yes" />
       <then>
	       <echo  message="winexetask calling "/>

	       <propertyregex property="exetaskmachine"  input="${exeargs}" regexp="([^\s]+)" select="\1" casesensitive="true" override="true"/>
               <propertyregex property="exetaskport"  input="${exeargs}" regexp="([^\s]+)\s([^\s]+)" select="\2" casesensitive="true" override="true"/>
               <propertyregex property="exetaskinputzip"  input="${exeargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)" select="\3" casesensitive="true" override="true"/>
               <propertyregex property="exetaskopzip"  input="${exeargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\4" casesensitive="true" override="true"/>
               <propertyregex property="exetaskbit"  input="${exeargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\5" casesensitive="true" override="true"/>
	       <propertyregex property="exetaskprdname"  input="${exeargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\6" casesensitive="true" override="true"/>
	       <propertyregex property="exeendoptions"  input="${exeargs}" regexp="(?:([^\s]*\s)){6}(.*)" select="\2"  casesensitive="true" override="true"/>


                <if>
                        <contains  substring="_64bit" string = "${exetaskbit}" />
                <then>
			<property name="allexeargs_val" value="${exetaskmachine} ${exetaskinputzip} ${exetaskopzip} ${exetaskbit} ${exetaskprdname} --platform=x64 ${exeendoptions}"/>
                </then>
                <else>
			<property name="allexeargs_val" value="${exetaskmachine} ${exetaskinputzip} ${exetaskopzip} ${exetaskbit} ${exetaskprdname} ${exeendoptions}"/>
                </else>
		</if>

		<if>
               		<isset property="winexe_flags"/>
               <then>
		       <property name="allexeargs" value="${allexeargs_val} ${winexe_flags}"/>

		</then>
		<else>
			<property name="allexeargs" value="${allexeargs_val}"/>

		</else>
		</if>

		<echo  message="${allexeargs}"/>


		<if>
			<isset property="winhome"/>
			<then>
                                <propertycopy name="is_replace_proj_dir" from="${val}_dir_tozip" silent="true"/>
                                <if>
                                <isset property="is_replace_proj_dir"/>
                                <then>
                                <antcall target="exec">
                                      <param name="val" value="${val}_replace"/>
                                      <param name="${val}_replace_exec_dir" value="${basedir}"/>
                                      <param name="${val}_replace_exec_command" value="sh"/>
                                      <param name="${val}_replace_exec_args" value="-x ${build_dir}/utility_packages/exe/rce_installshield_cmd.sh ${is_replace_proj_dir}"/>
                                </antcall>
                                </then>
                                </if>
                              ...
                              <antcallback target="ziptask">
                                ...
                                <param name="${val}_zip_name" value="${exetaskinputzip}"/>
               ...
               <antcall target="winexetask">
                  <param name="val" value="${val}"/>
                  <param name="${val}_winexetask" value="${allexeargs}"/>
               </antcall>
...
<else>
	...
	<if>
        ...
        <if>
                ...
                <then>
                        <propertyregex property="exeargs_mod" input="${exeargs}" regexp="${exe_argument}" replace="integ-exe8" casesensitive="true"/>
                </then>
        </if>
        <if>
...
<if>
        ...
        <then>
                <antcall target="geturl">
                        <param name="${val}_url" value="https://build.zohocorp.com/me/did_injection_utils/milestones/stable/DIDInjectionUtils.zip"/>
...
<!-- ======================================================================
	...
	<else>
    <echo message="${ordername} : ${callorder} ;" file="${basedir}/buildlogs/build_time_new.txt" append="yes"/>
    <echo message="" file="${basedir}/buildlogs/build_time_new.txt" append="yes"/>
     			...
     			<if>
					...
					<then>
        				...
        				<sequential>
							...
							<param name="${val}_url" value="http://cm-server/thirdparty_backup/@{file}"/>
...
<target name="sec_validations" depends="init">
	...
	<antcall target="security_check"/>
       ...
       <if>
          <and>
                  <isset property="commerce_filtercheck"/>
                  <equals arg1="${commerce_filtercheck}" arg2="yes"/>
                  <equals arg1="${product_name}" arg2="ZOHOPAGES"/>
          </and>
            <then>
                <property name="securityfilter_jar_name" value="${basedir}/XMLReader_pages.jar"/>
                <property name="securityfilter_script" value="${basedir}/security_build_check_zohopages.sh"/>
            </then>
            <else>
                <property name="securityfilter_jar_name" value="${basedir}/XMLReader.jar"/>
                <property name="securityfilter_script" value="${basedir}/security_build_check.sh"/>
            </else>
       </if>
...
<target name="security_check" depends="init">
	<antcall target="exec">
		...
		<param name="${val}_exec_args" value="-x ${securityfilter_script} 2 ${securityfilter_jar_name} ${filelist}"/>
	</antcall>
...
<target name="extract_depfiles" depends="init">
	...
	<if>
		...
		<then>
			<propertyregex property="s_url" input="${dep_zip}" regexp="(.*)/(.*)" select="\1" casesensitive="true" override="true"/>
                        ...
                        <propertyregex property="sas_dependency_dir" input="${s_url}" regexp="https://build.zohocorp.com/" replace="" casesensitive="true"/>
	...
	<!-- To avoid the ZComponents npm install issue, done the below changes...-->
	<propertyregex property="zcomp_match" input="${dep_zip}" regexp="(.*)/(.*zoho-components.zip.*)" select="\2" casesensitive="true" override="true"/>
	<if>
	    <isset property="zcomp_match" />
	    <then>
		<echo message="${zcomp_match}" />
		<propertyregex property="zc_url" input="${dep_zip}" regexp="(.*)/(.*)" select="\1" casesensitive="true"
		    override="true" />
		<propertyregex property="zc_dependency_dir" input="${zc_url}" regexp="http://build/" replace=""
		    casesensitive="true" />
		<propertyregex property="zc_dependency_dir" input="${zc_url}" regexp="https://build.zohocorp.com/" replace="" casesensitive="true" />
		<property name="zc_dependency_path" value="/zoho/build/downloads/dload/${zc_dependency_dir}/node_modules.zip" />
		<if>
		    <available file="${zc_dependency_path}" type="file" />
		    <then>
			<exec dir="${prop_dep_destdir}/${dep_param}/build/" executable="unzip" failonerror="true">
			    <arg value="-o" />
			    <arg value="/zoho/build/downloads/dload/${zc_dependency_dir}/node_modules.zip"/>
			</exec>
			<replaceregexp file="${prop_dep_destdir}/${dep_param}/build/zcomponents-merge.sh" match="npm install" replace="#npm install"/>
			<chmod dir="${prop_dep_destdir}/${dep_param}/build/node_modules" perm="755" includes="*" excludes=""/>
		    </then>
		</if>
	    </then>
	</if>
...
<target name="gridwarupd" depends="init">
	<property name="gridupd" value="gridwarupd"/>
        <echo message="As per sas team confirmation gridwarupd task invocation removed."/>
	...
	<antcall target="replacefile">
		...
		<param name="${val}_tok" value="&lt; SECURITY FILTER START WILL BE UNCOMMENTED BY BUILD SCRIPT &gt;"/>
		<param name="${val}_value" value=""/>
	</antcall>
	<antcall target="replacefile">
		<param name="${val}_file" value="${build_dir}/GRID_WAR/WEB-INF/web.xml"/>
		<param name="${val}_tok" value="&lt; SECURITY FILTER END WILL BE UNCOMMENTED BY BUILD SCRIPT &gt;"/>
		<param name="${val}_value" value=""/>
	</antcall>
	<antcall target="genjar">
...
<target name="srccompile" depends="init">
	...
	<propertycopy name="src_compilerarg_jdk_version" from="${val}_srccompile_compilerarg_${srccompile_target}" silent="true"/>
        <propertycopy name="src_compilerarg_jar_manifestupd" from="${val}_srccompile_manifestjarupd" silent="true"/>

	<!--  Maven Format Jar Creation Input start -->
	<propertycopy name="mvn_groupid" from="${val}_maven_groupid" silent="true"/>
	<propertycopy name="mvn_artifactid" from="${val}_maven_artifactid" silent="true"/>
	<propertycopy name="mvn_version" from="${val}_maven_artifact_version" silent="true"/>
	<!--  Maven Format Jar Creation Input end -->
	...
	<if>
        ...
        <echo message="Compilerarg set in srccompile task is -- ${src_compilerarg}"/>
	<if>
                <and>
			...
			<if>
				...
				<then>
					...
					<echo message="converting ${cjar} to javac source and target compilation is finished"/>
                                ...
                                <elseif>
				<isset property="src_compilerarg_jdk_version" />
				<then>
				<echo message="src_compilerarg_jdk_version:: ${src_compilerarg_jdk_version}"/>
						<antcall target="compilesrc">
							<param name="val" value="${val}"/>
							<param name="${val}_classpath" value="${compileclp}"/>
							<param name="${val}_classes_dir" value="${src_classesdir}"/>
							...
							<param name="${val}_javac_sourceversion" value="${srccompile_source}"/>
							<param name="${val}_javac_targetversion" value="${srccompile_target}"/>
							<param name="${val}_javac_compilerarg" value="${src_compilerarg_jdk_version}"/>
						</antcall>
						<echo message="converting ${cjar} to javac source and target compilation is finished"/>
				</then>
				</elseif>
	...
	<if>
        ...
        <if>
            <isset property="src_compilerarg_jar_manifestupd" />
            <then>
                <property name="jar_info_update" value="true" />
	</then>
            <else>
                <property name="jar_info_update" value="false" />
	</else>
	</if>
	<antcall target="genjar">
		<param name="val" value="${val}"/>
		...
		<param name="${val}_jar_update" value="${jarupd}"/>
                <param name="${val}_jar_info_update" value="${jar_info_update}"/>
	</antcall>
	<echo message="converting ${sjc} to srcgenjar task is finished"/>

        <if>
		<and>
		<isset property="mvn_groupid"/>
		<isset property="mvn_artifactid"/>
		<isset property="mvn_version"/>
                <length string="${mvn_groupid}" trim="true" when="greater" length="0"/>
                <length string="${mvn_artifactid}" trim="true" when="greater" length="0"/>
                <length string="${mvn_version}" trim="true" when="greater" length="0"/>
		</and>
	<then>
	<echo message="Maven format jar package started for ${compilejar}"/>
        <antcall target="mavenpkg">
		<param name="val" value="${val}"/>
		<param name="${val}_maven_jar_filepath" value="${compilejar}"/>
                <param name="${val}_maven_jar_groupid" value="${mvn_groupid}"/>
                <param name="${val}_maven_jar_artifactid" value="${mvn_artifactid}"/>
                <param name="${val}_maven_jar_artifact_version" value="${mvn_version}"/>
        </antcall>
	<echo message="Maven format jar package ended for ${compilejar}"/>
	</then>
        </if>
...
<!-- ============================================================================
     Maven Format Archive Generation
=============================================================================== -->

<target name="mavenpkg">
<property name="mvnarc" value="mavenpkg"/>
<record name="${basedir}/buildlogs/${mvnarc}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${mvnarc}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${mvnarc}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<!--  Maven Format Jar Creation Input start -->

<propertycopy name="jarfile_path" from="${val}_maven_jar_filepath" silent="false"/>
<propertycopy name="maven_groupid" from="${val}_maven_jar_groupid" silent="false"/>
<propertycopy name="maven_artifactid" from="${val}_maven_jar_artifactid" silent="false"/>
<propertycopy name="maven_version" from="${val}_maven_jar_artifact_version" silent="false"/>

<echo message="Group-Id: ${maven_groupid}"/>
<echo message="Artifact-Id: ${maven_artifactid}"/>
<echo message="Version: ${maven_version}"/>

<property name="maven_package_dir" value="${basedir}/maven_format_package"/>
<property name="maven_cmd_location" value="/usr/maven/apache-maven-3.9.1/bin/mvn"/>
<property name="mvn_cmd_args" value="install:install-file -Dfile=${jarfile_path} -DgroupId=${maven_groupid} -DartifactId=${maven_artifactid} -Dversion=${maven_version} -Dpackaging=jar -DgeneratePom=true -DlocalRepositoryPath=${maven_package_dir}"/>

<!--  Maven Format Jar Creation Input end -->

<condition property="builddir_status" value="yes">
  <and>
...
<if>
  <and>
    <isset property="builddir_status"/>
    <equals arg1="${builddir_status}" arg2="yes"/>
  </and>
  <then>
    <if>
      <not>
    ...
    <property name="maven_outdir" value="${build_dir}/output"/>
  </then>
  <else>
...
<if>
    <and>
        <isset property="jarfile_path"/>
        <available file="${jarfile_path}" type="file"/>
    </and>
    <then>
           <if>
		   <and>
		       <isset property="maven_groupid"/>
		       <isset property="maven_artifactid"/>
		       <isset property="maven_version"/>
                       <length string="${maven_groupid}" trim="true" when="greater" length="0"/>
                       <length string="${maven_artifactid}" trim="true" when="greater" length="0"/>
                       <length string="${maven_version}" trim="true" when="greater" length="0"/>
                   </and>
	   <then>
                <exec dir="${basedir}" executable="${maven_cmd_location}" failonerror="true" resultproperty="mvnExitValue">
			<arg line="${mvn_cmd_args}"/>
                </exec>
           </then>
           <else>
                   <echo message="Error: Maven GroupID,ArtifactID,Version should not be empty."/>
                   <fail message="Error: Maven GroupID,ArtifactID,Version should not be empty."/>
           </else>
          </if>
    </then>
</if>

<condition property="is_directory_not_empty">
   <resourcecount when="greater" count="0">
      <fileset dir="${maven_package_dir}" />
   </resourcecount>
</condition>

<if>
        <isset property="is_directory_not_empty"/>
        <then>
		<echo message="&#10;[INFO] ${maven_outdir}/maven_archives.zip generation started .."/>
                <exec dir="${maven_package_dir}" executable="zip" failonerror="true">
                        <arg line="-r"/>
			<arg line="${maven_outdir}/maven_archives.zip"/>
                        <arg line="."/>
		</exec>
		<echo message="&#10;[INFO] ${maven_outdir}/maven_archives.zip generated.."/>
        </then>
</if>

<record name="${basedir}/buildlogs/${mvnarc}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${mvnarc}_${val}_${msec}.log" action="stop"/>
</target>
...
<target name="copytask" depends="init">
	...
	<propertycopy name="copytask_empty_dir_opt" from="${val}_copytask_emptydir_needed" silent="true"/>
        <if>
	       <and>
	...
	<if>
        ...
        <if>
                <and>
                        <isset property="copytask_empty_dir_opt"/>
                        <equals arg1="${copytask_empty_dir_opt}" arg2="yes"/>
                </and>
                <then>
                        <antcall target="copy">
...
<target name="jartask" depends="init">
	...
	<propertyregex property="sas_match_patt" input="${trim_val}" regexp="(.*http|http[s]://(build|build.zohocorp.com|build.csez.zohocorpin.com)/framework/saslite.*)" select="\1" casesensitive="true" override="true"/>
	<if>
		<and>
	...
	<if>
                <isset property="jartsk_params"/>
                <then>
                	<propertyregex property="jar_upd_match_new" input="${jartsk_params}" regexp="([^,]+)[,]{1}([^,]+)[,]{1}([^,]+)[,]{1}([^,]*)[,]{1}([^,]+){1}" select="\5" casesensitive="true"/> 
                	<echo message="JarUpdateMatchNew:-${jar_upd_match_new}" />			<if>
				<and>
					<isset property="jar_upd_match_new"/>
					<equals arg1="${jar_upd_match_new}" arg2="true"/>
				</and>
        	                <then>
		       ...
		       <else>
			       <if>
	               	       <contains  substring="true" string = "${jartsk_params}" />
			       <then>
				     	<property name="jar_upd" value="true"/>
				</then>
				<else>
 ...
 <if>
         <isset property="jartsk_params"/>
         <then>
                 <propertyregex property="jar_info_update" input="${jartsk_params}" regexp="([^,]+)[,]{1}([^,]+)[,]{1}([^,]+)[,]{1}([^,]*)[,]{1}([^,]+)[,]{1}([^,]+)" select="\6" casesensitive="true"/> 
                 <echo message="JarInfoUpdate:-${jar_info_update}" />
                 <if>
                         <and>
                                 <isset property="jar_info_update"/>
                                 <equals arg1="${jar_info_update}" arg2="yes"/>
                         </and>
                         <then>
                                 <property name="jar_info_upd" value="true"/>
                         </then>
                <else>
                         <property name="jar_info_upd" value="false"/>
			</else>
	               </if>
	       </then>
	...
	<then>
            <if>
            <equals arg1="${product_name}" arg2="GAC"/>
            <then>
                <property name="additional_jar_excludes" value=""/>
            </then>
            <else>
                <property name="additional_jar_excludes" value="**/zohoinstrumentation.jar **/iotrace.jar"/>
            </else>
          </if>
	...
	<antcall target="genjar">
		...
		<param name="${val}_jar_excludes" value="${jar_excludes} ${additional_jar_excludes}"/>
		<param name="${val}_jar_update" value="${jar_upd}"/>
                <param name="${val}_jar_info_update" value="${jar_info_upd}"/>
	</antcall>
	</then>
	<else>
	<antcall target="genjar">
...
<target name="dlltask" depends="init">
    ...
    <propertyregex property="dlltaskmachine"  input="${dllargs}" regexp="([^\s]+)" select="\1" casesensitive="true" override="true"/>
    <propertyregex property="dlltaskinputzip"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)" select="\3" casesensitive="true" override="true"/>
        <propertyregex property="dlltaskbat"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\5" casesensitive="true" override="true"/>
    ...
    <antcall target="exec">
        ...
        <param name="${val}_exec_dir" value="${basedir}"/>
        <param name="${val}_exec_command" value="sh"/>
	<param name="${val}_exec_args" value="hg_utils/build/winpackdet.sh ${dlltaskmachine} ${dlltaskinputzip} ${dlltaskbat} ${basedir}/unzipsrc"/>
    </antcall>


<if>
	<equals arg1="${wintask_needed}" arg2="yes" />
       <then>
	       <echo  message="wintask calling "/>
	       <propertyregex property="dlltaskmachine"  input="${dllargs}" regexp="([^\s]+)" select="\1" casesensitive="true" override="true"/>
               <propertyregex property="dlltaskport"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)" select="\2" casesensitive="true" override="true"/>
               <propertyregex property="dlltaskinputzip"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)" select="\3" casesensitive="true" override="true"/>
               <propertyregex property="dlltaskopzip"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\4" casesensitive="true" override="true"/>
	       <propertyregex property="dlltaskbat"  input="${dllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\5" casesensitive="true" override="true"/>
		<propertyregex property="dllendoptions"  input="${dllargs}" regexp="(?:([^\s]*\s)){5}(.*)" select="\2"  casesensitive="true" override="true"/>

		<if>
                	<not><isset property="dllendoptions" /></not>
	                <then>
                        	<property name="dllendoptions" value=""/>
                	</then>
        	</if>


		<property name="alldllargs" value="${dlltaskmachine} ${dlltaskinputzip} ${dlltaskopzip} ${dlltaskbat} ${dllendoptions}"/>

	       <antcall target="wintask">
                  <param name="val" value="${val}"/>
		  <param name="${val}_wintask" value="${alldllargs}"/>
               </antcall>
...
<target name="macpkgtask" depends="init">
	...
	<propertycopy name="maclyteopval" from="${val}_macpkg_opdir" silent="true"/>
        <property name="maclyteop" value="${maclyteopval}"/>
...
<if>
         <and>
               <equals arg1="${exe_bin_skip_needed}" arg2="yes" />
               <equals arg1="${macpkguser}" arg2="installanywhere" />
         </and>

<then>
         <record name="${basedir}/buildlogs/${mptask}_${val}_${msec}_bin_macpkgtask_skipped.log" action="start" loglevel="error"/>
         <echo  message="linux bin task skipped "/>
...
<else>
	 ...
	 <condition property="get_mac_det">
             <or>
                 <contains  substring="integ-ios" string = "${macpkghost}" />
                 <contains  substring="integ-macserver" string = "${macpkghost}" />
        ...
        <if>
             ...
             <and>
                <isset property="get_mac_det"/>
                <contains  substring="lyte" string = "${macpkglocalsrc}" />
		<equals arg1="${linux_lyte_needed}" arg2="yes"/>
             </and>
             <then>
                 <dirname property="lytesrc_dirpath" file="${macpkglocalsrc}"/>
                 <basename property="lyteop_filename" file="${macpkglocaldest}"/>
                 <dirname property="lyteop_destpath" file="${macpkglocaldest}"/>

                <copy todir="${basedir}" overwrite="true" includeEmptyDirs="false">
                                <fileset dir="${basedir}/hg_utils/build" includes="linux_lyte_docker.sh" excludes=""/>
                </copy>

                <exec dir="${basedir}/" executable="ssh" failonerror="true" resultproperty="exitValue">
                    <arg line="-o StrictHostKeyChecking=no  ${lyte_username}@${lyte_server} 'mkdir -p ${lytesrc_dirpath}'"/>
                </exec>

                <exec dir="${basedir}/" executable="scp" failonerror="true" resultproperty="exitValue">
                     <arg line="linux_lyte_docker.sh ${lyte_username}@${lyte_server}:${lytesrc_dirpath}"/>
                 </exec>

                <exec dir="${basedir}/" executable="scp" failonerror="true" resultproperty="exitValue">
                            <arg line="${macpkglocalsrc} ${lyte_username}@${lyte_server}:${lytesrc_dirpath}"/>
                 </exec>

                   <if>
			<and>
                         <equals arg1="${lyte_loc_needed}" arg2="yes"/>
			 <equals arg1="${lyte_npminstall_needed}" arg2="yes"/>
			</and>
                    <then>
		      	     <exec dir="${basedir}/" executable="ssh" failonerror="true" resultproperty="exitValue">
		             <arg line="${lyte_username}@${lyte_server} 'bash -x ${lytesrc_dirpath}/linux_lyte_docker.sh ${macpkglocalsrc} ${build_id} ${lyteop_filename} ${lyte_imagename} ${lyte_loc_needed} ${lyte_npminstall_needed}'"/>
                	     </exec>
                    </then>
			<elseif>
				<equals arg1="${lyte_loc_needed}" arg2="yes"/>
			<then>
				...
				<exec dir="${basedir}/" executable="ssh" failonerror="true" resultproperty="exitValue">
                             <arg line="${lyte_username}@${lyte_server} 'bash -x ${lytesrc_dirpath}/linux_lyte_docker.sh ${macpkglocalsrc} ${build_id} ${lyteop_filename} ${lyte_imagename} ${lyte_loc_needed}' "/>
                             </exec>
			</then>
		</elseif>

		<elseif>
                                <isset property="maclyteop" />
                                <then>

                                <exec dir="${basedir}/" executable="ssh" failonerror="true" resultproperty="exitValue">
                                        <arg line="${lyte_username}@${lyte_server} 'bash -x ${lytesrc_dirpath}/linux_lyte_docker.sh ${macpkglocalsrc} ${build_id} ${lyteop_filename} ${lyte_imagename} ${maclyteop}' "/>
                             </exec>
                        </then>
                        </elseif>
                    ...
                    <else>
	    	    	<exec dir="${basedir}/" executable="ssh" failonerror="true" resultproperty="exitValue">
		        <arg line="${lyte_username}@${lyte_server} 'bash -x ${lytesrc_dirpath}/linux_lyte_docker.sh ${macpkglocalsrc} ${build_id} ${lyteop_filename} ${lyte_imagename} '"/>
                	</exec>
                    </else>
                   </if>
        <basename property="lyte_src_filename" file="${macpkglocalsrc}"/>

      <propertyregex property="lyte_filename" input="${lyte_src_filename}" regexp=".zip" replace="" casesensitive="true"/>
               <exec dir="${basedir}/" executable="scp" failonerror="false" resultproperty="exitValue">
                       <arg line="${lyte_username}@${lyte_server}:${lytesrc_dirpath}/lytelogs.zip ${lyteop_destpath}"/>
 	        </exec>
                <antcall target="unzip">
                                   <param name="${val}_unzip_srcdir" value="${lyteop_destpath}"/>
                                   <param name="${val}_unzip_todir" value="${build_dir}/output/"/>
                                   <param name="${val}_files_include" value="lytelogs.zip"/>
                </antcall>
                <exec dir="${basedir}/" executable="scp" failonerror="true" resultproperty="exitValue">
                       <arg line="${lyte_username}@${lyte_server}:${lytesrc_dirpath}/${lyte_filename}/${lyteop_filename}  ${lyteop_destpath}"/>
                </exec>
	...
	<!--
	<if>
                    <contains  substring="integ-hm5" string = "${macpkghost}" />
                    <then>
                    <property name="macpkghost_val" value="integc8-ide"/>
                   </then>

                 <else>
                        <property name="macpkghost_val" value="${macpkghost}"/>
                 </else>
	...
	<if>
		<and>
			  ...
			  <param name="${val}_bincreation_remotedest" value="${macpkgremotedest}"/>
                           <param name="${val}_bincreation_host" value="${macpkghost_val}"/>
	...
	<elseif>
        ...
        <if>
                ...
                <then>
                	...
                	<then>
                        	<sshexec host="${macpkghost_val}" username="${macpkguser}" password="${macpkgpasswd}" command="mkdir -p ${macpkg_dir}" trust="true" serverAliveIntervalSeconds="300"/>
	                        <scp file="${macpkglocalsrc}" todir="${macpkguser}@${macpkghost_val}:${macpkgremotesrc}" password="${macpkgpasswd}" trust="true" serverAliveIntervalSeconds="300"/>
        	                <sshexec host="${macpkghost_val}" username="${macpkguser}" password="${macpkgpasswd}" command="${macpkgcmd}" trust="true" serverAliveIntervalSeconds="300"/>
                	        <scp file="${macpkguser}@${macpkghost_val}:${macpkgremotedest}" todir="${macpkglocaldest}" password="${macpkgpasswd}" trust="true" serverAliveIntervalSeconds="300"/>
                	</then>
                	<else>
                        	<sshexec host="${macpkghost_val}" username="${macpkguser}" password="${macpkgpasswd}" command="mkdir -p ${macpkg_dir}" trust="true"/>
	                        <scp file="${macpkglocalsrc}" todir="${macpkguser}@${macpkghost_val}:${macpkgremotesrc}" password="${macpkgpasswd}" trust="true"/>
        	                <sshexec host="${macpkghost_val}" username="${macpkguser}" password="${macpkgpasswd}" command="${macpkgcmd}" trust="true"/>
                	        <scp file="${macpkguser}@${macpkghost_val}:${macpkgremotedest}" todir="${macpkglocaldest}" password="${macpkgpasswd}" trust="true"/>
                        </else>
                        </if>
                </then>
                <else>
                        <if>
                                ...
                                <then>
                                        <sshexec host="${macpkghost_val}" username="${macpkguser}" keyfile="${user.home}/.ssh/id_rsa" command="mkdir -p ${macpkg_dir}" trust="true"/>
                                        <scp file="${macpkglocalsrc}" todir="${macpkguser}@${macpkghost_val}:${macpkgremotesrc}" keyfile="${user.home}/.ssh/id_rsa" trust="true"/>
                                        <sshexec host="${macpkghost_val}" username="${macpkguser}" keyfile="${user.home}/.ssh/id_rsa" command="${macpkgcmd}" trust="true"/>
                                        <!--scp file="${macpkguser}@${macpkghost}:${macpkgremotedest}" todir="${macpkglocaldest}" keyfile="${user.home}/.ssh/id_rsa" trust="true"/-->
					<echo message="Receiving following output package/file ${macpkgremotedest} from ${macpkghost_val}"/>
		                        <exec dir="${basedir}" executable="scp" failonerror="true" resultproperty="exitValue">
                                                   <arg line="-o StrictHostKeyChecking=no -r ${macpkguser}@${macpkghost_val}:${macpkgremotedest} ${macpkglocaldest}"/>
                                        </exec>
                                </then>
                        <else>
				<echo message="Authentication required to connect ${macpkghost_val} host."/>
				<echo message="Authentication required to connect ${macpkghost_val} host." file="${basedir}/build_err.txt" append="true"/>
    ...
    <antcall target="eco">
                ...
                <param name="${val}_macpkgmachinename_eco_msg" value="${macpkghost_val}"/>
...
<target name="jarjar" depends="init">
  ...
  <record name="${basedir}/buildlogs/${jarjar}_${val}_${msec}.log" action="start" loglevel="info"/>
  <record name="${basedir}/buildlogs/${jarjar}_${val}_${msec}_err.log" action="start" loglevel="error"/>
  <record name="${basedir}/buildlogs/${jarjar}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
  ...
  <propertycopy name="jarmainclass" from="${val}_jarjar_mainclass" silent="true"/>

  <if>
    <isset property="jarmainclass"/>
    <then>
      <taskdef name="jarjar" classname="${jarmainclass}" classpath="${jarclasspath}"/>
    </then>
    <else>
  ...
  <record name="${basedir}/buildlogs/${gtask}_${jarjar}_${msec}_err.log" action="stop"/>
  <record name="${basedir}/buildlogs/${gtask}_${jarjar}_${msec}.log" action="stop"/>
</target>
...
<target name="sha256sum" depends="init">
        <property name="sha256sum" value="sha256"/>

        <record name="${basedir}/buildlogs/${sha256sum}_${val}_${msec}.log" action="start" loglevel="info"/>
        <record name="${basedir}/buildlogs/${sha256sum}_${val}_${msec}_err.log" action="start" loglevel="error"/>
        <record name="${basedir}/buildlogs/${sha256sum}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

        <propertycopy name="inputfile" from="${val}_sha256_inputfile" silent="false"/>

                <mkdir dir="${build_dir}/output"/>
                 <fail message="Provided input file doesn't exist ${inputfile}">
                        <condition>
                        <not>
                ...
                <exec dir="${output_dir}" executable="sha256sum" failonerror="true" append="true" output="${output_dir}/checksum.txt">
                             <arg line="${inputfile}"/>
                 </exec>

                <basename property="inputfilename" file="${inputfile}"/>

                <loadfile property="outputfile" srcfile="${output_dir}/checksum.txt"/>
                   <propertyregex property="inputsha256sum" input="${outputfile}" regexp="(^.*)  (.*)" select ="\1" casesensitive="true"/>

        <record name="${basedir}/buildlogs/${sha256sum}_${val}_${msec}_err.log" action="stop"/>
       <record name="${basedir}/buildlogs/${sha256sum}_${val}_${msec}.log" action="stop"/>

        <replace file="${output_dir}/checksum.txt" token="${build_dir}/" value="" />
...
<propertycopy name="excludefiles" from="${val}_contextpkg_excludes" silent="true"/>
<echo message="context : ${context}"/>
<echo message="contextpkgpara_order : ${context} " file="${basedir}/buildlogs/build_time_new.txt" append="yes"/>
<echo message="" file="${basedir}/buildlogs/build_time_new.txt" append="yes"/>
<propertyregex property="contextval"
               input="${context}"
                     regexp="(.*),"
...
<if>
		...
		<antcall target="getfiletransferdep">
                 </antcall>
...
<!-- ======================================================================
                                     GET Jetty filetransfer zip TARGET
====================================================================== -->

<target name="getfiletransferdep" depends="init">

<property name="getftdep" value="getfiletransferdep"/>
<record name="${basedir}/buildlogs/${getftdep}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${getftdep}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${getftdep}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
<property name="fltransfer_prod_home" value="${basedir}/dependencies/integ_file_transfer" />

        <antcall target="geturl">
	<param name="${val}_url" value="${cpf_build_url}"/>
...
<record name="${basedir}/buildlogs/${getftdep}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${getftdep}_${msec}.log" action="stop"/>
</target>
...
<target name="getaccountcreationdep" depends="init,wget_credentials">
...
<antcall target="geturl">
	<param name="${val}_url" value="https://build.zohocorp.com/framework/accountcreation/milestones/stable/AccountCreation.zip"/>
	<param name="${val}_dest_zip" value="${build_dir}/zips/accountcreation/AccountCreation.zip"/>
        <param name="${val}_url_username" value="${wurl_username}" />
        <param name="${val}_url_password" value="${wurl_password}" />
</antcall>
...
<propertycopy name="src_skipnon_tests" from="${val}_junit_skipnontests" silent="true"/>
<propertycopy name="src_compile_option" from="${val}_junit_exclude_src_compile" silent="true"/>
<propertycopy name="output_html_format" from="${val}_junit_html_format_output_needed" silent="true"/>
<propertycopy name="junit5" from="${val}_junit5_report_enabled" silent="true"/>

<if>
    <and>
      <isset property="src_skipnon_tests"/>
      <equals arg1="${src_skipnon_tests}" arg2="true"/>
    </and>
    <then>
          <property name="skipnontest" value="true"/>
    </then>
<else>
    <property name="skipnontest" value="false"/>
</else>
</if>
...
<if>
  <and>
      <isset property="src_compile_option"/>
      <equals arg1="${src_compile_option}" arg2="no" />
  </and>
  <then>
     <property name="compilation_needed" value="no"/>
  </then>
  <else>
     <property name="compilation_needed" value="yes"/>
  </else>
</if>

<if>
   <equals arg1="${compilation_needed}" arg2="yes" />
   <then>
   <javac
        srcdir="${src_dir}"
...
<if>
	...
	<then>
	  <if>
	    <and>
	      <isset property="junit5"/>
	      <equals arg1="${junit5}" arg2="yes" />
	    </and>
	  <then>

	  <echo message="junit 5 report generation started ...."/>
	  ...
	  <testclasses haltOnFailure="yes" outputdir="${src_output_dir}">
	 ...
	 <echo message="junit 5 report generation ended ...."/>
	</then>
	...
	<else>
	    <junit printsummary="yes" haltonfailure="yes">
          ...
          <batchtest skipNonTests="${skipnontest}" fork="yes" todir="${src_output_dir}">
		        <fileset dir="${src_dir}">
		          <include name="${src_include}"/>
			  <exclude name="${src_exclude}"/>
		        </fileset>
...
<else>
	...
	<if>
	  ...
	  <testclasses haltOnFailure="no" outputdir="${src_output_dir}">
	     <fileset dir="${src_classes_dir}">
	       <include name="${src_include}"/>
	       <exclude name="${src_exclude}"/>
	     </fileset>
...
<if>
	<and>
	<isset property="output_html_format"/>
	<equals arg1="${output_html_format}" arg2="yes"/>
	</and>
	<then>
	<junitreport todir="${src_output_dir}">
		<fileset dir="${src_output_dir}">
			<include name="TEST-*.xml"/>
		</fileset>
		<report format="frames" todir="${src_output_dir}/reports"/>
	</junitreport>
	</then>
</if>
...
<target name="fingerprint_cdn" depends="init">
    <property name="fprintcdn" value="fingerprint_cdn" />
    <record name="${basedir}/buildlogs/${fprinticdn}_${val}_${msec}.log" action="start" loglevel="info"/>
    <record name="${basedir}/buildlogs/${fprintcdn}_${val}_${msec}_err.log" action="start" loglevel="error"/>
   <record name="${basedir}/buildlogs/${fprintcdn}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

   <propertycopy name="srcdir" from="${val}_fingerprint_cdn_srcdir" silent="false" />
   <propertycopy name="destdir" from="${val}_fingerprint_cdn_destdir" silent="false" />
    <propertycopy name="file_replacedir" from="${val}_fingerprint_cdn_replace" silent="true" />
    <propertycopy name="file_no_slice" from="${val}_fingerprint_cdn_slice" silent="true" />

    <antcall target="geturl">
    <param name="${val}_url" value="https://build.zohocorp.com/zoho/nimbus_content_hash/milestones/Channel1/ContentHashGenerator.zip" />
    <param name="${val}_dest_zip" value="${build_dir}/ContentHashGenerator.zip" />
    </antcall>

   <mkdir dir="${build_dir}/finger_cdn"/>

    <antcall target="unzip">
    <param name="${val}_unzip_srcdir" value="${build_dir}/" />
     <param name="${val}_unzip_todir" value="${build_dir}/finger_cdn" />
    <param name="${val}_files_include" value="ContentHashGenerator.zip" />
    </antcall>
  <if>
        <and>
             <isset property="file_replacedir"/>
             <isset property="file_no_slice"/>
        </and>
        <then>
                <echo message="contentHash.sh --source '${srcdir}' --output '${destdir}'  --replace '${file_replacedir}' --slice '${file_no_slice}'" />
                <exec dir="${build_dir}/finger_cdn/bin" executable="sh" failonerror="true">
                   <arg line="contentHash.sh --source '${srcdir}' --output '${destdir}'  --replace '${file_replacedir}' --slice '${file_no_slice}'" />
                 </exec>
        </then>
        <elseif>
        ...
        <isset property="file_no_slice"/>
        <then>
                <echo message="contentHash.sh --source '${srcdir}' --output '${destdir}'  --slice '${file_no_slice}'" />
                <exec dir="${build_dir}/finger_cdn/bin" executable="sh" failonerror="true">
                        <arg line="contentHash.sh --source '${srcdir}' --output '${destdir}'  --slice '${file_no_slice}'" />
                 </exec>
        </then>
        </elseif>
        <elseif>
                <isset property="file_replacedir"/>
                <then>
                <echo message="contentHash.sh --source '${srcdir}' --output '${destdir}' --replace '${file_replacedir}'" />
                        <exec dir="${build_dir}/finger_cdn/bin" executable="sh" failonerror="true">
                        <arg line="contentHash.sh --source '${srcdir}' --output '${destdir}'  --replace '${file_replacedir}'" />
                        </exec>

                </then>
        </elseif>
        <else>
                   <echo message="contentHash.sh --source '${srcdir}' --output '${destdir}'" />
                  <exec dir="${build_dir}/finger_cdn/bin" executable="sh" failonerror="true">
                   <arg line="contentHash.sh --source '${srcdir}' --output '${destdir}' " />
        </exec>
...
<!-- ======================================================================
                                     ANDROIDAPP DEPENDED TARGET
====================================================================== -->

<target name="androidresolver" depends="init">

<property name="zak_access" value="${env.HOME}/${build_id}_ZAND/"/>
<property name="zak_prop" value="${zak_access}/cred_zaks.properties"/>
<property name="zak_zip" value="${zak_access}/zaks_config.zip"/>
<property name="zak_extpath" value="${env.HOME}/kst"/>
<property name="aespath" value="${env.HOME}/integ/aes.jar"/>

<echo message="Android profile configuration ..."/>

<if>
<available file="${zak_prop}" type="file"/>
<then>
      <property name="zakpropfile" value="yes"/>
</then>
</if>

<if>
    <equals arg1="${zakpropfile}" arg2="yes"/>
    <then>
      <loadproperties srcFile="${zak_prop}"/>
    </then>
</if>

<exec dir="${basedir}" executable="${env.JAVA_HOME}/bin/java" resultproperty="exitValue" outputproperty="zakcredls" logError="true">
    <arg line="-jar ${aespath} ${zak} decrypt"/>
</exec>
<exec dir="${basedir}" executable="${env.JAVA_HOME}/bin/java" resultproperty="exitValue" outputproperty="zakpass" logError="true">
    <arg line="-jar ${aespath} ${zakps} decrypt"/>
</exec>
<exec dir="${basedir}" executable="${env.JAVA_HOME}/bin/java" resultproperty="exitValue" outputproperty="zakalias" logError="true">
    <arg line="-jar ${aespath} ${zakal} decrypt"/>
</exec>

<exec dir="${basedir}" executable="unzip" resultproperty="exitValue"  logError="true">
    <arg line="-q -o -P ${zakcredls} -d ${zak_extpath} ${zak_zip}"/>
</exec>

<if>
	<not><available file="${basedir}/gradle.encrypted.properties" type="file"/></not>
	<then>
		<move file="${zak_extpath}/gradle.encrypted.properties" tofile="${basedir}/gradle.encrypted.properties" failonerror="false"/>
	</then>
</if>
...
<target name="androidapp" depends="init,androidresolver,nexus-auth,wget_credentials">
...
<propertycopy name="web_cache" from="android_ext_dep_web_cache_enabled" silent="true"/>
...
<property name="htmldepreport" value="htmlDependencyReport"/>
<property name="signconfig_script" value="${utlility_dir}/android_sign_config_changes.sh"/>
<property name="flutter_encrptfile" value="${env.HOME}/.gradle/gradle.encrypted.properties"/>
<property name="new_sign_products" value="${env.HOME}/${build_id}_ZAND/android_new_sign_product.properties"/>
<var name="newsign_allowed_product_branch" value="no"/>

<!-- Take New sign enabled product list -->
<loadproperties srcFile="${new_sign_products}"/>

<if>
<isset property="new_sign_product_list"/>
<then>
    <for param="prdlist" list="${new_sign_product_list}">
      <sequential>
        <if>
         <equals arg1="${product_name}" arg2="@{prdlist}"/>
         <then>
              <if>
                 <isset property="@{prdlist}_BRANCH"/>
                 <then>
                      <propertycopy name="branchnames" from="@{prdlist}_BRANCH"/>
                       <for param="branchlist" list="${branchnames}">
                        <sequential>
				<if>
				 <or>
				     <equals arg1="${branch_name}" arg2="@{branchlist}"/>
				     <equals arg1="${branch_name}" arg2="ALL"/>
			         </or>
				 <then>
				      <var name="newsign_allowed_product_branch" unset="true"/>
				      <var name="newsign_allowed_product_branch" value="yes"/>
				      <echo message="${line.separator}INFO::: New signing keystore selected for this branch build.${line.separator}"/>
			      </then>
                             </if>
                        </sequential>
                       </for>
...
<!-- New sign enabled product list end -->

<if>
    <or>
    <equals arg1="${product_name}" arg2="ANDROID_APP"/>
    <equals arg1="${product_name}" arg2="ZOHONOTEBOOK_ANDROID"/>
    <equals arg1="${product_name}" arg2="ZOHOIGNITE_ANDROID"/>
    <equals arg1="${product_name}" arg2="ZSIQAVLIBRARY_ANDROID"/>
    <equals arg1="${product_name}" arg2="ARATTAI_ANDROID"/>
    </or>
    <then>
        <property name="skiphtmlreport" value="yes"/>
    </then>
</if>
...
<if>
    <available file="${signconfig_script}" type="file"/>
    <then>
    <antcall target="exec">
        <param name="val" value="${val}_signrep"/>
        <param name="${val}_signrep_exec_dir" value="${basedir}"/>
        <param name="${val}_signrep_exec_command" value="sh"/>
	<param name="${val}_signrep_exec_args" value="${signconfig_script} ${rootdir} ${newsign_allowed_product_branch}"/>
        <param name="${val}_signrep_exec_failonerror" value="false"/>
    </antcall>
    </then>
</if>
...
<property name="app_rep_value_kt" value="id&#40;&quot;com.android.application&quot;&#41;${line.separator}id&#40;&quot;project-report&quot;&#41;"/>
<property name="lib_rep_value_kt" value="id&#40;&quot;com.android.library&quot;&#41;${line.separator}id&#40;&quot;project-report&quot;&#41;"/>

<property name="app_rep_value_groovy" value="id 'com.android.application'${line.separator}id 'project-report'"/>
<property name="lib_rep_value_groovy" value="id 'com.android.library'${line.separator}id 'project-report'"/>


<if>
    <available file="${appdir}/build.gradle" type="file"/>
    <then>
	<property name="plugincheck" value="${appdir}/build.gradle"/>
	...
	<propertyregex property="grep_dq_report_txt_gvy" input="${plugin_check}" regexp="(id &quot;project-report&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_sq_report_txt_gvy" input="${plugin_check}" regexp="(id 'project-report'*)(.*)" select="\1" casesensitive="true" override="true"/>

	<propertyregex property="grep_sq_app_txt_gvy" input="${plugin_check}" regexp="(id 'com.android.application'*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_dq_app_txt_gvy" input="${plugin_check}" regexp="(id &quot;com.android.application&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>

	<propertyregex property="grep_sq_lib_txt_gvy" input="${plugin_check}" regexp="(id 'com.android.library'*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_dq_lib_txt_gvy" input="${plugin_check}" regexp="(id &quot;com.android.library&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>

    </then>
    <else>
	<loadfile property="plugin_check_kt" srcFile="${appdir}/build.gradle.kts"/>
	<propertyregex property="grep_dq_report_txt_kt" input="${plugin_check_kt}" regexp="(&quot;project-report&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_sq_report_txt_kt" input="${plugin_check_kt}" regexp="('project-report'*)(.*)" select="\1" casesensitive="true" override="true"/>

	<propertyregex property="grep_sq_app_txt_kt" input="${plugin_check_kt}" regexp="('com.android.application'*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_dq_app_txt_kt" input="${plugin_check_kt}" regexp="(&quot;com.android.application&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>

	<propertyregex property="grep_sq_lib_txt_kt" input="${plugin_check_kt}" regexp="('com.android.library'*)(.*)" select="\1" casesensitive="true" override="true"/>
	<propertyregex property="grep_dq_lib_txt_kt" input="${plugin_check_kt}" regexp="(&quot;com.android.library&quot;*)(.*)" select="\1" casesensitive="true" override="true"/>
    </else>
</if>

<if>
    <isset property="plugincheck"/>
<then>
<if>
	<not>
	<or>
		...
		<isset property="grep_sq_report_txt"/>
                <isset property="grep_dq_report_txt_gvy"/>
                <isset property="grep_sq_report_txt_gvy"/>
	</or>
	</not>
	<then>
           <if>
		...
		<elseif>
		  <isset property="grep_sq_app_txt_gvy"/>
		  <then>
                        <property name="app_tok_value" value="id 'com.android.application'"/>
                        <property name="app_replace_value" value="${app_rep_value_groovy}"/>
                  </then>
	        </elseif>

                <elseif>
                  <isset property="grep_dq_app_txt_gvy"/>
                  <then>
                        <property name="app_tok_value" value="id &quot;com.android.application&quot;"/>
                        <property name="app_replace_value" value="${app_rep_value_groovy}"/>
                  </then>
                ...
                <elseif>
                   <isset property="grep_dq_lib_txt_gvy"/>
                  <then>
                        <property name="app_tok_value" value="id &quot;com.android.library&quot;"/>
                        <property name="app_replace_value" value="${lib_rep_value_groovy}"/>
                  </then>
                </elseif>
                <elseif>
                   <isset property="grep_sq_lib_txt_gvy"/>
                  <then>
                        <property name="app_tok_value" value="id 'com.android.library'"/>
                        <property name="app_replace_value" value="${lib_rep_value_groovy}"/>
                  </then>
...
<if>
        <not>
        <or>
                <isset property="grep_dq_report_txt_kt"/>
                <isset property="grep_sq_report_txt_kt"/>
        </or>
        </not>
        <then>
           <if>
                <isset property="grep_sq_app_txt_kt"/>
                <then>
                        <property name="app_tok_value_kt" value="id&#40;'com.android.application'&#41;"/>
                        <property name="app_replace_value_kt" value="${app_rep_value_kt}"/>
                </then>
                <elseif>
                  <isset property="grep_dq_app_txt_kt"/>
                  <then>
			<echo message="condition matched..."/>
                ...
                <elseif>
                   <isset property="grep_dq_lib_txt_kt"/>
                  <then>
                        <property name="app_tok_value_kt" value="id&#40;&quot;com.android.library&quot;&#41;"/>
                        <property name="app_replace_value_kt" value="${lib_rep_value_kt}"/>
                  </then>
                </elseif>
                <elseif>
                   <isset property="grep_sq_lib_txt_kt"/>
                  <then>
                        <property name="app_tok_value_kt" value="id&#40;'com.android.library'&#41;"/>
                        <property name="app_replace_value_kt" value="${lib_rep_value_kt}"/>
                  </then>
...
<elseif>
        <and>
        <isset property="app_tok_value_kt"/>
        <isset property ="app_replace_value_kt"/>
        </and>
        <then>
        <antcall target="replacefile">
                 <param name="val" value="${val}_plugname_kt"/>
                 <param name="${val}_plugname_kt_file" value="${appdir}/build.gradle.kts"/>
                 <param name="${val}_plugname_kt_tok" value="${app_tok_value_kt}"/>
                 <param name="${val}_plugname_kt_value" value="${app_replace_value_kt}"/>
        </antcall>
</then>
</elseif>
</if>
...
<if>
   ...
   <then>
        ...
        <param name="${val}_wrapperjar_copy_todir" value="${rootdir}/gradle/wrapper"/>
        <param name="${val}_wrapperjar_fileset_dir" value="${utlility_dir}/gradle/wrapper"/>
        <param name="${val}_wrapperjar_copy_includes" value="gradle-wrapper.jar"/>
        </antcall>
   </then>
</if>
...
<if>
	...
	<mkdir dir="${build_dir}/output/zips"/>
		...
		<param name="${val}_androidsrc_zip_exclude" value="local.properties **/*.keystore **/build/** **/build/ .gradle/** ./gradle/"/>
        </antcall>
	</then>
</if>

<!-- android external maven dep web cache -->

<if>
<and>
        <isset property="web_cache"/>
        <istrue value="${web_cache}"/>
        <not>
                <equals arg1="${androidcmd}" arg2="flutter"/>
        </not>
</and>
<then>
    <!-- property name="aespath" value="${user.home}/integ/aes.jar"/>
    <property name="jdk.location" value="/usr/java/jdk1.8.0_112/bin/java"/>
    <exec dir="${basedir}" executable="${jdk.location}" resultproperty="exitValue" outputproperty="nexusUser" logError="true">
        <arg line="-jar ${aespath} ${env.NEXUS_ANDROID_USER} decrypt"/>
    </exec>
    <exec dir="${basedir}" executable="${jdk.location}" resultproperty="passExitValue" outputproperty="nexusPass" logError="true">
        <arg line="-jar ${aespath} ${env.NEXUS_ANDROID_PASSWD} decrypt"/>
    </exec -->
    <property name="nexusCredentials" value=" -Dorg.gradle.nexusUser=${nexus_user} -Dorg.gradle.nexusPassword=${nexus_pass} -PcredentialsLocation=${basedir} -Dorg.gradle.zandroid.buildUser=${wurl_username} -Dorg.gradle.zandroid.buildPassword=${wurl_password}"/>
    <var name="temp_androidarg" value="${androidarg}${nexusCredentials}"/>
    <var name="androidarg" value="${temp_androidarg}"/>

    <var name="temp_htmldepreport" value="${htmldepreport}${nexusCredentials}"/>
    <var name="htmldepreport" value="${temp_htmldepreport}"/>

<!-- Comment the external Maven repo from gradle files and add the cache repo entry -->
    <exec dir="${basedir}" executable="sh" resultproperty="repoExitValue" logError="true">
	<arg line="${utlility_dir}/android_ext_dep_url_cache.sh ${rootdir}"/>
    </exec>
</then>
</if>
...
<if>
        <equals arg1="${androidcmd}" arg2="flutter"/>
        <then>
            <echo message="Flutter running under from ${rootdir} dir.."/>
            <echo message="Flutter build started ..."/>
	    <property name="flutterproj" value="flutter"/>
	    <if>
		    <not><available file="${flutter_encrptfile}" type="file"/></not>
		    <then>
                         <copy file="${basedir}/gradle.encrypted.properties" tofile="${flutter_encrptfile}"/>
		    </then>
	    </if>
            <antcall target="exec">
                ...
                <param name="${val}_build_exec_args" value="${androidarg}"/>
            </antcall>
        </then>
        <else>
            <antcall target="exec">
		...
		<param name="${val}_build_exec_failonerror" value="true"/>
            </antcall>
...
<!-- Create Android Profile report folder to know the execution time for each task from gradle -->
	...
	<antcall target="eco">
        	...
        	<param name="${val}_profilescript_eco_msg" value="nameofhtml=${val}_profile.html;if [ -d ${rootdir}/build/reports/profile ];then for htmlname in `find ${rootdir}/build/reports/profile -iname '*.html'`; do profilefoldername=`dirname $htmlname`; mv $htmlname $profilefoldername/$nameofhtml; done; cp -rvf ${profilefoldername}/** ${profiledir}; fi"/>
	...
	<antcall target="exec">
		...
		<param name="${val}_profile_exec_failonerror" value="true"/>
	</antcall>
...
<if>
        <not>
           <or>
             <isset property="flutterproj"/>
             <isset property="skiphtmlreport"/>
           </or>
        </not>
        <then>
...
<antcall target="exec">
	...
	<param name="${val}_report_exec_args" value="gradlew ${htmldepreport} --info"/>
	<param name="${val}_report_exec_failonerror" value="true"/>
</antcall>

<if>
<available file="${appdir}/build/reports/project" type="dir"/>
<then>
    <property name="dep_report_dir" value="${appdir}/build/reports/project"/>
</then>
<elseif>
<available file="${rootdir}/build/reports/project" type="dir"/>
<then>
    <property name="dep_report_dir" value="${rootdir}/build/reports/project"/>
</then>
</elseif>
</if>
...
<param name="${val}_reportzip_execdir" value="${dep_report_dir}"/>
...
<antcall target="eco">
	<param name="val" value="${val}_ecoscript"/>
        <param name="${val}_ecoscript_eco_msg" value="find -iname '*.class'; if [ $? -eq 0 ]; then find -type f \( -name '*.class' -o -iname '*.jar' \) | grep -v 'thirdparty_packages\|utility_packages\|gradle-wrapper.jar\|outputs\|android_dependency_folder' | zip ${build_dir}/output/android_java_classes.zip -@ ; cd ${gradle_cachedir}/..; find android_dependency_folder -type f -iname '*.jar' | grep -v 'android_dependencies_folder/caches/modules-2' | zip ${build_dir}/output/android_java_classes.zip -@ ; else echo 'no class available to generate the zip for security report'; fi"/>
...
<antcall target="exec">
	...
	<param name="${val}_classeszip_exec_failonerror" value="true"/>
</antcall>
...
<target name="bundletoapk" depends="init,androidresolver">
...
<property name="bundletooljar" value="${env.ANDROID_HOME}/tools/bundletool-all-1.10.0.jar"/>
<property name="androidkeystore" value="${zak_extpath}/za_rks.keystore"/>
...
<antcall target="exec">
	...
	<param name="${val}_atob_exec_args" value="-jar ${bundletooljar} build-apks --bundle=${bundlepath} --output=${apkpath} --mode=${mode} --ks=${androidkeystore} --ks-key-alias=${zakalias} --key-pass=pass:${zakpass} --ks-pass=pass:${zakpass}"/>
</antcall>
...
<propertycopy name="scaladoc_xml_jar" from="${val}_scaladocxml_jar" silent="true"/>
<propertycopy name="scaladoc_parser_jar" from="${val}_scaladocparser_jar" silent="true"/>
...
<if>
   <and>
   <isset property="scaladoc_xml_jar"/>
   <isset property="scaladoc_parser_jar"/>
   </and>
   <then>

        <property name="scala-xml_jar" value="${scaladoc_xml_jar}"/>
        <property name="scala-parser_jar" value="${scaladoc_parser_jar}"/>


  </then>
       <else>
        <property name="scala-xml_jar" value="${scala.dir}/lib/scala-xml_2.11-1.0.4.jar"/>
        <property name="scala-parser_jar" value="${scala.dir}/lib/scala-parser-combinators_2.11-1.0.4.jar"/>
...
<path id="build.classpath">
    ...
    <pathelement location="${scala-library.jar}" />
    <pathelement location="${scala-reflect.jar}"/>
    <pathelement location="${scala-xml_jar}"/>
    <pathelement location="${scala-parser_jar}"/>
...
<target name="iosapp" depends="init">
  ...
  <propertycopy name="workspaceopt" from="${val}_ignore_workspace" silent="true"/>
  <propertycopy name="buildsystem" from="${val}_modern_build_system_needed" silent="true"/>
  ...
  <if>
    <and>
      ...
      <equals arg1="${buildsystem}" arg2="no"/>
    </and>
    <then>
      <property name="modern_build_system" value="NO"/>
    </then>
    <else>
      <property name="modern_build_system" value="YES"/>
    </else>
  </if>
  <condition property="ios">
    <not>
      <isset property="maccatalyst_enabled"/>
    </not>
  </condition>
  <condition property="ios">
    <not>
      <and>
        <isset property="maccatalyst_enabled"/>
        <isset property="ios_enabled"/>
      </and>
    </not>
  </condition>
  <condition property="both_os">
    <and>
      <isset property="maccatalyst_enabled"/>
      <isset property="ios_enabled"/>
      <equals arg1="${maccatalyst_enabled}" arg2="yes"/>
      <equals arg1="${ios_enabled}" arg2="yes"/>
    </and>
  </condition>
  <condition property="both_os">
    <and>
  ...
  <condition property="macos">
    <and>
      ...
      <isset property="ios_enabled"/>
      <equals arg1="${maccatalyst_enabled}" arg2="yes"/>
      <equals arg1="${ios_enabled}" arg2="no"/>
    </and>
  </condition>
  <condition property="ios">
    <and>
      <isset property="maccatalyst_enabled"/>
      <isset property="ios_enabled"/>
      <equals arg1="${maccatalyst_enabled}" arg2="no"/>
      <equals arg1="${ios_enabled}" arg2="yes"/>
    </and>
  </condition>
  <if>
    <isset property="both_os"/>
    <then>
      <echo message="This is Both iOS and MacOS Build"/>
    </then>
    <elseif>
      <isset property="macos"/>
      <then>
        <echo message="This is Mac OS Build"/>
      </then>
    </elseif>
    <elseif>
      <isset property="ios"/>
      <then>
        <echo message="This is iOS Build"/>
      </then>
    </elseif>
    <else>
      <fail message="None of following ios build variable set :: ios_enabled=yes/no or maccatalyst_enabled=yes/no, kindly check with integration team."/>
    </else>
  </if>
  <!-- Copy SRC Files -->
  <antcall target="copy">
  ...
  <property name="configuration_error_message" value="The below params are mandatory, check and add the missed one in ant.properties, kindly add the below line and corresponding value."/>
  <!-- Set ios properties -->
  ...
  <property name="execute_cmd" value="-o StrictHostKeyChecking=no -l  ${mclogin} ${mcname} cd ${mac_build_src_dir}/build; sh -x ios_build.sh | tee ios_build.log 2&gt;&amp;1"/>
  <property name="xcodebuild" value="/Applications/${xcver}.app/Contents/Developer/usr/bin/xcodebuild"/>
    ...
    <then>
      <if>
        ...
        <then>
          ...
          <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;Pod Installation failed, kindly check the error logs&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/podinstall.sh" append="true"/>
  ...
  <if>
    ...
    <else>
      <if>
        <isset property="ios_app_projects"/>
        <then>
          <for list="${ios_app_projects}" param="proj">
            <sequential>
                    ...
                    <then>
                      ...
                      <for list="${configvalue}" param="config">
                        <sequential>
                          ...
                          <if>
                            ...
                            <then>
                              <if>
                                ...
                                <then>
                                  <fail message="${configuration_error_message}${line.separator}@{proj}_@{config}_scheme_name=&lt;value&gt;${line.separator}@{proj}_@{config}_profile_dir=&lt;value&gt;${line.separator}"/>
                                </then>
                              </if>
                            </then>
                            <elseif>
                              ...
                              <then>
                                <if>
                                  <not>
                                    <and>
                                      <isset property="@{proj}_@{config}_scheme_name"/>
                                      <isset property="maccatalyst_@{config}_profile_dir"/>
                                    </and>
                                  </not>
                                  <then>
                                    <fail message="${configuration_error_message}${line.separator}@{proj}_@{config}_scheme_name=&lt;value&gt;${line.separator}maccatalyst_@{config}_profile_dir=&lt;value&gt;${line.separator}"/>
                                  </then>
                                </if>
                              </then>
                            </elseif>
                            <elseif>
                              ...
                              <then>
                                <if>
                                  ...
                                  <then>
                                    <fail message="${configuration_error_message}${line.separator}@{proj}_@{config}_scheme_name=&lt;value&gt;${line.separator}@{proj}_@{config}_profile_dir=&lt;value&gt;${line.separator}maccatalyst_@{config}_profile_dir=&lt;value&gt;${line.separator}"/>
                                  </then>
                                </if>
                              </then>
                            </elseif>
                          ...
                          <var name="ios_profiledir" unset="true"/>
                          <var name="macos_profiledir" unset="true"/>
                          <propertycopy name="schemename" from="@{proj}_@{config}_scheme_name"/>
                          <if>
                            <isset property="ios"/>
                            <then>
                              <propertycopy name="ios_profiledir" from="@{proj}_@{config}_profile_dir"/>
                              <echo message="${line.separator}&#9;sh ${mac_build_src_dir}/build/placemobileprovision.sh ${mac_build_src_dir}/${ios_profiledir} ios" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                              <echo message="${line.separator}&#9;bash ${mac_build_src_dir}/build/plist_generator.sh ${mac_build_src_dir}/${ios_profiledir}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                              <echo message="${line.separator}&#9;build ${mac_build_src_dir}/${projdir} &quot;${projname}&quot; &quot;${schemename}&quot; ${mac_build_src_dir}/${ios_profiledir}/exportOptions.plist &quot;@{config}&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                            </then>
                            <elseif>
                              <isset property="macos"/>
                              <then>
                                <propertycopy name="macos_profiledir" from="maccatalyst_@{config}_profile_dir"/>
                                <echo message="${line.separator}&#9;sh ${mac_build_src_dir}/build/placemobileprovision.sh ${mac_build_src_dir}/${macos_profiledir} mac" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                                <echo message="${line.separator}&#9;build ${mac_build_src_dir}/${projdir} &quot;${projname}&quot; &quot;${schemename}&quot; ${mac_build_src_dir}/${macos_profiledir}/exportOptions.plist &quot;@{config}&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                              </then>
                            </elseif>
                            <elseif>
                              <isset property="both_os"/>
          ...
          <echo message="${line.separator}}${line.separator}build()${line.separator}{${line.separator}&#9;PROJECT_HOME=&quot;${1}&quot;${line.separator}&#9;PROJECT_NAME=&quot;${2}&quot;${line.separator}&#9;SCHME_NAME=&quot;${3}&quot;${line.separator}&#9;PLIST_PATH=&quot;${4}&quot;${line.separator}&#9;CONFIGURATION=&quot;${5}&quot;${line.separator}&#9;PACKAGE_NAME=&quot;${PROJECT_NAME// /}&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
          ...
          <if>
            <isset property="ios"/>
            <then>
              <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; clean" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
              <if>
                <isset property="maccatalyst_enabled"/>
                <then>
                  <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -workspace &quot;${PROJECT_NAME}.xcworkspace&quot; -destination &quot;generic/platform=iOS&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                </then>
                <else>
                  <if>
                    <isset property="workspaceopt"/>
                    <then>
                      <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -UseModernBuildSystem=${modern_build_system}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                    </then>
                    <else>
                      <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -workspace &quot;${PROJECT_NAME}.xcworkspace&quot; -UseModernBuildSystem=${modern_build_system}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                    </else>
                  </if>
                </else>
              </if>
              <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} iOS app generation failed, kindly check the error logs&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
              ...
              <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} archive generation failed, kindly check the error logs \n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
              <echo message="${line.separator}${line.separator}&#9;zip -ry ${CURDIR}/output/${PROJECT_NAME}_${CONFIGURATION}_iOS.zip &quot;${PROJECT_NAME}.xcarchive&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
            ...
            <elseif>
              <isset property="macos"/>
              <then>
                <echo message="${line.separator}&#9;echo &quot;\n\n${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst Build Started...\n\n&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; clean" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -workspace &quot;${PROJECT_NAME}.xcworkspace&quot; -destination &quot;generic/platform=macOS,variant=Mac Catalyst&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst app generation failed, kindly check the error logs\n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;productbuild --component &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive/Products/Applications/${PROJECT_NAME}.app&quot; &quot;/Applications&quot; --sign &quot;3rd Party Mac Developer Installer: Zoho Corporation (TZ824L8Y37)&quot; &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive/Products/Applications/${PACKAGE_NAME}.pkg&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst generation failed, kindly check the error logs \n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;zip -ry ${CURDIR}/output/${PROJECT_NAME}_${CONFIGURATION}_MacOS.zip &quot;${PROJECT_NAME}.xcarchive&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;echo &quot;${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst build ended ..&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}}${line.separator}main $*${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
              </then>
            </elseif>
            <elseif>
              ...
              <then>
                <!-- iOS Build Started-->
                <echo message="${line.separator}&#9;echo &quot;\n\n${PROJECT_NAME} ${CONFIGURATION} iOS Build Started...\n\n&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; clean" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -workspace &quot;${PROJECT_NAME}.xcworkspace&quot; -destination &quot;generic/platform=iOS&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} iOS app generation failed, kindly check the error logs \n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -exportArchive -exportOptionsPlist ${PLIST_PATH} -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -exportPath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive/Products/Applications&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} archive generation failed, kindly check the error logs \n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;zip -ry ${CURDIR}/output/${PROJECT_NAME}_${CONFIGURATION}_iOS.zip &quot;${PROJECT_NAME}.xcarchive&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;echo &quot;${PROJECT_NAME} ${CONFIGURATION} iOS build ended ..&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <!-- iOS Build Ended-->
                <!-- Mac Catalyst Build Started-->
                <echo message="${line.separator}${line.separator}&#9;if [ -d &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n CleanUp iOS Build Setup.\n\n&quot;${line.separator}&#9;&#9;rm -rf &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot;${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;echo &quot;\n\n${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst Build Started...\n\n&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; clean" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}&#9;${xcodebuild} -scheme &quot;${SCHME_NAME}&quot; -configuration &quot;${CONFIGURATION}&quot; archive -archivePath &quot;${PROJECT_HOME}/${PROJECT_NAME}.xcarchive&quot; -workspace &quot;${PROJECT_NAME}.xcworkspace&quot; -destination &quot;generic/platform=macOS,variant=Mac Catalyst&quot;" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}${line.separator}&#9;if [ $? -ne 0 ]${line.separator}&#9;then${line.separator}&#9;&#9;echo &quot;\n\n Xcodebuild ${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst app generation failed, kindly check the error logs \n\n&quot;${line.separator}&#9;&#9;exit 1${line.separator}&#9;fi" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                ...
                <echo message="${line.separator}&#9;echo &quot;${PROJECT_NAME} ${CONFIGURATION} Mac Catalyst build ended ..&quot;${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <echo message="${line.separator}}${line.separator}main $*${line.separator}" file="${mac_src_base_dir}/build/ios_build.sh" append="true"/>
                <!-- Mac Catalyst Build Ended-->
              </then>
            </elseif>
          </if>
        </then>
        <else>
          <fail message="ios_app_projects variable should be availabe, add the following line in ant.properties and provide the value ${line.separator}ios_app_projects=&lt;project name comma separated&gt;"/>
        </else>
      </if>
    </else>
  </if>
  ...
  <!-- End -->
  <antcall target="eco">
    <param name="val" value="${val}_iosmachinename"/>
    <param name="${val}_iosmachinename_eco_msg" value="${mcname}"/>
    <param name="${val}_iosmachinename_eco_file" value="${basedir}/dependency_server_name.txt"/>
    <param name="${val}_iosmachinename_eco_opt" value="true"/>
  </antcall>
...
<target name="wintask" depends="init">
        ...
        <record name="${basedir}/buildlogs/${dtask}_${val}_${msec}_err.log" action="start" loglevel="error" />
        <record name="${basedir}/buildlogs/${dtask}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes" />
        <propertycopy name="windllargs" from="${val}_wintask" silent="false" />
        <propertycopy name="javaclp" from="${val}_java_classpath" silent="true" />
        <propertycopy name="javacls" from="${val}_java_class" silent="true" />
        <propertycopy name="dllunzip_exclude" from="${val}_files_exclude" silent="true" />
        <propertycopy name="winbuildtype" from="${val}_wintask_buildtype" silent="true" />

        <property name="fltransfer_prod_home" value="${basedir}/dependencies/integ_file_transfer" />
        <property name="javaclass" value="com.integ.websocket.client.ClientRunner" />
	<property name="Targetjre" value="/usr/java/zulu11.41.23_runtime/bin/java" />


        <if>
                <isset property="winbuildtype" />
                <then>
                        <property name="wintaskbuildtype" value="${winbuildtype}" />
                </then>
                <else>
                        <property name="wintaskbuildtype" value="host" />
                </else>
        </if>
        ...
        <propertyregex property="dll_machine_name" input="${windllargs}" regexp="([^\s]+)" select="\1" casesensitive="true" override="true" />
        <propertyregex property="input_zip" input="${windllargs}" regexp="([^\s]+)\s([^\s]+)" select="\2" casesensitive="false" />
        <propertyregex property="output_zip" input="${windllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)" select="\3" casesensitive="false" />
        <propertyregex property="batfile_name" input="${windllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\4" casesensitive="false" />

        <!-- java duplicate File finder -->
        <antcallback target="javatask">
                <param name="val" value="duplicate" />
                <param name="${val}_java_classpath" value="${basedir}/hg_utils/utility_packages/exe/Regression.jar" />
                <param name="${val}_java_class" value="com.adventnet.testtools.regression.DuplicateFileFinder" />
                <param name="${val}_java_args" value="${input_zip}" />
        </antcallback>

	<!--antcall target="exec">
            <param name="val" value="${val}_getpackdet"/>
            <param name="${val}_getpackdet_exec_dir" value="${basedir}"/>
            <param name="${val}_getpackdet_exec_command" value="sh"/>
            <param name="${val}_getpackdet_exec_args" value="hg_utils/build/winpackdet.sh ${dll_machine_name} ${input_zip} ${batfile_name} ${basedir}/unzipsrc"/>
        </antcall-->


        <!-- Download and unzip file transfer production build -->
        <if>
                <not>
                        <available file="${fltransfer_prod_home}" type="dir" />
                </not>
                <then>
                        <antcall target="geturl">
				<param name="${val}_url" value="${cpf_build_url}" />
        ...
        <!-- Add classpath property by recursively traversing lib directory -->
        <property name="javaclasspath" value="${fltransfer_prod_home}" />
        <if>
                <isset property="javaclasspath" />
                <then>
                        <if>
                                <available file="${javaclasspath}" type="dir" />
                                <then>
                                        <var name="javaclasspath" value="${javaclasspath}" />
                                        <for param="jarfiles">
                                                <path>
                                                        <fileset dir="${javaclasspath}">
                                                                <include name="**/*.jar" />
                                                        </fileset>
                                                </path>
                                                <sequential>
                                                        <var name="javaclasspath" value="${javaclasspath}:@{jarfiles}" />
                                                </sequential>
                                        </for>
                                </then>
                        </if>
        ...
        <!-- Start file transfer client program -->
        <echo message="----- Establishing connection to remote server -------------------" />
        <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
                <sysproperty key="prod.home" value="${fltransfer_prod_home}" />
                <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/" />
                <jvmarg value="-Dsun.zip.disableMemoryMapping=true" />
		<arg line="${product_name} ${branch_name} ${basedir} ${source_basedir}/../logs/ ${build_id} ${val} ${wintaskbuildtype} ${windllargs}" />
        </java>
...
<!-- ======================================================================
    WINDOWS EXE CREATION TASK TARGET
====================================================================== -->
<target name="winexetask" depends="init">
        <property name="winetask" value="winexetask" />
        <record name="${basedir}/buildlogs/${winetask}_${val}_${msec}.log" action="start" loglevel="info" />
        <record name="${basedir}/buildlogs/${winetask}_${val}_${msec}_err.log" action="start" loglevel="error" />
        <record name="${basedir}/buildlogs/${winetask}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes" />
        <propertycopy name="winexedllargs" from="${val}_winexetask" silent="false" />

        <propertycopy name="winexebuildtype" from="${val}_winexetask_buildtype" silent="true" />
	<propertycopy name="winexeprodir" from="${val}_winexetask_projectdir" silent="true" />
	<propertycopy name="winexe_proj_zipdir" from="${val}_winexetask_project_dir_needed" silent="true" />

        <property name="fltransfer_prod_home" value="${basedir}/dependencies/integ_file_transfer" />
        <property name="javaclass" value="com.integ.websocket.client.WinExeClientRunner" />
        <property name="Targetjre" value="/usr/java/zulu11.41.23_runtime/bin/java" />

<if>
         <equals arg1="${exe_bin_skip_needed}" arg2="yes" />
<then>
        <record name="${basedir}/buildlogs/${etask}_${val}_${msec}_winexeskipped.log" action="start" loglevel="warn" append="yes"/>
           <echo  message="winexetask skipped "/>
...
<else>
        ...
        <if>
        <and>
        <isset property="winexe_proj_zipdir"/>
        <equals arg1="${winexe_proj_zipdir}" arg2="yes"/>
        </and>
        <then>
                <property name="winexetask_rmi_structure_needed" value="yes"/>
	</then>
	<elseif>
		<and>
		<isset property="winexe_proj_zipdir"/>
		<equals arg1="${winexe_proj_zipdir}" arg2="no"/>
		</and>
		<then>
			<property name="rmi_not_needed" value="yes"/>
		</then>
	</elseif>
        </if>

        <if>
                <isset property="winexebuildtype" />
                <then>
                        <property name="exebuildtype" value="${winexebuildtype}" />
                </then>
                <else>
                        <property name="exebuildtype" value="host" />
                </else>
        </if>
        ...
        <if>
                ...
                <then>
                        <echo message="Build id is not passed for wintask" file="${basedir}/build_err.txt" append="true" />
                        <fail message="Build id is not passed" />
                </then>
        </if>
        ...
        <if>
                ...
                <then>
                        <echo message="product_name param is not passed for wintask" file="${basedir}/build_err.txt" append="true" />
                        <fail message="product_name param is not passed" />
                </then>
        </if>

        <if>
                <isset property="winexeprodir" />
                <then>
                        <property name="winexesourcedir" value="${winexeprodir}" />
                </then>
                <else>
			...
			<propertyregex property="project_dir" input="${winexedllargs}" regexp="([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)\s([^\s]+)" select="\5" casesensitive="false" />

                        <property name="winexesourcedir" value="${basedir}/${isdir}/projects/${project_dir}" />
                </else>
        </if>

        <propertyregex property="input_zip" input="${winexedllargs}" regexp="([^\s]+)\s([^\s]+)" select="\2" casesensitive="false" />
        <echo message="----- Input zip file - ${input_zip} ------------" />
        <echo message="----- Check for duplicate files in input zip since windows is a case sensitive file system ------------" />

        <!-- java duplicate File finder -->

        <java classpath="${basedir}/hg_utils/utility_packages/exe/Regression.jar" fork="yes" failonerror="true" classname="com.adventnet.testtools.regression.DuplicateFileFinder">
                <arg line="${input_zip}" />
        </java>

        <!-- Download production build and copy common build files to installation directory -->
        ...
        <antcallback target="copy">
                <param name="${val}_copy_todir" value="${winexesourcedir}" />
                <param name="${val}_fileset_dir" value="${build_dir}/dependencies/did_injection_utils/DIDInjectionUtils" />
                <param name="${val}_copy_includes" value="Script_Files/** Setup_Files/**" />
                <param name="${val}_copy_excludes" value="" />
	</antcallback>
        ...
        <if>
        <isset property="winexesourcedir"/>
        <then>
        <antcall target="exec">
            <param name="val" value="${val}_replace"/>
            <param name="${val}_replace_exec_dir" value="${basedir}"/>
            <param name="${val}_replace_exec_command" value="sh"/>
            <param name="${val}_replace_exec_args" value="-x ${build_dir}/utility_packages/exe/rce_installshield_cmd.sh ${winexesourcedir}"/>
        </antcall>
        <antcall target="exec">
            <param name="val" value="${val}_publisher"/>
            <param name="${val}_publisher_exec_dir" value="${basedir}"/>
            <param name="${val}_publisher_exec_command" value="sh"/>
            <param name="${val}_publisher_exec_args" value=" ${build_dir}/utility_packages/exe/publisher_name_change.sh ${winexesourcedir}"/>
        </antcall>
        </then>
        </if>
	...
	<if>
	<and>
		<equals arg1="${winexetask_rmi_structure_needed}" arg2="yes" />
		<not><isset property="rmi_not_needed"/></not>
        </and>
         <then>

	 	<antcallback target="ziptask">
                                <param name="${val}_execdir" value="${basedir}"/>
                                <param name="${val}_zip_name" value="${input_zip}"/>
                                <param name="${val}_dir_tozip" value="${isdir}/projects/${project_dir}"/>
                                <param name="${val}_zip_exclude" value="${zipexclude}"/>
		</antcallback>
	   </then>
        ...
        <!--Binary Build Changes case starts-->
        <echo message="Binary Build value is ${binary_build}" />
        <if>
             <and>
                <isset property="binary_build"/>
                <equals arg1="${binary_build}" arg2="yes"/>
            </and>
            <then>
                   <dirname property="srczip_dir_path" file="${input_zip}"/>
                   <basename property="exe_src_zip_name" file="${input_zip}"/>
                   <antcallback target="unzip">
                       <param name="${val}_unzip_srcdir" value="${srczip_dir_path}"/>
                       <param name="${val}_unzip_todir" value="${build_dir}/${val}"/>
                       <param name="${val}_files_include" value="${exe_src_zip_name}"/>
                       <param name="${val}_files_exclude" value="${unzip_exclude}"/>
                       <param name="${val}_pattern_include" value="**/WindowsInstallerSrc.zip"/>
                   </antcallback>
                   <antcallback target="unzip">
                       <param name="${val}_unzip_srcdir" value="${build_dir}/${val}"/>
                       <param name="${val}_unzip_todir" value="${winexesourcedir}"/>
                       <param name="${val}_files_include" value="WindowsInstallerSrc.zip"/>
                       <param name="${val}_files_exclude" value="${unzip_exclude}"/>
                   </antcallback>
            </then>
        </if>
        <!--Binary Build Changes case ends-->

        <!-- Start file transfer client program -->

        <echo message="----- Computed project directory - ${winexesourcedir} ------------" />
        <echo message="----- Establishing connection to remote server -------------------" />
        <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
                <sysproperty key="prod.home" value="${fltransfer_prod_home}" />
                <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/" />
                <jvmarg value="-Dsun.zip.disableMemoryMapping=true" />
		<arg line="${product_name} ${branch_name} ${basedir} ${source_basedir}/../logs/ ${build_id} ${val} ${exebuildtype} ${winexesourcedir} ${winexedllargs}" />
        </java>
...
<target name="signppm" depends="init,signtask_prop">
<property name="sppm" value="signppm"/>
<record name="${basedir}/buildlogs/${sppm}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${sppm}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${sppm}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="ppmpath" from="${val}_signppm_folder_path" silent="false"/>

<echo message="${ppmpath}"/>

<fileset id="getppmpath" dir="${ppmpath}" includes="**/*.ppm">
    <depth max="1"/>
</fileset>
<pathconvert pathsep="${line.separator}" property="ppm" refid="getppmpath"/>
...
<then>
	<fail message="PPM file not available under ${ppmpath} !.. Kindly provide correct PPM path .."/>
</then>
</if>

<!-- New signing changes added in condition start here -->
<if>
<not>
<isset property="old_sign"/>
</not>
<then>

<for param="ppmfilepath" list="${ppm}" delimiter="${line.separator}">
      <sequential>
	<echo message="FilePath: @{ppmfilepath}"/>
        <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
                <sysproperty key="prod.home" value="${fltransfer_prod_home}" />
                <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/" />
                <jvmarg value="-Dsun.zip.disableMemoryMapping=true" />
		<arg line="@{ppmfilepath} @{ppmfilepath} ${basedir} ${source_basedir}/../logs/ ${product_name} ${branch_name} ${build_id} linux --individual-file-signing --specific-host-connect"/>
        </java>
      </sequential>
</for>
...
<property name="aespath" value="${user.home}/integ/aes.jar"/>
<property name="keyfilepath" value="${user.home}/integ/pfx/sectigo-cs.pfx"/>
<property name="old_sign_products" value="${user.home}/integ/pfx/ppm_old_sign_product.properties"/>
<property name="keyaliasname" value="{9DD425EC-409F-45CE-B386-C98A4CAC1692}"/>
<property name="store_type" value="pkcs12"/>
<property name="keystorepasswd" value="QQXN5NhuNcG5i6bD2bWmrQ=="/>
<!--property name="keystorepasswd" value="QyTT7hxlDgacoSNiflQ3yg=="/-->


<if>
	<and>
	<isset property="old_sign"/>
	<equals arg1="${old_sign}" arg2="yes"/>
        </and>
	<then>
	    <if>
	        <available file="${old_sign_products}" type="file"/>
		<then>
		    <loadproperties srcFile="${old_sign_products}"/>
		</then>
            </if>
    </then>
    <else>
	 <echo message="PPM Signing Error: kindly remove the old_sign=yes property from ant.properties to sign using valid certificate.."/>
         <fail message="PPM Signing Error: kindly remove the old_sign property from ant.properties to sign using valid certificate.."/>
    </else>
</if>

<if>
<isset property="old_sign_product_list"/>
<then>
    <for param="prdlist" list="${old_sign_product_list}">
      <sequential>
        <if>
	 <equals arg1="${product_name}" arg2="@{prdlist}"/>
         <then>
                       ...
                       <for param="branchlist" list="${branchnames}">
                        <sequential>
                             <if>
                                 <equals arg1="${branch_name}" arg2="@{branchlist}"/>
                                 <then>
                                      <property name="allowed_product_branch" value="yes"/>
                                 </then>
			     </if>
                        </sequential>
                       </for>
...
<available file="${aespath}" type="file"/>
<then>
	<exec dir="${basedir}" executable="java" resultproperty="exitValue" outputproperty="outputvalue" logError="true">
	<arg line="-jar ${aespath} ${keystorepasswd} decrypt"/>
	</exec>
	...
	<equals arg1="${exitValue}" arg2="0"/>
	<then>
		<var name="keystorepasswd" unset="true"/>
		<property name="keystorepasswd" value="${outputvalue}"/>
		<echo message="Auth Decryption ..."/>
	</then>
	<else>
		<fail message="Certificate auth is mismatch !.. Kindly check with integration-team"/>
	</else>
	</if>
</then>
</if>
...
<if>
    <and>
        <isset property="allowed_product_branch"/>
	<equals arg1="${allowed_product_branch}" arg2="yes"/>
	</and>
<then>
<for param="ppmfilepath" list="${ppm}" delimiter="${line.separator}">
      <sequential>
      	      <echo message="@{ppmfilepath}"/>
      	     <!-- retry delay will work since ant 1.8.3 -->
             <retry retrycount="10" retrydelay="60000">
		 <signjar jar="@{ppmfilepath}" storetype="${store_type}" alias="${keyaliasname}" keystore="${keyfilepath}"  storepass="${keystorepasswd}" tsaurl="http://timestamp.sectigo.com/rfc3161"/>
             </retry>
      </sequential>
</for>
</then>
<else>
	 <echo message="PPM Signing Error: ${product_name} product ${branch_name} branch build not allowed to sign with old certificate, kindly check with integration-team .."/>
         <fail message="PPM Signing Error: ${product_name} product ${branch_name} branch build not allowed to sign with old certificate, kindly check with integration-team .."/>
</else>
</if>
...
<record name="${basedir}/buildlogs/${sppm}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${sppm}_${val}_${msec}.log" action="stop"/>
...
<target name="signfile" depends="init,signtask_prop">
<property name="sjson" value="signfile"/>
<record name="${basedir}/buildlogs/${sjson}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${sjson}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${sjson}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="filepath" from="${val}_signfile_folder_path" silent="false"/>
<propertycopy name="file_includes" from="${val}_signfile_includes" silent="true"/>
<propertycopy name="fileextension" from="${val}_signfile_fileextension" silent="false"/>
<propertycopy name="signkey" from="${val}_signfile_signkey" silent="true"/>
<propertycopy name="detachsign" from="${val}_signfile_detachsignature" silent="true"/>

<if>
    <and>
        <isset property="file_includes"/>
        <length string="${file_includes}" trim="true" when="greater" length="0"/>
    </and>
<then>
	<fileset id="getfilepath" dir="${filepath}" includes="${file_includes}"/>
</then>
<else>
<fileset id="getfilepath" dir="${filepath}" includes="**/*.${fileextension}">
    <depth max="3"/>
</fileset>
</else>
</if>
<pathconvert pathsep="${line.separator}" property="json" refid="getfilepath"/>
...
<then>
        <fail message="File is not available under ${filepath} !.. Kindly provide valid File path .."/>
</then>
</if>

<condition property="signkey_prop" value="--signKey=${signkey}">
<and>
    <isset property="signkey"/>
    <length string="${signkey}" trim="true" when="greater" length="0"/>
</and>
</condition>
<condition property="detachsign_prop" value="--detachedSignature=${detachsign}">
<and>
    <isset property="detachsign"/>
    <length string="${detachsign}" trim="true" when="greater" length="0"/>
</and>
</condition>

<!-- Json sign property param configuration starts -->

<property name="json_param" value=""/>

<if>
   <and>
       <isset property="signkey_prop"/>
       <isset property="detachsign_prop"/>
   </and>
<then>
	<var name="json_param" unset="true"/>
	<property name="json_param" value="${signkey_prop} ${detachsign_prop}"/>
</then>
<elseif>
    <and>
    <or>
    ...
    <then>
        <var name="json_param" unset="true"/>
        <property name="json_param" value="${signkey_prop}"/>
    </then>
</elseif>
<elseif>
    <and>
    ...
    <then>
        <var name="json_param" unset="true"/>
        <property name="json_param" value="${detachsign_prop}"/>
    </then>
</elseif>
</if>
<!-- Json sign property param configuration ends -->

<for param="jsonfilepath" list="${json}" delimiter="${line.separator}">
      <sequential>
	     <echo message="@{jsonfilepath}"/>
	     <basename property="signfile_suffix" file="@{jsonfilepath}" suffix="${fileextension}"/>
	     <if>
		<isset property="signfile_suffix"/>
             <then>
             <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
                <sysproperty key="prod.home" value="${fltransfer_prod_home}" />
                <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/" />
                <jvmarg value="-Dsun.zip.disableMemoryMapping=true" />
		<arg line="@{jsonfilepath} @{jsonfilepath} ${basedir} ${source_basedir}/../logs/ ${product_name} ${branch_name} ${build_id} json --individual-file-signing --specific-host-connect --json-file-signing ${json_param}"/>
	</java>
	</then>
	</if>
      </sequential>
...
<!-- ====================================================================================================
                              Linux Sign Property
=======================================================================================================-->
<target name="signtask_prop">

<property name="fltransfer_prod_home" value="${basedir}/dependencies/integ_file_transfer" />
<property name="javaclass" value="com.integ.websocket.sign.client.SigningClientRunner" />
<property name="Targetjre" value="/usr/java/zulu11.41.23_runtime/bin/java"/>
<property name="javaclasspath" value="${fltransfer_prod_home}" />

<echo message="Sign task profile configuration .."/>
        <!-- Download and unzip file transfer production build -->
...
<!-- ===================================================================================================
					Nexus Maven Authentication
=======================================================================================================-->

<target name="nexus-auth">
    <property name="aespath" value="${user.home}/integ/aes.jar"/>
    <property name="jdk.location" value="/usr/java/jdk1.8.0_112/bin/java"/>
    <property name="nexus.prop.location" value="${user.home}/nexusMaven/nx-env.properties"/>

    <echo message="Nexus Profile Configuration ..."/>

    <if>
       <available file="${nexus.prop.location}" type="file"/>
       <then>
           <loadproperties srcFile="${nexus.prop.location}"/>
       </then>
    </if>

    <exec dir="${basedir}" executable="${jdk.location}" resultproperty="exitValue" outputproperty="nexus_user" logError="true">
        <arg line="-jar ${aespath} ${NEXUS_MAVEN_USER} decrypt"/>
    </exec>
    <exec dir="${basedir}" executable="${jdk.location}" resultproperty="passExitValue" outputproperty="nexus_pass" logError="true">
        <arg line="-jar ${aespath} ${NEXUS_MAVEN_PASSWD} decrypt"/>
    </exec>
</target>

<!-- ===================================================================================================
                                                  Windows Package Sign Task
=======================================================================================================-->
<target name="winsign" depends="init,signtask_prop">
  <property name="wsign" value="winsign"/>
  <record name="${basedir}/buildlogs/${wsign}_${val}_${msec}.log" action="start" loglevel="info"/>
  <record name="${basedir}/buildlogs/${wsign}_${val}_${msec}_err.log" action="start" loglevel="error"/>
  <record name="${basedir}/buildlogs/${wsign}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
  <!-- If you want to sign multiple package, kindly zip the required packages and pass to <val>_winsign_filepath and set the <val>_winsign_groupfile_sign_enabled property as yes-->

  <propertycopy name="sign_filepath" from="${val}_winsign_filepath" silent="false"/>
  <propertycopy name="groupsign_status" from="${val}_winsign_groupfile_sign_enabled" silent="true"/>
  <propertycopy name="specifichost_status" from="${val}_winsign_specific_host_enabled" silent="true"/>

  <if>
    <and>
      <isset property="groupsign_status"/>
      <equals arg1="${groupsign_status}" arg2="yes"/>
    </and>
    <then>
      <property name="signoptions" value=""/>
    </then>
    <elseif>
      <not>
        <isset property="groupsign_status"/>
      ...
      <then>
        <property name="signoptions" value="--individual-file-signing"/>
      </then>
    </elseif>
    <else>
      <property name="signoptions" value="--individual-file-signing"/>
  ...
  <if>
    <and>
      <isset property="specifichost_status"/>
      <equals arg1="${specifichost_status}" arg2="yes"/>
    </and>
    <then>
      <property name="specifichost" value="--specific-host-connect"/>
      <var name="signoptions_temp" value="${signoptions} ${specifichost}"/>
      <var name="signoptions" value="${signoptions_temp}"/>
    </then>
  </if>
  <echo message="Input FilePath: ${sign_filepath}"/>
  <echo message="Sign Options: ${signoptions}"/>
  <java jvm="${Targetjre}" classpath="${javaclasspath}" fork="yes" failonerror="true" classname="${javaclass}" maxmemory="1024m">
    <sysproperty key="prod.home" value="${fltransfer_prod_home}"/>
    <sysproperty key="integ.logspath" value="${fltransfer_prod_home}/logs/"/>
    <jvmarg value="-Dsun.zip.disableMemoryMapping=true"/>
    <arg line="${sign_filepath} ${sign_filepath} ${basedir} ${source_basedir}/../logs/ ${product_name} ${branch_name} ${build_id} windows ${signoptions}"/>
  </java>
  <record name="${basedir}/buildlogs/${wsign}_${val}_${msec}_err.log" action="stop"/>
  <record name="${basedir}/buildlogs/${wsign}_${val}_${msec}.log" action="stop"/>
</target>

<!-- ===================================================================================================
                                     Thirdparty Jar Download Through Nexus Maven
=======================================================================================================-->

<target name="tpartifacts" depends="init,nexus-auth">
<property name="tpfiles" value="tpartifacts"/>
<record name="${basedir}/buildlogs/${tpfiles}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${tpfiles}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${tpfiles}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="tpfilepath" from="${val}_ivy_xmlfile_path" silent="false"/>

<property name="tp_commercial_filepath" value="${basedir}/thirdparty-commercial-ivy.xml"/>
<property name="tpsettingsfile" value="${env.HOME}/nexusMaven/tp_ivysettings.xml"/>
<property name="in_pkg_filepath" value="${basedir}/thirdparty-internal-ivy.xml"/>
<property name="tpdest" value="${build_dir}/pkg/thirdparty_packages/lib"/>
<property name="tp_migrated_list" value="${basedir}/tp_migration_list.txt"/>
<property name="tp_home" value="${build_dir}/pkg"/>
<property name="tp_zipfile" value="${build_dir}/output/tp_components.zip"/>

<if>
<not>
   <available file="${tpdest}" type="dir"/>
</not>
<then>
   <mkdir dir="${tpdest}"/>
</then>
...
<if>
    <not>
    <or>
        <available file="${tpfilepath}" type="file"/>
        <available file="${source_basedir}/thirdparty_packages" type="dir"/>
        <available file="${tp_commercial_filepath}" type="file"/>
    </or>
    </not>
    <then>
	    <fail message="thirdparty-ivy.xml file not available!.. Kindly check with integration team.."/>
    </then>
</if>

<property name="ivy.dep.file" value="${tpfilepath}"/>
<property name="ivy.lib.dir" value="${tpdest}"/>
<property name="ivy.report.todir" value="${build_dir}/output/reports/tp_packages"/>

<if>
<not>
   <available file="${ivy.report.todir}" type="dir"/>
</not>
<then>
...
<or>
<available file="${tpfilepath}" type="file"/>
<available file="${in_pkg_filepath}" type="file"/>
<available file="${tp_commercial_filepath}" type="file"/>
</or>
<then>

<ivy:settings file="${tpsettingsfile}" xmlns:ivy="antlib:org.apache.ivy.ant">
<credentials host="zartifacts.csez.zohocorpin.com" realm="Sonatype Nexus Repository Manager" username="${nexus_user}" passwd="${nexus_pass}"/>
</ivy:settings>
...
<available file="${in_pkg_filepath}" type="file"/>
<then>
    <var name="ivy.dep.file" unset="true"/>
    <var name="ivy.dep.file" value="${in_pkg_filepath}"/>
...
<ivy:report todir="${ivy.report.todir}" xsl="true" xmlns:ivy="antlib:org.apache.ivy.ant"/>
...
<available file="${tp_commercial_filepath}" type="file"/>
<then>
<antcall target="tpcmlartifacts">
        <param name="${val}_cml_ivy_xmlfile_path" value="${tp_commercial_filepath}"/>
</antcall>
</then>
</if>

<echo message="${line.separator}"/>
<echo message="Third party files copying from ${source_basedir} to ${tp_home} .."/>
<copy todir="${tp_home}" overwrite="true" verbose="true">
	<fileset dir="${source_basedir}" includes="internal_packages/** new_packages/** thirdparty_packages/** thirdparty_sources/**"/>
</copy>
<echo message="Third party files copied to ${tp_home} .."/>

<if>
    <available file="${tp_migrated_list}" type="file"/>
    <then>
        <exec dir="${basedir}" executable="sh" failonerror="true" resultproperty="exitValue">
		<arg line="${basedir}/hg_utils/build/tp_filemove_script.sh ${tpdest} ${tp_home} ${tp_migrated_list}"/>
        </exec>
    </then>
</if>
...
<if>
    <and>
        <isset property="module_name_commercial"/>
	<available file="${source_basedir}/../${module_name_commercial}" type="dir"/>
    </and>
    <then>
	<echo message="INFO:: ${module_name_commercial} found.. Copying commercial files to ${tp_home}"/>
        <copy todir="${tp_home}" overwrite="true" verbose="true">
		<fileset dir="${source_basedir}/../${module_name_commercial}" includes="**" excludes="*.hg/** README.md"/>
        </copy>
	<echo message="INFO:: ${module_name_commercial} files copied.."/>
    </then>
</if>
...
<echo message="${line.separator}"/>
<echo message="${tp_zipfile} generation starated .."/>
<exec dir="${build_dir}/pkg" executable="zip" failonerror="true">
        <arg line="-r"/>
        <arg line="${tp_zipfile}"/>
        <arg line="."/>
</exec>

<record name="${basedir}/buildlogs/${tpfiles}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${tpfiles}_${val}_${msec}.log" action="stop"/>

</target>

<!-- ===================================================================================================
                                          Nexus Commercial Package Artifacts Download
=======================================================================================================-->

<target name="tpcmlartifacts" depends="init,nexus-auth">
<property name="tpcmaf" value="tpcmartifacts"/>
<record name="${basedir}/buildlogs/${tpcmaf}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${tpcmaf}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${tpcmaf}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="ivy_filepath" from="${val}_cml_ivy_xmlfile_path" silent="false"/>

<property name="tp_commercial_settingsfile" value="${env.HOME}/nexusMaven/tp_commercial_ivysettings.xml"/>
<property name="tpdest_commercial" value="${build_dir}/pkg/thirdparty_packages/lib"/>

<var name="ivy.dep.file" unset="true"/>
<var name="ivy.dep.file" value="${ivy_filepath}"/>
<property name="ivy.lib.dir" value="${tpdest_commercial}"/>
<property name="ivy.report.todir" value="${build_dir}/output/reports/tp_packages"/>

<ivy:settings file="${tp_commercial_settingsfile}" xmlns:ivy="antlib:org.apache.ivy.ant">
<credentials host="zartifacts.csez.zohocorpin.com" realm="Sonatype Nexus Repository Manager" username="${nexus_user}" passwd="${nexus_pass}"/>
</ivy:settings>
...
<record name="${basedir}/buildlogs/${tpcmaf}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${tpcmaf}_${val}_${msec}.log" action="stop"/>
</target>

<!-- ===================================================================================================
                                Nexus Artifacts Download
=======================================================================================================-->

<target name="getartifacts" depends="init,nexus-auth">
<property name="af" value="getartifacts"/>
<record name="${basedir}/buildlogs/${af}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${af}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${af}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="ivy_filepath" from="${val}_artifacts_ivyxml_path" silent="false"/>
<propertycopy name="dest_dir" from="${val}_artifacts_dest_dir" silent="false"/>

<property name="settingsfile" value="${env.HOME}/nexusMaven/ivysettings.xml"/>
...
<if>
    <not>
        <available file="${ivy_filepath}" type="file"/>
    </not>
    <then>
	    <fail message="${ivy_filepath} file not available!.. Kindly check and proceed.."/>
    </then>
</if>

<property name="ivy.dep.file" value="${ivy_filepath}"/>
<property name="ivy.lib.dir" value="${dest_dir}"/>
<property name="ivy.report.todir" value="${build_dir}/output/reports/internal_packages"/>
...
<ivy:settings file="${settingsfile}" xmlns:ivy="antlib:org.apache.ivy.ant">
<credentials host="zartifacts.csez.zohocorpin.com" realm="Sonatype Nexus Repository Manager" username="${nexus_user}" passwd="${nexus_pass}"/>
</ivy:settings>
...
<record name="${basedir}/buildlogs/${af}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${af}_${val}_${msec}.log" action="stop"/>
...
<target name="gradletask" depends="init,wget_credentials,nexus-auth">
<property name="gtask" value="gradletask"/>
<record name="${basedir}/buildlogs/${gtask}_${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/${gtask}_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/${gtask}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>

<propertycopy name="gradle_execdir" from="${val}_gradle_execdir" silent="false"/>
<propertycopy name="gradle_cmd" from="${val}_gradle_cmd" silent="false"/>
<propertycopy name="gradle_arguments" from="${val}_gradle_arguments" silent="false"/>

    <exec dir="${gradle_execdir}" executable="${gradle_cmd}" failonerror="true" resultproperty="exitValue">
	    <arg line="${gradle_arguments} -Dorg.gradle.buildUser=${wurl_username} -Dorg.gradle.buildPassword=${wurl_password} -Dorg.gradle.nexusUser=${nexus_user} -Dorg.gradle.nexusPassword=${nexus_pass}"/>
    </exec>

<record name="${basedir}/buildlogs/${gtask}_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/${gtask}_${val}_${msec}.log" action="stop"/>
</target>

<!-- ===================================================================================================
                                             WGETRC USER CREDENTIALS READER Tasks
=======================================================================================================-->
<target name="wget_credentials">
<if>
...
<!-- ===================================================================================================
                                                  Bower Install Alternative Tasks
=======================================================================================================-->

<target name="bower" depends="init">
<record name="${basedir}/buildlogs/bower__${val}_${msec}.log" action="start" loglevel="info"/>
<record name="${basedir}/buildlogs/bower_${val}_${msec}_err.log" action="start" loglevel="error"/>
<record name="${basedir}/buildlogs/bower_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
<propertycopy name="bower_json_dir" from="${val}_json_dir"  silent="false"/>
<!--antcall target="copy">
	<param name="${val}_fileset_dir" value="/zoho/build/downloads/"/>
	<param name="${val}_copy_todir" value="${bower_json_dir}/"/>
	<param name="${val}_copy_includes" value="node_modules/"/>
</antcall>
<antcall target="copy">
	<param name="${val}_fileset_dir" value="${basedir}"/>
	<param name="${val}_copy_todir" value="${bower_json_dir}/"/>
	<param name="${val}_copy_includes" value="bowerJsonParser.js"/>
</antcall>
<exec dir="${bower_json_dir}" executable="sh" failonerror="true" resultproperty="exitValue">
	<arg line=" ${basedir}/integ_bower_install.sh ${bower_json_dir} ${product_name}" />
</exec>
<if>
<available file="${bower_json_dir}/bower_orig.json" type="file"/>
<then>
<antcallback target="copyfile">
<param name="${val}_copy_src_file" value="${bower_json_dir}/bower_orig.json"/>
<param name="${val}_copy_tofile" value="${bower_json_dir}/bower.json"/>
</antcallback>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-dom/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-dom/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-dom/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-dom" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lytedom/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lytedom/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lytedom/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lytedom" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/crux-components/dist" type="dir"/>
        <then>
    <echo message="CruxComponents Directory Exists..." />
<antcall target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/crux-components/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/crux-components"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcall>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/crux-components" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-ui-editor/dist" type="dir"/>
        <then>
    <echo message="CruxComponents Directory Exists..." />
<antcall target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-ui-editor/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-ui-editor"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcall>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-ui-editor" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-data/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-data/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-data/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-data" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-router/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-router/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-router/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-router" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-component/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-component/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-component/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-component" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/crux_components/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/crux_components/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/crux_components/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/crux_components" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<if>
    <available file="${bower_json_dir}/bower_components/lyte-texteditor/dist" type="dir"/>
    <then>
    <echo message="LyteDom Directory Exists..." />
<antcallback target="copy">
        <param name="${val}_fileset_dir" value="${bower_json_dir}/bower_components/lyte-texteditor/dist/"/>
        <param name="${val}_copy_todir" value="${bower_json_dir}/bower_components/lyte-texteditor/"/>
        <param name="${val}_copy_includes" value="**"/>
        <param name="${val}_copy_excludes" value=""/>
</antcallback>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/bower_components/lyte-texteditor" includes="dist/" excludes=""/>
</delete>
</then>
</if>
<delete verbose="true" includeEmptyDirs="true">
       <fileset dir="${bower_json_dir}/" includes="bowerJsonParser.js" excludes=""/>
</delete-->
<exec dir="${bower_json_dir}" executable="bower" failonerror="true">
              <arg line=" install --allow-root --force" />
</exec>
<record name="${basedir}/buildlogs/bower_${val}_${msec}_err.log" action="stop"/>
<record name="${basedir}/buildlogs/bower_${val}_${msec}.log" action="stop"/>
</target>

<!-- ===================================================================================================
                        Mickeylite related tasks combined as single
=======================================================================================================-->

<target name="getmickey" depends="init">
        <property name="gmickey" value="getmickey"/>
        <record name="${basedir}/buildlogs/${gmickey}_${val}_${msec}.log" action="start" loglevel="info"/>
        <record name="${basedir}/buildlogs/${gmickey}_${val}_${msec}_err.log" action="start" loglevel="error"/>
        <record name="${basedir}/buildlogs/${gmickey}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
        <propertycopy name="dep_src"  from="${val}_getmickey_url" silent="false"/>
        <propertycopy name="mickey_dest"  from="${val}_getmickey_destination" silent="false"/>
        <property name="prop_zip_destdir" value="${build_dir}/zips"/>
        <property name="prop_dep_destdir" value="${build_dir}/dependencies/"/>

        <record name="${basedir}/buildlogs/${gmickey}_${val}_${msec}_err.log" action="stop"/>
        <record name="${basedir}/buildlogs/${gmickey}_${val}_${msec}.log" action="stop"/>
<antcall target="geturl">
                                        <param name="${val}_url" value="${dep_src}"/>
                                             <param name="${val}_dest_zip" value="${prop_zip_destdir}/mickeylite_custom/ZohoMickeyLite.zip"/>
                                </antcall>

                                 <antcall target="unzip">
                                       <param name="${val}_unzip_srcdir" value="${prop_zip_destdir}/mickeylite_custom"/>
                                       <param name="${val}_unzip_todir" value="${prop_dep_destdir}/mickeylite_custom"/>
                                       <param name="${val}_files_include" value="ZohoMickeyLite.zip"/>
                               </antcall>
                                <antcallback target="copy">
                                    <param name="${val}_copy_todir" value="${mickey_dest}/blog/mickeytxtfromsas"/>
                                    <param name="${val}_fileset_dir" value="${mickey_dest}/blog"/>
                                    <param name="${val}_copy_includes" value="mickeylite.txt"/>
                                    <param name="${val}_copy_excludes" value=""/>
                                </antcallback>
                <antcallback target="del">
                                <param name="${val}_del_fileset_dir" value="${mickey_dest}/conf/udt"/>
                                <param name="${val}_delete_includes" value="zchar.dt"/>
                                <param name="${val}_delete_excludes" value=""/>
                </antcallback>
                                <antcallback target="copy">
                                    <param name="${val}_copy_todir" value="${mickey_dest}/"/>
                                    <param name="${val}_fileset_dir" value="${prop_dep_destdir}/mickeylite_custom/AdventNet/MickeyLite"/>
                                    <param name="${val}_copy_includes" value="conf/udt/* lib/AdvAuthorization.jar lib/AdvCache.jar lib/AdvCoreLogging.jar lib/AdvCustomView.jar lib/AdventNetTableComponents.jar lib/AdvI18n.jar lib/AdvMicroFramework.jar lib/AdvPersistence.jar lib/AdvTaskEngine.jar lib/framework-tools.jar lib/jta-1.1.jar lib/jna.jar lib/Mail.jar lib/mysql_connector.jar lib/SwisSQLAPI.jar lib/records.jar blog/mickeylite.txt lib/toolkit-commons.jar"/>
                                    <param name="${val}_copy_excludes" value=""/>
                            </antcallback>
                                <antcallback target="copy">
                                    <param name="${val}_copy_todir" value="${mickey_dest}/lib"/>
                                    <param name="${val}_fileset_dir" value="${prop_dep_destdir}/mickeylite_custom/AdventNet/MickeyLite/bin"/>
                                    <param name="${val}_copy_includes" value="run.jar"/>
                                    <param name="${val}_copy_excludes" value=""/>
                            </antcallback>
</target>
<!-- ===================================================================================================
                        Gradle task to remove unused classes from jars.
=======================================================================================================-->

<target name="trimjar" depends="init">
<property name="trimjar_execdir" value="${basedir}/hg_utils/utility_packages/UnusedClassesRemover_uli/"/>
<property name="GRADLE_PATH" value="/usr/gradle/gradle-8.5/bin/"/>
<property name="JRE_PATH" value="/usr/java/jre_1_8_25_64bit/"/>
<propertycopy name="trimjar_source_jars" from="${val}_trimjar_source_dir" silent="false"/>
<propertycopy name="trimjar_trimmed_jars" from="${val}_trimjar_dest_dir" silent="false"/>
<propertycopy name="trimjar_trimmer_conf" from="${val}_trimjar_conf" silent="false"/>
<property name="trimjarCommand" value="${GRADLE_PATH}/gradle"/>

        <record name="${basedir}/buildlogs/trimjar_${msec}.log" action="start" loglevel="info"/>
        <record name="${basedir}/buildlogs/trimjar_${msec}_err.log" action="start" loglevel="error"/>

                <exec dir="${trimjar_execdir}" executable="${trimjarCommand}" failonerror="true" resultproperty="exitValue">
                    <arg line="runProguard"/>
                    <arg line="-PinputJars=${trimjar_source_jars}"/>
                    <arg line="-PoutputJars=${trimjar_trimmed_jars}"/>
		    <arg line="-PproguardFile=${trimjar_trimmer_conf}"/>
		    <arg line="-PJRE_HOME=${JRE_PATH}"/>
                </exec>

                <echo message="ExitValue from task: ${exitValue}"/>
                <if>
                    <not>
                        <equals arg1="${exitValue}" arg2="0"/>
                    </not>
        ...
        <record name="${basedir}/buildlogs/trimjar_${msec}_err.log" action="stop"/>
        <record name="${basedir}/buildlogs/trimjar_${msec}.log" action="stop"/>
</target>

<!-- ===================================================================================================
                                                       TPSRCCOMPILE TARGET
=======================================================================================================-->
<target name="tpsrccompile" depends="init">
        <property name="tpjar" value="tpsrccompile"/>
   
        <record name="${basedir}/buildlogs/${tpjar}_${val}_${msec}.log" action="start" loglevel="info"/>
        <record name="${basedir}/buildlogs/${tpjar}_${val}_${msec}_err.log" action="start" loglevel="error"/>
        <record name="${basedir}/buildlogs/${tpjar}_${val}_${msec}_err.log" action="start" loglevel="warn" append="yes"/>
   
        <!-- compilesrc attributes start -->
        <propertycopy name="compilesrc_sourcedir" from="${val}_tpsrccompile_srcdir" silent="false"/>
        <propertycopy name="jar_path" from="${val}_tpsrccompile_jar" silent="false"/>
        <propertycopy name="compilesrc_jdk" from="${val}_tpsrccompile_executable" silent="false"/>
        <propertycopy name="compilesrc_compiler" from="${val}_tpsrccompile_compiler" silent="false"/>
        <propertycopy name="compilesrc_classpath" from="${val}_tpsrccompile_classpath" silent="false"/>
        <propertycopy name="compilesrc_include" from="${val}_tpsrccompile_includes" silent="false"/>
   
        <propertycopy name="compilesrc_classesdir" from="${val}_tpsrccompile_classesdir" silent="true"/>
        <propertycopy name="compilesrc_exclude" from="${val}_tpsrccompile_excludes" silent="true"/>
        <propertycopy name="jar_jdk" from="${val}_tpsrccompile_jdk" silent="true"/>
        <!-- compilesrc attributes end -->
   
        <if>
                <isset property="compilesrc_classesdir"/>
                <then>
                        <property name="compile_classesdir" value="${compilesrc_classesdir}"/>
                </then>
                <else>
                        <property name="compile_classesdir" value="${build_dir}/${tpjar}/classes"/>
                </else>
        </if>
        <if>
                <isset property="compilesrc_exclude"/>
                <then>
                        <property name="compile_exclude" value="${compilesrc_exclude}"/>
                </then>
                <else>
                        <property name="compile_exclude" value=""/>
                </else>
        </if>
        <if>
                <isset property="jar_jdk"/>
                <then>
                        <property name="jarjdk" value="${jar_jdk}"/>
                </then>
                <else>
                        <property name="jarjdk" value="${JAVA_HOME}"/>
                </else>
        </if>
   
        <echo message="Compilation started: Calling compilesrc target"/>
        <antcall target="compilesrc">
                <param name="val" value="${val}"/>
                <param name="${val}_compile_srcdir" value="${compilesrc_sourcedir}"/>
                <param name="${val}_classpath" value="${compilesrc_classpath}"/>
                <param name="${val}_compile_includes" value="${compilesrc_include}"/>
                <param name="${val}_javac_exe" value="${compilesrc_jdk}/bin/javac"/>
                <param name="${val}_javac_compiler" value="${compilesrc_compiler}"/>
                <param name="${val}_compile_excludes" value="${compile_exclude}"/>
                <param name="${val}_classes_dir" value="${compile_classesdir}"/>
        </antcall>
   
        <echo message="Compilation ended:"/>
   
        <available file="${jar_path}" property="jar_exists"/>
        <condition property="is_jarfile">
                <and>
                        <isset property="jar_exists"/>
                        <matches string="${jar_path}" pattern=".*\.jar$"/>
                </and>
        </condition>
   
        <fail message="JAR file does not exists or is not a valid jar file" unless="is_jarfile"/>
        <echo message="JAR file is valid and available"/>
        <echo message="Computing the md5sum and sha1 values"/>
   
        <checksum file="${jar_path}" todir="checksums/" algorithm="MD5" property="md5"/>
        <checksum file="${jar_path}" todir="checksums/" algorithm="SHA1" property="sha1"/>
   
        <echo message="ORIGINAL MD5SUM of ${jar_path}: ${md5}"/>
        <echo message="MODIFIED SHA1 of ${jar_path}: ${sha1}"/>
   
        <echo message="Source files movement started"/>
        <antcall target="copytask">
           <param name="${val}_copytask" value="${compilesrc_sourcedir},${compile_classesdir},${compilesrc_include},${compile_exclude}"/>
        </antcall>
        <echo message="Source files movement ended"/>

        <echo message="JAR updation started"/>
        <echo message="JAVA_HOME used to update jar: ${jarjdk}"/>
        <antcall target="exec">
                <param name="${val}_exec_dir" value="${basedir}"/>
                <param name="${val}_exec_command" value="${jarjdk}/bin/jar"/>
                <param name="${val}_exec_args" value="uf ${jar_path} -C ${compile_classesdir} ."/>
        </antcall>
        <echo message="JAR updated with the class and source files"/>
