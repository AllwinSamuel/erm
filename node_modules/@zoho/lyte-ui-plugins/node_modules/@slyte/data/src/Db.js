...
class Db extends Service{
    ...
    static register(opts){
        ...
        Object.defineProperty(this, "Schema", {
            value: class Schema extends SchemaClass{
                // static register(opts, parent){
                //     this.__class = SchemaClass;
                //     var name;
                //     if(this.schemaName){
        ...
        var SM = this.Schema; 
        // var dbHashName = hash ? this.name+"#"+hash : this.name;
        this.Schema.schema = {};
    ...
    constructor(opts){
        ...
        if(this.constructor.Connector && newGetSuperClass(this.constructor.Connector, false, Connector)){
            var cparentName = Object.getPrototypeOf(this.constructor.Connector).name, context, defaultConnec;
            switch(cparentName){
                case "RESTConnector":
                    context = "REST";
                    break;
        ...
        if(this.constructor.Serializer && newGetSuperClass(this.constructor.Serializer, false, Serializer)){
            var sparentName = Object.getPrototypeOf(this.constructor.Serializer).name, context, defaultSerz;
            switch(sparentName){
                case "RESTSerializer":
                    context = "REST";
                    break;
        ...
        for(var dKey in dTypeDef){
            this.Transform[dKey] = dTypeDef[dKey];
        }
    ...
    triggerUpdate(def, pkVal, keys, qP, customData){
        ...
        var obj = {};
        var name = def._name;
        var ins = this.cache.getEntity(def.def, pkVal);
        if(ins){
            var def = ins.$.schema,
            pK = def._arrPk,
            ...
            return def.connector.constructor.put(this, name, obj, ins, true, customData, qP);	
        }
