...
function updateFieldValidation(db, schema, key, deserialize, ignoreValidation){
	...
	records.forEach(function(item){
		...
		if(deserialize){
			...
			if((item.hasOwnProperty(key) && (boolChk || empD) && db.Transform[fldTypeName].prototype.hasOwnProperty("deserialize"))){
				item[key] = db.Transform[fldTypeName].prototype.deserialize(item[key],key,schema._name,getpKVal(item,schema));
			}
		}
	...
	if(validateOptions){
		...
		if( obj || (record.hasOwnProperty(key) && !record.$.error.hasOwnProperty(key))){
			for(var property in field){
				var resp = checkProperty(property, val, key, field[property], record, isEntity(record) ? record.$.schema._name : undefined, db, Dberror.errorCodes, undefined, field, true);
				if(resp !== true){
		...
		if(!field){
			...
			if(value !== record[attr]){
				...
				if(!opts || (opts && opts.skipValidation !== true)){		
					for(var property in field){
						var resp = checkProperty(property, value, attr, field[property], record, schema._name, db, Dberror.errorCodes, undefined, field, true);
						if(resp != true){
							if(typeof resp == "object"){
								resp.value = value;
							} 
...
function partialData(rec, prop, relRec, type, polymorphicType){
	var arr = rec[prop];
...
function addDeepNest(record, extended,type,parentRel,ignoreRel, mp){
	var relatedRecord, 
	mp = mp || new Map(),
	db = record.$.schema.db,
	...
	relations.forEach(function(rels, key){
		rels.forEach(function(item){
			...
			if(!item.relKey){
				relatedRecord = getRelatedRecord(record,getSchemaObj(db, item.relatedTo),item.dummy);
			}
			else{
				relatedRecord = record[item.relKey];
			}
			var relMod = item.relatedTo;
			if(relatedRecord && relMod){
				inv = getBackwardRel(schema,item,getSchemaObj(db, item.relatedTo));
				var mppass = false, nmp;
				if(inv !== parentRel){		
					if(!mp.get(inv)){
		...
		relations.forEach(function(item){
			...
			if(bMod){
				...
				if(inv && inv.opts){
					...
					part = inv.opts.serialize == "partial" ? true : false;
					if(!item.relKey){
						invRecs = getRelatedRecord(rec,_bDef,item.dummy);
					}
					else{
					...
					if(Array.isArray(invRecs)){
						invRecs.forEach(function(invItm){
							if(invItm instanceof bMod){
								if(inv.relKey){	
									if(inv.relType == "belongsTo" && invItm && invItm.$ && invItm.$.partial && invItm.$.partial[inv.relKey] && invItm.$.partial[inv.relKey].has(rec)){
										...
										if(Array.isArray(invArr) && invArr.partial && invArr.partial.has(rec)){
...
function removeSelfCircularReference(db, name, obj, expose, type, partialObj, partialRef, parentRel){
	...
	for(var key in obj){
		...
		if(field == parentRel || (removePk && expose != true && expose != "state")){
			delete obj[key];
			continue;
		}
		if(!field){
		...
		if(field && field.type != "relation" && !expose  && db.Transform[field.type] && db.Transform[field.type].serialize && obj.hasOwnProperty(key)){			
			obj[key] = db.Transform[field.type].serialize(obj[key],key,record);
			continue;
		}
...
function toJSONObj(db, schema, data, expose, type, partial, parentRel){
	var copyObj, 
	pkVal, 
	name = schema._name, 
	pK = schema._pK;
	...
	removeSelfCircularReference(db, name, copyObj,expose,type, partialMp, partial && partial.ref ? partial.ref : undefined, parentRel);
	if(expose == "idb"){
...
function toJSON(db,name,obj,expose,type,partialObj,parentRel){
	var copyObj, 
	def = db.schema[name];
	db.$.recStack = {};
	if(Array.isArray(obj)){
		...
		for(var i=0; i<obj.length; i++){
			copyObj = toJSONObj(db, def, obj[i], expose, type, partialObj, parentRel);
			arr.push(copyObj);
		}
		return arr;
	}
	else if(obj && (typeof obj === "object" || isEntity(obj))){
		copyObj = toJSONObj(db, def, obj, expose, type, partialObj, parentRel);
	}
	db.$.recStack = {};
	return copyObj;
}
		...
		if(fldType != "relation"){
			...
			if(!skipValidation){
				for(var property in fieldKeys){
					var resp = checkProperty(property, val, field, fieldKeys[property], record, def._name, db, Dberror.errorCodes, undefined, fieldKeys, true);
					if(resp != true){
						if(typeof resp == "object"){
							resp.value = val;
						}
	...
	for(var key in data){
		...
		if(fld){
			if(fld.type == "relation" && data[key]){
				...
				if(data.hasOwnProperty(key) && (boolChk || empD) && db.Transform.hasOwnProperty(fieldKeys.type) && db.Transform[fieldKeys.type].hasOwnProperty("deserialize")){
					data[key] = db.Transform[fieldKeys.type].deserialize(data[key],key,def._name,getpKVal(data,def));
				}
			}
		}
		else{
...
function establishLink(db,forward,backward,data,relatedRecord,index,ignorePartial, isDuplicate){
	if(!relatedRecord){
		return "ERR13";
	}
	...
	if(forward.relType == "belongsTo"){
		...
		if(!ignorePartial && forward.opts && forward.opts.serialize){
			ret = partialData(data, fRelKey, relatedRecord, "added");
	...
	if(backward.dummy){
		...
		if(backward.relType == "belongsTo"){
			...
			if(!ignorePartial && backward.opts && backward.opts.serialize){
				bret = partialData(relatedRecord, bRelKey, data, "added");
			}
			var bnest;
			if(!ignorePartial){
...
function removeFromStore(def,keys,fromStore,ignorePartial, delayPer, onlyRem, deep, parentRel){
	var data = def.data,
	db = def.db,
	lyte = db.lyte;
	if(data.length == 0){
	...
	for(var i=0; i<keys.length; i++){
		...
		if(relations.size){
			toDemolishRelation(def, index, ignorePartial, onlyRem, delayPer);	
		}
		var deleted;
...
function toDemolishRelation(def,index,ignorePartial,onlyRem,delayPers){
	...
	relations.forEach(function(rel, key){
		for(var i=0; i<rel.length; i++){
			...
			if(relDef){
				...
				toDemolishLink(def, record, relation, ignorePartial, onlyRem, delayPers);
			}
		}		
	});
}

function toDemolishLink(def,record,relation,ignorePartial,onlyRem,delayPers){
	var db = def.db,
	...
	if(bRelation){
		if(Array.isArray(records)){
			for(var i=0; i<records.length; i++){
				demolishLink(db, record, priKey, records[i], bRelation.relKey, relation, bRelation, ignorePartial, undefined, onlyRem, delayPers);
			} 
		}
		else if(isEntity(records)){
			demolishLink(db, record, priKey, records, bRelation.relKey, relation, bRelation, ignorePartial, undefined, onlyRem, delayPers);
		}
	}
}
...
function demolishLink(db, record, priKey, relatedRecord, bRelKey, relation, bRelation, ignorePartial, ignoreAttrCheck, onlyRem, delayPers){
	...
	if(!ignorePartial && onlyRem !== true){
		...
		if(bRelation && bRelation.opts && bRelation.opts.serialize){
			ret = partialData(relatedRecord, bRelKey, record, "removed", polymorphicType);
	...
	for(var key in data){
		...
		if(field){
			if(field.type != "relation"){
				...
				var boolChk = !empD && data[key]
				if(data.hasOwnProperty(key) && (boolChk || empD) && db.Transform.hasOwnProperty(field.type) && db.Transform[field.type].hasOwnProperty("deserialize")){
					data[key] = db.Transform[field.type].deserialize(data[key],key,def._name,getpKVal(data,def));
				}
			...
			if(rec.hasOwnProperty(key)){
				...
				if(Array.isArray(data)){
					...
					for(var k=0; k<dLen; k++){
						establishLink(db,item,bRel,rec,data[k],undefined,true);										
					}
				}
				else{
					establishLink(db,item,bRel,rec,rec[key],undefined,true);
				}
			}
		}						
	});
...
function mergeResponse(db, data , schema , response , pK , partialObj, cPersist){
    ...
    removeDirtyStack(data);
    // removeDeepNest(data);
}

function removeDirtyStack(data){
    data.$._attributes = {};
        ...
        if(Array.isArray(partialObj)){				
            partialObj.forEach(function(item, index){
                if(item.$){
                    ...
                    data.partial ? data.partial.delete(item) : undefined;
                }
            });
        }
    }
...
function mergePartialObj(db, partialObj, data, response, doMerge, cPersist){
    ...
    if((partialObj.$.type == "added" || (data && data.$ && data.$.isNew == true)) && isRec){
