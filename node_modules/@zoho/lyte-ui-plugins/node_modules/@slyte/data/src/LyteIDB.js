...
class LyteIDB extends Service{
    ...
    constructor(opts){
        ...
        this.init = new Promise(function(resolve, reject){
          ...
          dbName = self.name = opts ? opts.name : db._name; 
          self.initWorker(opts?opts.url:undefined, db).then(function(worker){
            ...
            setTimeout(function(){
                if(!hasAnotherTab){
                    if(restoreData === false && localStorage.getItem("idb"+host)){
                      localStorage.setItem("idb"+host, 1);
    ...
    openIDB(models, name, version){
      LyteIDB.worker.postMessage({tolyteidb:true,models:models, type:"open", version:version, maintainOrder:LyteIDB.maintainOrder, name : "Slyte"+name});
    }
    ...
    removeIDBDatabase(){
        return new Promise(function(resolve, reject){
            var trans = indexedDB.deleteDatabase("lyte");
